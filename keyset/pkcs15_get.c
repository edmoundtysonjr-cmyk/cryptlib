...
static int setKeyAttributes( IN_HANDLE const CRYPT_HANDLE iCryptHandle,
							 INOUT_PTR const PKCS15_INFO *pkcs15infoPtr,
...
static int getTrustedCert( IN_ARRAY( noPkcs15objects ) \
						   ...
						   OUT_LENGTH_BOUNDED_Z( dataMaxLength ) \
								int *dataLength, 
						   IN_BOOL const BOOLEAN resetCertIndex )
	{
	...
	REQUIRES( isBooleanValue( resetCertIndex ) );
	...
	*dataLength = 0;
	...
	LOOP_MED_CHECKINC( trustedCertIndex < noPkcs15objects, 
					   trustedCertIndex++ )
		{
		ENSURES( LOOP_INVARIANT_MED_XXX( trustedCertIndex, 0, 
										 noPkcs15objects - 1 ) );
...
static int getConfigItem( IN_ARRAY( noPkcs15objects ) \
	...
	LOOP_INDEX i;
	int dataStartOffset, dataTotalSize;
	...
	*dataLength = 0;
	...
	LOOP_MED( ( i = 0, pkcs15infoPtr = NULL ), i < noPkcs15objects, i++ )
		{
	...
	ENSURES( dataTotalSize > 0 && dataTotalSize > dataStartOffset );
...
static int getItemFunction( INOUT_PTR KEYSET_INFO *keysetInfoPtr,
	...
	*iCryptHandle = CRYPT_ERROR;
	...
	if( pkcs15infoPtr->pubKeyData == NULL && \
		...
		retExt( CRYPT_ERROR_NOTFOUND, 
				( CRYPT_ERROR_NOTFOUND, KEYSET_ERRINFO, 
				  "No public key or certificate data present for" ) );
				  /* Rest added by caller */
		}
...
static int getSpecialItemFunction( INOUT_PTR KEYSET_INFO *keysetInfoPtr,
								   IN_ATTRIBUTE \
...
/* Fetch a sequence of certificates.  These functions are called indirectly 
   ...
   a certificate chain.  The 'stateInfo' value is handled as an OUT_INT 
   parameter rather than the more obvious INOUT_PTR since the state is managed 
   explicitly by the caller, who uses it to pass in a next-key-ID reference 
   instead of a current-key-ID one */
...
static int getItem( INOUT_ARRAY( noPkcs15objects ) PKCS15_INFO *pkcs15info, 
	...
	*certDataPtr = intToByte( tag );
	if( cryptStatusError( status ) )
		{
		/* Since we're being called indirectly by the certificate code, the
		   error reporting will be handled by that so we just pass the error
		   info up from the lower-level function without adding anything more
		   at this level */
		return( status );
		}
#if 0	/* This could in theory be used to propagate a label from a private 
		   key + certificate to the standalone certificate if it's read 
		   using cryptGetPublicKey() in order to allow it to be addressed
		   if written to another keyset, but it's probably better to use
		   the CRYPT_IATTRIBUTE_HOLDERNAME for this when the certificate is
		   written */
	if( pkcs15infoPtr->labelLength > 0 && \
		!( options & KEYMGMT_FLAG_DATAONLY_CERT ) )
		{
		...
		/* The object has a label, set it for the context.  Since failing to
		   do so isn't a fatal error we continue with a diagnostic warning */
		setMessageData( &msgData, ( MESSAGE_CAST ) pkcs15infoPtr->label, 
						pkcs15infoPtr->labelLength ); 
		...
		if( cryptStatusError( status ) )
			{
			DEBUG_DIAG(( "Couldn't set label for newly-created "
						 "certificate" ));
			}
		}
#endif /* 0 */
...
static int getFirstItemFunction( INOUT_PTR KEYSET_INFO *keysetInfoPtr,
								 OUT_HANDLE_OPT CRYPT_CERTIFICATE *iCertificate,
								 OUT_INT_Z_ERROR int *stateInfo,
								 IN_ENUM( KEYMGMT_ITEM ) \
									const KEYMGMT_ITEM_TYPE itemType,
...
static int getNextItemFunction( INOUT_PTR KEYSET_INFO *keysetInfoPtr,
								OUT_HANDLE_OPT CRYPT_CERTIFICATE *iCertificate,
...
int initPKCS15get( INOUT_PTR KEYSET_INFO *keysetInfoPtr )
	{
	assert( isWritePtr( keysetInfoPtr, sizeof( KEYSET_INFO ) ) );
