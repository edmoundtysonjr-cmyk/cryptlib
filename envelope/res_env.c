...
static int checkPgpUsage( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
...
int initEnvelopeEncryption( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	REQUIRES( isBooleanValue( copyContext ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int checkSignatureActionFunction( IN_PTR const ACTION_LIST *actionListPtr,
										 IN_INT_Z const int signingKeyPresent,
										 STDC_UNUSED const int param2 )
	{
	assert( isReadPtr( actionListPtr, sizeof( ACTION_LIST ) ) );

	REQUIRES( isBooleanValue( signingKeyPresent ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int checkMissingInfo( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	switch( envelopeInfoPtr->usage )
		...
		case ACTION_MAC:
			...
			if( findPreAction( envelopeInfoPtr, ACTION_KEYEXCHANGE_PKC ) == NULL && \
				...
				setObjectErrorInfo( envelopeInfoPtr, CRYPT_ENVINFO_KEY, 
		...
		case ACTION_SIGN:
			...
			if( findPostAction( envelopeInfoPtr, ACTION_SIGN ) == NULL )
				{
				setObjectErrorInfo( envelopeInfoPtr, CRYPT_ENVINFO_SIGNATURE, 
									CRYPT_ERRTYPE_ATTR_ABSENT );
				return( CRYPT_ERROR_NOTINITED );
				}
...
int addKeysetInfo( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	if( *iKeysetPtr != CRYPT_ERROR )
		{
		setObjectErrorInfo( envelopeInfoPtr, keysetFunction,
...
static int addPasswordInfo( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	if( actionResult == ACTION_RESULT_ERROR || \
		...
		{
		setObjectErrorInfo( envelopeInfoPtr, CRYPT_ENVINFO_PASSWORD,
							CRYPT_ERRTYPE_ATTR_PRESENT );
...
static int addPgpPasswordInfo( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	if( findPreAction( envelopeInfoPtr, ACTION_KEYEXCHANGE_PKC ) != NULL || \
		...
		{
		setObjectErrorInfo( envelopeInfoPtr, CRYPT_ENVINFO_PUBLICKEY,
							CRYPT_ERRTYPE_ATTR_PRESENT );
		return( CRYPT_ERROR_INITED );
		}
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int addContextInfo( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
						   IN_ENUM( ACTIONLIST ) \
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int addEnvelopeInfo( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
							IN_ATTRIBUTE const CRYPT_ATTRIBUTE_TYPE envInfo,
	...
	REQUIRES( sanityCheckEnvelope( envelopeInfoPtr ) );
	REQUIRES( ( envInfo >= CRYPT_OPTION_ENCR_ALGO && \
				envInfo <= CRYPT_OPTION_ENCR_MAC ) || \
			  ( envInfo == CRYPT_OPTION_ENCR_HASHPARAM ) || \
			  ( envInfo > CRYPT_ENVINFO_FIRST && \
				envInfo < CRYPT_ENVINFO_LAST ) ||
			  ( envInfo == CRYPT_IATTRIBUTE_INCLUDESIGCERT ) || \
	...
	switch( envInfo )
		...
		case CRYPT_ENVINFO_SESSIONKEY:
			...
			if( TEST_FLAG( envelopeInfoPtr->flags, ENVELOPE_FLAG_AUTHENC ) && \
				envelopeInfoPtr->type != CRYPT_FORMAT_PGP )
				{
				setObjectErrorInfo( envelopeInfoPtr, CRYPT_ENVINFO_INTEGRITY,
									CRYPT_ERRTYPE_ATTR_PRESENT );
				return( CRYPT_ERROR_INITED );
				}
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int addEnvelopeInfoString( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
