...
time_t getTokenTime( const CK_TOKEN_INFO *tokenInfo )
	...
	static_assert( sizeof( tokenInfo->utcTime ) > 14, "UTCTime field size" );
...
int getContextDeviceInfo( IN_HANDLE const CRYPT_HANDLE iCryptContext,
						  ...
						  OUT_PTR_PTR_COND PKCS11_INFO **pkcs11InfoPtrPtr )
	{
...
static int controlFunction( INOUT_PTR DEVICE_INFO *deviceInfo,
	...
	/* Handle token present/active checks.  Note that this doesn't 
	   explicitly set a value for the boolean that's returned but only 
	   checks that the token is present and the user is logged in, the 
	   boolean value is set at a higher level */
	...
	if( type == CRYPT_DEVINFO_SET_AUTHENT_SUPERVISOR )
		...
		if( pkcs11Info->defaultSSOPinLen <= 0 )
			{
			setObjectErrorInfo( deviceInfo, CRYPT_DEVINFO_INITIALISE, 
...
int genericEndFunction( /* const */ INOUT_PTR CONTEXT_INFO *contextInfoPtr )
	{
...
static void adjustKeyParity( INOUT_BUFFER_FIXED( length ) BYTE *key, 
	...
	LOOP_MED( i = 0, i < length, i++ )
		...
		ENSURES_V( LOOP_INVARIANT_MED( i, 0, length - 1 ) );
...
static int cipherInitKey( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
						  IN_BUFFER( keyLength ) const void *key, 
						  IN_LENGTH_SHORT const int keyLength )
	{
	static const CK_OBJECT_CLASS class = CKO_SECRET_KEY;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int hmacInitKey( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
						IN_BUFFER( keyLength ) const void *key, 
						IN_LENGTH_SHORT const int keyLength )
	{
	static const CK_OBJECT_CLASS class = CKO_SECRET_KEY;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int cipherGenerateKey( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int hmacGenerateKey( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
							IN_RANGE( bytesToBits( MIN_KEYSIZE ),
									  bytesToBits( CRYPT_MAX_PKCSIZE ) ) \
								const int keySizeBits )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int cipherEncrypt( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
						  INOUT_BUFFER_FIXED( length ) void *buffer, 
						  IN_LENGTH const int length, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int cipherDecrypt( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
						  INOUT_BUFFER_FIXED( length ) void *buffer, 
						  IN_LENGTH const int length, 
						  const CK_MECHANISM_TYPE mechanismType )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int cipherEncryptECB( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int cipherEncryptCBC( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
							 INOUT_BUFFER_FIXED( length ) BYTE *buffer, 
							 IN_LENGTH int length )
	{
	const CAPABILITY_INFO *capabilityInfoPtr = \
...
#if defined( USE_RC4 ) 
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int cipherEncryptCTR( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
							 INOUT_BUFFER_FIXED( length ) BYTE *buffer, 
							 IN_LENGTH int length )
	{
	const CAPABILITY_INFO *capabilityInfoPtr = \
...
#endif /* USE_RC4 */
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int cipherDecryptECB( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
							 INOUT_BUFFER_FIXED( length ) BYTE *buffer, 
							 IN_LENGTH int length )
	{
	const CAPABILITY_INFO *capabilityInfoPtr = \
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int cipherDecryptCBC( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
							 INOUT_BUFFER_FIXED( length ) BYTE *buffer, 
							 IN_LENGTH int length )
	{
	const CAPABILITY_INFO *capabilityInfoPtr = \
...
#if defined( USE_RC4 ) 
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int cipherDecryptCTR( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
							 INOUT_BUFFER_FIXED( length ) BYTE *buffer, 
							 IN_LENGTH int length )
	{
	const CAPABILITY_INFO *capabilityInfoPtr = \
...
static const PKCS11_MECHANISM_INFO mechanismInfoConv[] = {
	...
	{ CKM_RC4, CKM_RC4_KEY_GEN, CKM_RC4, CKF_NONE, 
	  CRYPT_ALGO_RC4, CRYPT_MODE_CFB, CKK_RC4,
	  genericEndFunction, cipherInitKey, cipherGenerateKey, 
...
static CK_MECHANISM_TYPE getMechanism( const GETMECH_TYPE mechType,
	...
	LOOP_MED( i = 0, 
			  i < FAILSAFE_ARRAYSIZE( mechanismInfoConv, \
									  PKCS11_MECHANISM_INFO ) && \
					mechanismInfoConv[ i ].cryptAlgo != cryptAlgo && \
					mechanismInfoConv[ i ].cryptAlgo != CRYPT_ERROR,
			  i++ )
		{
		ENSURES_EXT( LOOP_INVARIANT_MED( i, 0, 
										 FAILSAFE_ARRAYSIZE( mechanismInfoConv, \
															 PKCS11_MECHANISM_INFO ) - 1 ),
					 CKM_NONE );
		}
	...
	LOOP_MED_CHECKINC( i < FAILSAFE_ARRAYSIZE( mechanismInfoConv, \
											   PKCS11_MECHANISM_INFO ) && \
							mechanismInfoConv[ i ].cryptMode != cryptMode && \
							mechanismInfoConv[ i ].cryptAlgo != CRYPT_ERROR,
					   i++ )
		{
		ENSURES_EXT( LOOP_INVARIANT_MED_XXX( i, 0, 
											 FAILSAFE_ARRAYSIZE( mechanismInfoConv, \
																 PKCS11_MECHANISM_INFO ) - 1 ),
					 CKM_NONE );
		}
...
int setDevicePKCS11( INOUT_PTR DEVICE_INFO *deviceInfo, 
	...
	FNPTR_SET( deviceInfo->getRandomFunction, NULL );
			   /* Can only be set when the device is accessed since it 
			      depends on CKF_RNG being available */
