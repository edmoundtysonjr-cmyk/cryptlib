...
static int readKeyIdentifiers( INOUT_PTR STREAM *stream, 
							   INOUT_PTR PKCS15_INFO *pkcs15infoPtr,
							   IN_LENGTH const int endPos )
	{
	LOOP_INDEX identifierCount;
	int status;
	...
	LOOP_MED( ( identifierCount = 0, status = CRYPT_OK ),
		...
		switch( value )
			...
			case PKCS15_KEYID_ISSUERANDSERIALNUMBER:
				...
				status = getStreamObjectLength( stream, &iAndSLength, 8 );
			...
			case PKCS15_KEYID_URI:
				status = readCharacterString( stream, pkcs15infoPtr->uri, 
											  CRYPT_MAX_TEXTSIZE, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int readPubkeyAttributes( INOUT_PTR STREAM *stream, 
								 INOUT_PTR PKCS15_INFO *pkcs15infoPtr,
								 IN_LENGTH const int endPos, 
								 IN_BOOL const BOOLEAN isPubKeyObject )
	{
	...
	REQUIRES( isBufsizeRangeNZ( endPos ) && endPos > stell( stream ) );
	REQUIRES( isBooleanValue( isPubKeyObject ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int readCertAttributes( INOUT_PTR STREAM *stream, 
							   INOUT_PTR PKCS15_INFO *pkcs15infoPtr,
							   IN_LENGTH const int endPos )
	{
	...
	if( checkStatusLimitsPeekTag( stream, status, tag, endPos ) && \
		...
		readTime( stream, &pkcs15infoPtr->validFrom );
		status = readTime( stream, &pkcs15infoPtr->validTo );
		}
	else
		{
		if( checkStatusLimitsPeekTag( stream, status, tag, endPos ) && \
...
static int readClassAttributes( INOUT_PTR STREAM *stream, 
								INOUT_PTR PKCS15_INFO *pkcs15infoPtr,
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int readSubclassAttributes( INOUT_PTR STREAM *stream, 
								   INOUT_PTR PKCS15_INFO *pkcs15infoPtr,
								   IN_ENUM( PKCS15_OBJECT ) \
										const PKCS15_OBJECT_TYPE type )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int readTypeAttributes( INOUT_PTR STREAM *stream, 
							   INOUT_PTR PKCS15_INFO *pkcs15infoPtr,
							   IN_ENUM( PKCS15_OBJECT ) \
									const PKCS15_OBJECT_TYPE type,
	...
	REQUIRES( isEnumRange( type, PKCS15_OBJECT ) );
	REQUIRES( isBooleanValue( unrecognisedAttribute ) );
	REQUIRES( isBooleanValue( isCryptlibData ) );
...
int readObjectAttributes( INOUT_PTR STREAM *stream, 
						  INOUT_PTR PKCS15_INFO *pkcs15infoPtr,
						  IN_ENUM( PKCS15_OBJECT ) \
								const PKCS15_OBJECT_TYPE type, 
	...
	LOOP_INDEX i;
	int tag, length, outerLength, lastTag = CRYPT_ERROR, status;
	...
	LOOP_SMALL( i = 0,
		...
		ENSURES( LOOP_INVARIANT_SMALL( i, 0, 
									   FAILSAFE_ARRAYSIZE( allowedTypesTbl, \
														   ALLOWED_ATTRIBUTE_TYPES ) - 1 ) );
		}
	ENSURES( LOOP_BOUND_OK );
	...
	if( cryptStatusError( status ) )
		return( status );
	LOOP_MED( i = 0, 
			  i < FAILSAFE_ARRAYSIZE( allowedTypeInfo->subTypes, int ) && \
					allowedTypeInfo->subTypes[ i ] != CRYPT_ERROR,
			  i++ )
		{
		ENSURES( LOOP_INVARIANT_MED( i, 0, 
									 FAILSAFE_ARRAYSIZE( allowedTypeInfo->subTypes, \
														 int ) - 1 ) );
	...
	ENSURES( LOOP_BOUND_OK );
	ENSURES( i < FAILSAFE_ARRAYSIZE( allowedTypeInfo->subTypes, int ) );
	...
	/* Private-key objects come in two different flavours, the old PKCS #15 
	   1.0/1.1 form which only stores the private-key data and the PKCS #15 
