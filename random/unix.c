...
#include <pwd.h>
  ...
  #if 0	/* Formerly included alongside errno.h, presumably for very old
		   systems, but removed when some Linux distros replaced it with
		   a file with #error/#warning directives */
	#include <sys/errno.h>
  #endif /* 0 */
  #include <sys/ipc.h>
#endif /* !( QNX || MVS ) */
...
#endif /* Linux on x86 */
#if defined( __linux__ ) && ( OSVERSION >= 4 )
  /* For getauxval(), added in glibc 2.16 around 2012 */
  #include <sys/auxv.h>
#endif /* Linux */
...
static int getrusage( int who, OUT_PTR struct rusage *rusage )
	{
...
/* Static system information, only added once */

static int addStaticSystemInfo( void )
	{
#if defined( __linux__ ) && ( OSVERSION >= 4 )
	RANDOM_STATE randomState;
	BYTE buffer[ RANDOM_BUFSIZE + 8 ];
...
/* Hardware RNG data via /dev/hwrng */
...
static int getDevRandomData( void )
	...
	/* Because of a combination of various types of braindamage in the Linux
	   ...
	   entropy estimate via RNDGETENTCNT if available and display a warning
	...
	if( ( randFD = open( "/dev/random", O_RDONLY ) ) >= 0 )
		...
		if( randFD > 2 && \
			...
			DEBUG_DIAG(( "/dev/random doesn't contain enough entropy to "
						 ...
						 "in some operations that involve it failing" ));
			}
		close( randFD );
		}
#endif /* RNDGETENTCNT */
	...
	/* getrandom() was defined in kernel 3.17 and above but is rarely
	   ...
	   for the problem was created, https://lwn.net/Articles/802360/, in the
...
static FILE *my_popen( INOUT_PTR DATA_SOURCE_INFO *entry )
	{
...
static void externalSourcesPoll( const int existingEntropy )
	...
	if( gathererOldHandler.sa_handler != SIG_DFL )
		...
		memset( &newHandler, 0, sizeof( struct sigaction ) );
...
static int externalSourcesPollComplete( IN_BOOL const BOOLEAN force )
	{
	MESSAGE_DATA msgData;
...
#endif /* __iOS__  */
#ifdef USE_TPM_RNG
	DEBUG_PRINT_COND( debugRandom, ( ", system TPM RNG" ));
#endif /* USE_TPM_RNG */
	DEBUG_PRINT_COND( debugRandom, ( ".\n" ));
...
BOOLEAN forkCheck( IN_BOOL const BOOLEAN checkForked )
	{
	BOOLEAN hasForked;
