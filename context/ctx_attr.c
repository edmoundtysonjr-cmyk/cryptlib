...
static int exitError( INOUT_PTR CONTEXT_INFO *contextInfoPtr,
	...
	setObjectErrorInfo( contextInfoPtr, errorLocus, errorType );
	return( status );
	}

CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int exitErrorInited( INOUT_PTR CONTEXT_INFO *contextInfoPtr,
							IN_ATTRIBUTE const CRYPT_ATTRIBUTE_TYPE errorLocus )
	{
	assert( isWritePtr( contextInfoPtr, sizeof( CONTEXT_INFO ) ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int exitErrorNotInited( INOUT_PTR CONTEXT_INFO *contextInfoPtr,
							   IN_ATTRIBUTE const CRYPT_ATTRIBUTE_TYPE errorLocus )
	{
	assert( isWritePtr( contextInfoPtr, sizeof( CONTEXT_INFO ) ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int exitErrorNotFound( INOUT_PTR CONTEXT_INFO *contextInfoPtr,
							  IN_ATTRIBUTE const CRYPT_ATTRIBUTE_TYPE errorLocus )
	{
	assert( isWritePtr( contextInfoPtr, sizeof( CONTEXT_INFO ) ) );
	...
	switch( attribute )
		...
		case CRYPT_CTXINFO_MODE:
			...
			/* If the mode for the context isn't set to the initial default 
			   ...
			   vague, for stream ciphers it's (pseudo-)CFB, for block 
			   ciphers it's usually CBC unless we're working with specific 
			   hardware crypto that only supports one mode and that mode 
			   isn't CBC.  For now this only seems to be ECB so we add a 
			   special-case check for ECB-only operation */
			if( isStreamCipher( capabilityInfoPtr->cryptAlgo ) || \
				isSpecialStreamCipher( capabilityInfoPtr->cryptAlgo ) )
				{
				/* It's a stream cipher, the only possible mode is an
				   implicit (pseudo-)CFB so any attempt to change it isn't 
				   valid */
				if( value == CRYPT_MODE_CFB )
					return( CRYPT_OK );
				return( exitError( contextInfoPtr, CRYPT_CTXINFO_MODE,
		...
		case CRYPT_IATTRIBUTE_KEY_DLPPARAM:
			/* TLS 1.3 introduced additional nonstandard DH values that no-
			   one used, or possibly even wanted, alongside the standard 
			   ones, so the caller can signal the use of these alternative 
			   values by setting the LSB of the requested key size */
			status = loadDHparams( contextInfoPtr, value & ~1, 
								   ( value & 1 ) ? TRUE : FALSE );
			if( cryptStatusOK( status ) )
...
int setContextAttributeS( INOUT_PTR CONTEXT_INFO *contextInfoPtr,
	...
	switch( attribute )
		...
		case CRYPT_CTXINFO_IV:
			...
			/* Make sure that the data size is valid.  The AEAD modes 
			   require custom IV handling because of the special-snowflake 
		...
		case CRYPT_IATTRIBUTE_REKEY:
			...
			/* Some special-snowflake algorithms require rekeying on each 
			   ...
			   CRYPT_CTXINFO_KEY that works on already-initialised contexts.
			   We don't perform all of operations used for CRYPT_CTXINFO_KEY
			   because there's already a key loaded and we're just replacing
			   it with a new one */
			if( capabilityInfoPtr->cryptAlgo != CRYPT_ALGO_POLY1305 )
				return( CRYPT_ERROR_NOTAVAIL );
			if( dataLength < capabilityInfoPtr->minKeySize || \
				dataLength > capabilityInfoPtr->maxKeySize )
				return( CRYPT_ARGERROR_NUM1 );
