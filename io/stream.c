...
static int refillStream( INOUT_PTR STREAM *stream )
	{
	int length, status;
	...
	/* If we've moved to a different place in the file prepare to get new 
	   data into the buffer at the new location.  We may have reached EOF 
	   before this point but the seek will move us back before the EOF
	   position so we reset the stream's EOF indicator before continuing */
	if( TEST_FLAG( stream->flags, STREAM_FFLAG_POSCHANGED ) && \
		...
		/* The seek should have moved us back before the EOF position so
		   we reset the EOF indicator */
		CLEAR_FLAG( stream->flags, STREAM_FFLAG_EOF );
		}
	...
	/* We've refilled the stream buffer from the file, remember the 
	   details.  We don't update the position variables if there's been an 
	   explicit position change (e.g. via an sseek()) since that's already 
	   updated them, however there is one case where we need to do this and 
	...
	if( !TEST_FLAG( stream->flags, STREAM_FFLAG_POSCHANGED ) )
		...
		{
		/* We've moved to a different place in the file, if that's at or 
		   past the EOF then report an underflow error.  The reason why 
		   being exactly at EOF is also an error is because we're about to 
		   try reading data from the buffer so there needs to be at least 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int emptyStream( INOUT_PTR STREAM *stream, 
						IN_BOOL const BOOLEAN forcedFlush )
	{
	int status = CRYPT_OK;
	...
	REQUIRES_S( sanityCheckStream( stream ) );
	REQUIRES_S( stream->type == STREAM_TYPE_FILE );
	REQUIRES_S( isBooleanValue( forcedFlush ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int expandVirtualFileStream( INOUT_PTR STREAM *stream, 
...
int sgetc( INOUT_PTR STREAM *stream )
	{
	int ch;
...
int sread( INOUT_PTR STREAM *stream, 
	...
	/* Clear (some) return values.  We can't clear the return value for 
	   network streams because some work with composite structures 
	   containing metadata rather than simple I/O buffers */
	if( stream->type == STREAM_TYPE_MEMORY || \
		stream->type == STREAM_TYPE_FILE )
		{
#ifdef CONFIG_FUZZ
		/* When fuzzing, HTTP and EAP streams are simulated with memory 
		   streams */
		if( !sIsPseudoHTTPRawStream( stream ) && \
			!sIsPseudoEAPStream( stream ) && \
			!sIsPseudoHTTPStream( stream ) )
#endif /* CONFIG_FUZZ */
	...
	switch( stream->type )
		...
		case STREAM_TYPE_MEMORY:
...
#endif /* VIRTUAL_FILE_STREAM */
#ifdef CONFIG_FUZZ
			/* When fuzzing, we have to set up the memory stream read to 
			   look like a network read */
			if( sIsPseudoHTTPRawStream( stream ) || \
				sIsPseudoEAPStream( stream ) )
				{
				const NET_STREAM_INFO *netStream = \
			...
			if( sIsPseudoHTTPStream( stream ) )
				...
				ENSURES_S( httpDataInfo->bufSize >= stream->bufEnd );
		...
		case STREAM_TYPE_FILE:
			...
			/* Read the data from the file */
			LOOP_LARGE_REV_INITCHECK( dataLength = length, dataLength > 0 )
				{
		...
		case STREAM_TYPE_NETWORK:
			...
			if( cryptStatusError( status ) )
				...
				/* If the lower-level code has indicated that the error 
				   condition is fatal, record this fact and make it 
				   persistent for the stream */
				if( cryptStatusError( netStream->persistentStatus ) )
					{
					SET_FLAG( stream->flags, STREAM_FLAG_FATALERROR );
...
int sputc( INOUT_PTR STREAM *stream, IN_BYTE const int ch )
	{
	assert( isWritePtr( stream, sizeof( STREAM ) ) );
...
int swrite( INOUT_PTR STREAM *stream, 
...
RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int sSetError( INOUT_PTR STREAM *stream, IN_ERROR const int status )
	{
	assert( isWritePtr( stream, sizeof( STREAM ) ) );
...
STDC_NONNULL_ARG( ( 1 ) ) \
void sClearError( INOUT_PTR STREAM *stream )
	{
	assert( isWritePtr( stream, sizeof( STREAM ) ) );
...
static int skipStream( INOUT_PTR STREAM *stream, 
	...
	/* Unlike memory streams, file streams are quantised to the I/O buffer
	   ...
	   different position in the existing buffer, we have to adjust the
	   new position by the number of buffer-lots of data that we've already 
	   processed */
...
RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int setStreamBuffer( INOUT_PTR STREAM *stream, 
...
int sioctlSet( INOUT_PTR STREAM *stream, 
	...
	switch( type )
		...
		case STREAM_IOCTL_PARTIALREAD:
			REQUIRES_S( isBooleanValue( value ) );
...
RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int sioctlSetString( INOUT_PTR STREAM *stream, 
					 IN_ENUM( STREAM_IOCTL ) const STREAM_IOCTL_TYPE type, 
	...
	REQUIRES_S( ( sIsPseudoStream( stream ) && \
				...
				( ( stream->type == STREAM_TYPE_FILE || \
				  ...
				  ( type == STREAM_IOCTL_ERRORINFO || \
					type == STREAM_IOCTL_IOBUFFER ) ) || \
				( stream->type == STREAM_TYPE_NETWORK ) );
#endif /* Different build configurations */
	REQUIRES_S( isEnumRange( type, STREAM_IOCTL ) );
	REQUIRES_S( isBufsizeRangeNZ( dataLen ) );
	...
	switch( type )
		...
		case STREAM_IOCTL_IOBUFFER:
			REQUIRES_S( dataLen == 512 || dataLen == 1024 || \
						dataLen == 2048 || dataLen == 4096 || \
						dataLen == 8192 || dataLen == 16384 );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int sioctlGet( INOUT_PTR STREAM *stream, 
			   IN_ENUM( STREAM_IOCTL ) const STREAM_IOCTL_TYPE type, 
	...
	if( sIsPseudoStream( stream ) )
		...
		if( type == STREAM_IOCTL_GETCLIENTNAMELEN || \
			type == STREAM_IOCTL_GETCLIENTADDRLEN )
			*intDataPtr = 4;
		else
		if( type == STREAM_IOCTL_GETCLIENTNAME || 
			type == STREAM_IOCTL_GETCLIENTADDR )
			{
			REQUIRES_S( dataMaxLen >= 4 );
			memcpy( data, "test", 4 );
			}
		else
			{
	...
	switch( type )
		...
		case STREAM_IOCTL_CONNSTATE:
			...
			*intDataPtr = TEST_FLAG( netStream->nFlags, 
		...
		case STREAM_IOCTL_GETCLIENTNAMELEN:
			...
			if( netStream->clientAddressLen <= 0 )
				return( CRYPT_ERROR_NOTFOUND );
			*intDataPtr = netStream->clientAddressLen;
		...
		case STREAM_IOCTL_GETCLIENTPORT:
			...
			if( netStream->clientPort <= 0 )
				return( CRYPT_ERROR_NOTFOUND );
			*intDataPtr = netStream->clientPort;

			return( CRYPT_OK );
			}

		case STREAM_IOCTL_GETCLIENTADDR:
			{
			if( netStream->clientAddressBinaryLen <= 0 )
				return( CRYPT_ERROR_NOTFOUND );
			if( netStream->clientAddressBinaryLen > dataMaxLen )
				return( CRYPT_ERROR_OVERFLOW );
			REQUIRES_S( rangeCheck( netStream->clientAddressBinaryLen, 1, 
									dataMaxLen ) );
			memcpy( data, netStream->clientAddressBinary, 
					netStream->clientAddressBinaryLen );

			return( CRYPT_OK );
			}

		case STREAM_IOCTL_GETCLIENTADDRLEN:
			{
			REQUIRES_S( dataMaxLen == sizeof( int ) );

			if( netStream->clientAddressBinaryLen <= 0 )
				return( CRYPT_ERROR_NOTFOUND );
			*intDataPtr = netStream->clientAddressBinaryLen;
		...
		case STREAM_IOCTL_GETEXTRADATA:
			{
		...
		case STREAM_IOCTL_GETEXTRADATALEN:
			...
			return( getMetadataFunction( stream, NULL, 0, intDataPtr ) );
			}

		case STREAM_IOCTL_ISFATALERROR:
			{
			BOOLEAN *booleanDataPtr = ( BOOLEAN * ) data;

			REQUIRES_S( dataMaxLen == sizeof( BOOLEAN ) );

			*booleanDataPtr = TEST_FLAG( stream->flags, \
										 STREAM_FLAG_FATALERROR ) ? \
							  TRUE : FALSE;
...
int streamOffsetFromPosition( INOUT_PTR STREAM *stream,
	...
	REQUIRES( isBufsizeRange( startOffset ) );
...
int sFileToMemStream( OUT_PTR STREAM *memStream, 
	...
	if( !isWritePtr( memStream, sizeof( STREAM ) ) || \
		...
		{
		/* Since memStream() is an OUT_PTR parameter we can't use it with a
