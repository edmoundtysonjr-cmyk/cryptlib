...
int mapNetworkError( NET_STREAM_INFO *netStream, 
					 ...
					 IN_BOOL const BOOLEAN useHostErrorInfo, 
					 IN_ERROR int status )
	{
	...
	REQUIRES( isBooleanValue( useHostErrorInfo ) );
	REQUIRES( cryptStatusError( status ) );

	clearErrorInfo( &netStream->errorInfo );
	...
	LOOP_LARGE( i = 0, i < errorInfoSize && \
		...
		{
		ENSURES( LOOP_INVARIANT_LARGE( i, 0, errorInfoSize - 1 ) );
...
static int pingHost( INOUT_PTR NET_STREAM_INFO *netStream,
	...
	REQUIRES( isShortIntegerRangeMin( bufMaxSize, 64 ) );
...
static int processICMP( IN_BUFFER( icmpPacketLength ) const BYTE *icmpPacket,
	...
	REQUIRES( isShortIntegerRangeMin( icmpPacketLength, \
									  IP4_MIN_HEADERSIZE + ICMP_MIN_PACKETSIZE ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int diagnoseConnectionProblem( INOUT_PTR NET_STREAM_INFO *netStream, 
	...
	int length DUMMY_INIT, offset, status;
	...
	LOOP_MED( ( addrInfoCursor = addrInfoPtr, addressCount = 0 ),
		...
		{
		ENSURES( LOOP_INVARIANT_MED( addressCount, 0, IP_ADDR_COUNT - 1 ) );
	...
	FNPTR_SET( diagnosticNetStream.getMetadataFunctionOpt, NULL );
	...
	freeAddressInfo( addrInfoPtr );
	zeroise( &diagnosticNetStream, sizeof( NET_STREAM_INFO  ) );
	if( cryptStatusError( status ) )
		return( originalStatus );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int checkFirewallError( INOUT_PTR NET_STREAM_INFO *netStream )
	{
	...
	LOOP_MAX( i = 0, i < cbNeeded / sizeof( LPVOID ), i++ )
		...
		static const DRIVER_INFO driverInfoTbl[] = {
			...
			{ "mfenlfk.sys", 11, TRUE },	/* McAfee FW NDIS filter */
			...
			{ "symtdi.sys", 10, FALSE },	/* Symantec TDI (2K, XP) */
			{ "symtdiv.sys", 11, FALSE },	/* Symantec TDI (Vista) */
			{ "symnets.sys", 11, FALSE },	/* Symantec TDI (7 onwards) */
		...
		LOOP_INDEX_ALT driverIndex;
		int driverNameLen;

		ENSURES( LOOP_INVARIANT_MAX( i, 0, 
									 ( cbNeeded / sizeof( LPVOID ) ) - 1 ) );
		...
		LOOP_MED_ALT( driverIndex = 0, 
					  ...
					  driverIndex++ )
			{
			ENSURES( LOOP_INVARIANT_MED_ALT( driverIndex, 0, 
											 FAILSAFE_ARRAYSIZE( driverInfoTbl, \
																 DRIVER_INFO ) - 1 ) );
