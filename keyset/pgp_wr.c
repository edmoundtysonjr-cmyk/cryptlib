...
static int getUserID( IN_HANDLE const CRYPT_CONTEXT cryptHandle,
	...
	REQUIRES( isShortIntegerRangeMin( userIDmaxLength, 16 ) );

	/* Clear return values */
	REQUIRES( isShortIntegerRangeNZ( userIDmaxLength ) ); 
...
static int createSignedAttributes( OUT_BUFFER( attributeMaxLength, \
	...
	REQUIRES( isShortIntegerRangeMin( attributeMaxLength, \
									  MIN_CRYPT_OBJECTSIZE ) );

	/* Clear return values */
	REQUIRES( isShortIntegerRangeNZ( attributeMaxLength ) ); 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 3, 6, 11 ) ) \
static int createSubpacketSignature( OUT_BUFFER( sigMaxLength, \
	...
	*signatureLength = 0;
	...
	if( cryptStatusError( status ) )
		...
		retExt( status,
				( status, errorInfo,
				  "Couldn't hash PGP subpacket data" ) );
		}
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 3 ) ) \
int pgpWritePubkey( INOUT_PTR PGP_INFO *pgpInfoPtr,
	...
	if( cryptStatusError( status ) )
		...
		retExt( status,
				( status, errorInfo,
				  "Couldn't create PGP signed attributes" ) );
		}
	...
	if( cryptStatusError( status ) )
		...
		retExt( status,
				( status, errorInfo,
				  "Couldn't write PGP public key record" ) );
		}
	ENSURES( isIntegerRangeNZ( keyDataLength ) );
