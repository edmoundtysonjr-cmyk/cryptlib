...
/* Check whether anyone ever uses the higher compliance levels, this will
   produce a (harmless) compile error if built with 
   USE_CERTLEVEL_PKIX_FULL.  This check was added in cryptlib 3.4.3, 
   February 2016, and has never resulted in a problem report */
...
static int copyIssuerDnData( INOUT_PTR CERT_INFO *destCertInfoPtr,
	...
	REQUIRES( isShortIntegerRangeNZ( srcCertInfoPtr->issuerDNsize ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyRevocationInfo( INOUT_PTR CERT_INFO *certInfoPtr,
		...
		if( certInfoPtr->issuerDNsize != revInfoPtr->issuerDNsize || \
			...
			retExt( CRYPT_ERROR_INVALID,
					( CRYPT_ERROR_INVALID, CERTIFICATE_ERRINFO,
					  "%s issuer name doesn't match %s issuer name",
					  getCertTypeNameLC( certInfoPtr->type ),
					  getCertTypeName( revInfoPtr->type ) ) );
			}
		}
	else
		{
		...
		if( cryptStatusError( status ) )
			...
			retExt( status,
					( status, CERTIFICATE_ERRINFO,
					  "Couldn't copy issuer name to %s",
					  getCertTypeNameLC( certInfoPtr->type ) ) );
			}
		}
	...
	if( cryptStatusError( status ) )
		...
		if( status == CRYPT_ERROR_DUPLICATE )
			...
			retExt( status,
					( status, CERTIFICATE_ERRINFO,
					  "Certificate is already present in %s",
					  getCertTypeNameLC( certInfoPtr->type ) ) );
			}
		return( status );
		}
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyResponderURL( INOUT_PTR CERT_INFO *requestInfoPtr,
							 INOUT_PTR CERT_INFO *userCertInfoPtr )
	{
	...
	REQUIRES( isShortIntegerRangeNZ( urlSize ) );
...
static int setXyzzyKeyUsage( INOUT_PTR CERT_INFO *certInfoPtr,
	...
	if( !checkContextCapability( iCryptContext, 
		...
		{
		setObjectErrorInfo( certInfoPtr, CRYPT_CERTINFO_KEYUSAGE,
							CRYPT_ERRTYPE_ATTR_VALUE );
		retExt( CRYPT_ERROR_INVALID,
				( CRYPT_ERROR_INVALID, CERTIFICATE_ERRINFO,
				  "XYZZY %s must be signature-capable to create a "
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int copyKeyFromCertificate( INOUT_PTR CERT_INFO *destCertInfoPtr,
								   const CERT_INFO *srcCertInfoPtr )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int copyKeyFromContext( INOUT_PTR CERT_INFO *destCertInfoPtr,
							   IN_HANDLE const CRYPT_CONTEXT iCryptContext )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int checkCertWellFormed( INOUT_PTR CERT_INFO *certInfoPtr )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int sanitiseCertAttributes( INOUT_PTR CERT_INFO *certInfoPtr,
								   const DATAPTR_ATTRIBUTE templateListPtr )
	{
	LOOP_INDEX_PTR DATAPTR_ATTRIBUTE templateAttributeCursor;
	...
	LOOP_MAX( templateAttributeCursor = \
		...
		retExt( CRYPT_ERROR_INVALID,
				( CRYPT_ERROR_INVALID, CERTIFICATE_ERRINFO,
				  "%s contains conflicting attributes",
				  getCertTypeName( certInfoPtr->type ) ) );
		}
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyCertReqToCert( INOUT_PTR CERT_INFO *certInfoPtr,
							  INOUT_PTR CERT_INFO *certRequestInfoPtr )
	{
	int status;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyRevReqToCert( INOUT_PTR CERT_INFO *certInfoPtr,
							 INOUT_PTR CERT_INFO *revRequestInfoPtr )
	{
	int status;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyCertToRequest( INOUT_PTR CERT_INFO *crmfRequestInfoPtr,
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyCertToRevRequest( INOUT_PTR CERT_INFO *crmfRevRequestInfoPtr,
								 INOUT_PTR CERT_INFO *certInfoPtr )
	{
	int status;
	...
	if( DATAPTR_ISSET( crmfRevRequestInfoPtr->issuerName ) )
		{
		setObjectErrorInfo( crmfRevRequestInfoPtr,
							CRYPT_CERTINFO_CERTIFICATE,
							CRYPT_ERRTYPE_ATTR_PRESENT );
		retExt( CRYPT_ERROR_INITED,
				( CRYPT_ERROR_INITED, CERTIFICATE_ERRINFO,
				  "%s already contains an issuer name",
				  getCertTypeName( crmfRevRequestInfoPtr->type ) ) );
		}
	...
	ENSURES( rangeCheck( certInfoPtr->subjectDNsize, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyOcspReqToResp( INOUT_PTR CERT_INFO *certInfoPtr,
							  INOUT_PTR CERT_INFO *ocspRequestInfoPtr )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyCertToOCSPRequest( INOUT_PTR CERT_INFO *ocspRequestInfoPtr,
								  INOUT_PTR CERT_INFO *certInfoPtr )
	{
	...
	if( !ocspRequestInfoPtr->certHashSet )
		{
		setObjectErrorInfo( ocspRequestInfoPtr, CRYPT_CERTINFO_CACERTIFICATE,
							CRYPT_ERRTYPE_ATTR_ABSENT );
		retExt( CRYPT_ERROR_NOTINITED,
				( CRYPT_ERROR_NOTINITED, OCSPREQ_ERRINFO,
				  "%s doesn't contain CA certificate information",
				  getCertTypeName( ocspRequestInfoPtr->type ) ) );
		}
	...
	if( idLength > 256 )
		...
		REQUIRES( isShortIntegerRangeNZ( idLength ) );
	...
	if( cryptStatusError( status ) )
		...
		if( status == CRYPT_ERROR_DUPLICATE )
			...
			/* If this certificate is already present in the list, set the 
			   extended error code for it */
			setObjectErrorInfo( ocspRequestInfoPtr, 
								CRYPT_CERTINFO_CERTIFICATE,
								CRYPT_ERRTYPE_ATTR_PRESENT );
			retExt( status,
					( status, OCSPREQ_ERRINFO,
					  "%s is already present in %s",
					  getCertTypeName( certInfoPtr->type ),
					  getCertTypeNameLC( ocspRequestInfoPtr->type ) ) );
			}
		return( status );
		}
	...
	if( cryptStatusOK( status ) )
		...
		/* Since this isn't a critical extension (the ESSCertID is just a 
		   ...
		   there's a problem adding it, and ignore any additional error 
		   information */
		...
		( void ) addAttributeFieldString( &revInfoPtr->attributes,
								...
								FALSE, &localErrorInfo, &dummy1, &dummy2 );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyCaCertToOCSPReq( INOUT_PTR CERT_INFO *certInfoPtr,
								INOUT_PTR CERT_INFO *caCertInfoPtr )
	{
	HASH_FUNCTION_ATOMIC hashFunctionAtomic;
	STREAM stream;
	...
	if( cryptStatusError( status ) )
		...
		assert( DEBUG_WARN );
		sMemDisconnect( &stream );
		setObjectErrorInfo( certInfoPtr, CRYPT_CERTINFO_CACERTIFICATE,
							CRYPT_ERRTYPE_ATTR_VALUE );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyRtcsReqToResp( INOUT_PTR CERT_INFO *certInfoPtr,
							  IN_PTR const CERT_INFO *rtcsRequestInfoPtr )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyCertToRTCSRequest( INOUT_PTR CERT_INFO *rtcsRequestInfoPtr,
								  INOUT_PTR CERT_INFO *certInfoPtr )
	{
	...
	if( cryptStatusError( status ) )
		...
		if( status == CRYPT_ERROR_DUPLICATE )
			...
			/* If this certificate is already present in the list, set the 
			   extended error code for it */
			setObjectErrorInfo( rtcsRequestInfoPtr, 
								CRYPT_CERTINFO_CERTIFICATE,
								CRYPT_ERRTYPE_ATTR_PRESENT );
			retExt( status,
					( status, RTCSREQ_ERRINFO,
					  "%s is already present in %s",
					  getCertTypeName( certInfoPtr->type ),
					  getCertTypeNameLC( rtcsRequestInfoPtr->type ) ) );
			}
		return( status );
		}
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int copyUserCertInfo( INOUT_PTR CERT_INFO *certInfoPtr,
							 INOUT_PTR CERT_INFO *userCertInfoPtr,
...
int copyCertObject( INOUT_PTR CERT_INFO *certInfoPtr,
	...
	switch( certInfoType )
		...
		default:
			retIntError_Obj( addedCertInfoPtr->objectHandle );
		}
