...
/* When reading numeric values the encoder whose data we're processing may
   have added spurious leading zero padding.  The follow values defines the
   maximum number of leading zeroes that we'll accept and the maximum 
   allowable total bytes of data before leading zeroes are stripped */

#define MAX_LEADING_ZEROES			2
...
static int readNumericValue( INOUT_PTR STREAM *stream, 
							 ...
							 IN_BOOL const BOOLEAN isShortValue )
	{
	BYTE buffer[ MAX_NUMERIC_DATA_BYTES + 8 ];
	...
	*value = 0;
	...
	REQUIRES_S( rangeCheck( noBytes, 1, MAX_NUMERIC_DATA_BYTES ) );
	...
	if( buffer[ 0 ] == 0 && noBytes > 1 )
		{
		/* Encoding with leading zero(es) */
		LOOP_SMALL( i = 0, i < noBytes && buffer[ i ] == 0, i++ )
			{
			ENSURES_S( LOOP_INVARIANT_SMALL( i, 0, noBytes - 1 ) );
			}
		ENSURES_S( LOOP_BOUND_OK );
		if( i > MAX_LEADING_ZEROES )
			{
			/* More than two bytes of leading zeroes are treated as an 
			   error */
			return( sSetError( stream, CRYPT_ERROR_BADDATA ) );
			}
		noBytes -= i;
		if( noBytes <= 0 )
			{
			/* Broken encoding of the value zero as multiple zero bytes */
#if ( MAX_LEADING_ZEROES > 2 )	/* Already checked above if M_L_Z <= 2 */
			if( i > 2 )
				return( sSetError( stream, CRYPT_ERROR_BADDATA ) );
	...
	if( noBytes < 0 || noBytes > ( isShortValue ? 2 : 4 ) )
		{
		return( sSetError( stream, ( noBytes < 0 ) ? \
						   CRYPT_ERROR_BADDATA : CRYPT_ERROR_OVERFLOW ) );
		}
	...
	LOOP_SMALL( ( localValue = 0, i = 0 ), i < noBytes, i++ )
		...
		if( !isIntegerRangeMin( localValue, localValueTmp ) )
			...
			/* Integer overflow.  This shouldn't occur because we've already 
			   checked for it above, but we make the check explicit here */
			return( sSetError( stream, CRYPT_ERROR_BADDATA ) );
			}
		}
	ENSURES_S( LOOP_BOUND_OK );
	if( isShortValue )
		...
		if( !isShortIntegerRange( localValue ) || ( localValue & 0xFFFF8000UL ) )
			{
			/* Value must be < 32K for short values */
		...
		if( !isIntegerRange( localValue ) || ( localValue & 0x80000000UL ) )
			{
			/* Value must be < MAX_INTLENGTH for standard data */
			return( sSetError( stream, CRYPT_ERROR_OVERFLOW ) );
			}
		}
...
static int readLengthValue( INOUT_PTR STREAM *stream, 
	...
	*length = 0;
	...
	if( noLengthOctets > ( shortLen ? MAX_NUMERIC_DATA_BYTES_SHORT : \
									  MAX_NUMERIC_DATA_BYTES ) )
		return( sSetError( stream, CRYPT_ERROR_BADDATA ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int readNumeric( INOUT_PTR STREAM *stream, 
						OUT_OPT_LENGTH_Z long *value,
						IN_TAG_EXT const int tag,
						IN_BOOL const BOOLEAN isInteger )
	{
	...
	REQUIRES_S( isBooleanValue( isInteger ) );
	...
	if( length <= 0 )
		{
		/* A numeric value must consist of at least one byte, even if it's a
		   zero value */
		return( sSetError( stream, CRYPT_ERROR_BADDATA ) );
		}
...
static int readConstrainedData( INOUT_PTR STREAM *stream, 
	...
	*bufferLength = 0;
	...
	if( length <= bufferMaxLength )
		{
		*bufferLength = length;
		REQUIRES_S( rangeCheck( length, 1, bufferMaxLength ) );
		return( sread( stream, buffer, length ) );
		}

	/* Read the object, limiting the number of bytes read to the maximum 
	   buffer size.  The rangeCheck() here is a bit odd, it exists to 
	   document that there's a range check even though it's a tautological 
	   one */
	*bufferLength = bufferMaxLength;
	REQUIRES_S( rangeCheck( bufferMaxLength, 1, MAX_INTLENGTH_SHORT ) );
...
int readTag( INOUT_PTR STREAM *stream )
	{
	int tag, status;
...
RETVAL_RANGE( MAX_ERROR, MAX_TAG - 1 ) STDC_NONNULL_ARG( ( 1 ) ) \
int peekTag( INOUT_PTR STREAM *stream )
	{
	int tag, status;
...
int checkEOC( INOUT_PTR STREAM *stream )
	{
	BYTE eocBuffer[ 2 + 8 ];
	int tag, status;
...
int readRawObject( INOUT_PTR STREAM *stream, 
	...
	REQUIRES_S( isShortIntegerRangeMin( bufferMaxLength, 3 ) );
				/* Need to be able to process at least the tag, length, and 
				   one byte of content */
	REQUIRES_S( ( tag == NO_TAG ) || isValidTag( tag ) );
				/* Note tag != 0 */
...
static int readIntegerHeader( INOUT_PTR STREAM *stream, 
	...
	/* ASN.1 encoded values are signed while the internal representation is
	   ...
	   implementations get this wrong.  As with numeric value encodings, we 
	   allow up to MAX_LEADING_ZEROES bytes of non-DER leading zeroes.
	   
	   However this leads to a problem in that we need at least one zero
	   byte present to denote the value 0 so we can't just skip all leading 
	   ...
	   First we check for the simple case, a leading nonzero value */
	...
	LOOP_SMALL( noLeadingZeroes = 0, 
				...
				noLeadingZeroes++ )
		{
		ENSURES_S( LOOP_INVARIANT_SMALL( noLeadingZeroes, 0, length - 1 ) );
	...
	ENSURES_S( LOOP_BOUND_OK );
	if( noLeadingZeroes > MAX_LEADING_ZEROES )
		return( sSetError( stream, CRYPT_ERROR_BADDATA ) );
...
int readIntegerTag( INOUT_PTR STREAM *stream, 
...
static int readBignumInteger( INOUT_PTR STREAM *stream, 
	...
	if( length > CRYPT_MAX_PKCSIZE )
		return( sSetError( stream, CRYPT_ERROR_OVERFLOW ) );
	REQUIRES_S( rangeCheck( length, 1, CRYPT_MAX_PKCSIZE ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int readBignumTag( INOUT_PTR STREAM *stream, 
	...
	return( readBignumInteger( stream, bignum, minLength, maxLength, 
							   maxRange, tag, checkType ) );
	}

/* Special-case bignum routine that verifies that the bignum data to be read 
   corresponds to the in-memory bignum.  This is used after calling 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int checkBignumRead( INOUT_PTR STREAM *stream, 
					 INOUT_PTR TYPECAST( BIGNUM * ) struct BN *bignum, 
					 STDC_UNUSED IN_LENGTH_PKC const int dummy1, 
					 STDC_UNUSED IN_LENGTH_PKC const int dummy2, 
					 STDC_UNUSED IN_PTR_OPT TYPECAST( const BIGNUM * ) \
						const struct BN *dummy3, 
					 STDC_UNUSED IN_ENUM_OPT( BIGNUM_CHECK ) \
						const BIGNUM_CHECK_TYPE checkType,
					 IN_TAG_EXT const int tag )
	{
	void *bignumData;
...
/* Read a universal type and discard it, used to skip unknown or unwanted
   objects, the latter of which abound in PKI protocols.  The Opt variant 
   allows zero-length objects, required for particularly braindamaged PKI 
   protocols that specify mandatory fields that are actually optional, for 
   which the standard practice is to send a zero-length object which would 
   normally be treated as an error */
...
static int skipData( INOUT_PTR STREAM *stream,
	...
	REQUIRES_S( isBooleanValue( allowZeroLength ) );
	...
	if( length <= 0 )
		{
		/* If zero-length objects are allowed, we're done */
		if( allowZeroLength )
			return( CRYPT_OK );

		/* We're reading a data field so not having anything present in it 
		  is an error */
		return( sSetError( stream, CRYPT_ERROR_BADDATA ) );
		}
	return( sSkip( stream, length, MAX_INTLENGTH_SHORT ) );
...
RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int readUniversalData( INOUT_PTR STREAM *stream )
	{
	assert( isWritePtr( stream, sizeof( STREAM ) ) );
...
RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int readShortIntegerTag( INOUT_PTR STREAM *stream, 
...
RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int readEnumeratedTag( INOUT_PTR STREAM *stream, 
...
RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int readBooleanTag( INOUT_PTR STREAM *stream, 
...
int readOIDEx( INOUT_PTR STREAM *stream, 
	...
	/* Try and find the entry for the OID.  Since related groups of OIDs 
	   ...
	   for each entry, however it also means that we can't use matchOID() 
	   for the matching */
	LOOP_MED( oidEntry = 0, 
		...
		ENSURES_S( LOOP_INVARIANT_MED( oidEntry, 0, 
									   noOidSelectionEntries - 1 ) );

		/* Get the OID information at this position */
		oidPtr = oidSelection[ oidEntry ].oid;
		oidLength = sizeofOID( oidPtr );

		/* Check for a match-any wildcard OID */
		if( matchOID( oidPtr, oidLength, WILDCARD_OID ) )
			{
...
RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int readFixedOID( INOUT_PTR STREAM *stream, 
	...
	REQUIRES_S( rangeCheck( oidLength, MIN_OID_SIZE, MAX_OID_SIZE ) );
...
RETVAL STDC_NONNULL_ARG( ( 1, 2, 4 ) ) \
int readEncodedOID( INOUT_PTR STREAM *stream, 
	...
	REQUIRES_S( isShortIntegerRangeMin( oidMaxLength, MIN_OID_SIZE ) );
	REQUIRES_S( tag == NO_TAG || tag == BER_OBJECT_IDENTIFIER );

	/* Clear return values */
	REQUIRES_S( isShortIntegerRangeNZ( oidMaxLength ) ); 
...
static int readString( INOUT_PTR STREAM *stream, 
	...
	REQUIRES_S( ( isOctetString && \
				...
				( tag >= 0 && tag < MAX_TAG_VALUE ) );
	REQUIRES_S( isBooleanValue( isOctetString ) );
...
int readOctetStringTag( INOUT_PTR STREAM *stream, 
...
int readCharacterString( INOUT_PTR STREAM *stream, 
...
int readBitStringTag( INOUT_PTR STREAM *stream, 
	...
	/* Convert the bit count from the unused-remainder bit count into the 
	   total bit count, treating it as an error if we get close to the 32-
	   bit signed integer bit limit.  Not even CMP's crazy handlng of error 
	   subcodes gets close to 30-bit values, and it means we don't have to
	   perform all sorts of possible-overflow checks further on */
	noBits = ( length * 8 ) - noBits;
	if( noBits > 30 )
		return( sSetError( stream, CRYPT_ERROR_BADDATA ) );
	ENSURES_S( noBits >= 0 && noBits <= 30 );
	...
	LOOP_MED( ( value = 0, flag = 1, i = 0 ), i < noBits, 
			  ( flag <<= 1, i++ ) )
		{
		ENSURES_S( LOOP_INVARIANT_MED( i, 0, noBits - 1 ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int readTimeData( INOUT_PTR STREAM *stream, 
						 OUT_PTR time_t *timePtr, 
	...
	*timePtr = 0;
	...
	/* Read the encoded time data and make sure that the contents are 
	   valid */
	memset( buffer, 0, 16 );
	REQUIRES_S( rangeCheck( length, 1, 16 ) );
	status = sread( stream, buffer, length );
	if( cryptStatusError( status ) )
		return( status );
	...
	if( !isUTCTime )
		...
		if( cryptStatusError( status ) )
			return( status );
		ENSURES_S( rangeCheck( value, 19, 20 ) );
	...
	if( gmTime != utcTime )
		{
		DEBUG_DIAG(( "Applying correction for 1-hour time slip close to "
					 "DST changeover, %ld -> %ld", ( long ) *timePtr,
					 ( long ) ( *timePtr + 3600 ) ));
...
RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int readUTCTimeTag( INOUT_PTR STREAM *stream, 
...
RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int readGeneralizedTimeTag( INOUT_PTR STREAM *stream, 
							OUT_PTR time_t *timeVal, 
							IN_TAG_EXT const int tag )
	{
	int status;
...
/* Read a time value following the RFC 3280 rules for dealing with the 
   non-Y2K-safe times used in ASN.1 */

RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int readTime( INOUT_PTR STREAM *stream, 
			  OUT_PTR time_t *timeVal )
	{
	int status, tag;
	...
	*timeVal = 0;
	...
	if( cryptStatusError( status ) )
		return( status );
	if( tag == BER_TIME_UTC )
		return( readUTCTime( stream, timeVal ) );
	if( tag == BER_TIME_GENERALIZED )
		return( readGeneralizedTime( stream, timeVal ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int checkReadTag( INOUT_PTR STREAM *stream, 
						 IN_TAG_ENCODED_EXT const int tag,
						 IN_BOOL const BOOLEAN allowRelaxedMatch )
	{
	int tagValue, status;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int readObjectHeader( INOUT_PTR STREAM *stream, 
	...
	if( dataLength != CRYPT_UNUSED )
		...
		if( dataLength < minLength || \
			dataLength >= min( MAX_INTLENGTH_SHORT, MAX_BUFFER_SIZE ) )
			{
			return( sSetError( stream, CRYPT_ERROR_BADDATA ) );
			}
		}
...
static int readLongObjectHeader( INOUT_PTR STREAM *stream, 
	...
	if( dataLength != CRYPT_UNUSED )
		{
		if( dataLength < minLength || \
			dataLength >= min( MAX_INTLENGTH, MAX_BUFFER_SIZE ) )
			{
			return( sSetError( stream, CRYPT_ERROR_BADDATA ) );
			}
		}
...
int readSequenceExt( INOUT_PTR STREAM *stream, 
...
PARAMCHECK( lengthCheckType == LENGTH_CHECK_ZERO, length, OUT_OPT_LENGTH_SHORT_Z ) \
PARAMCHECK( lengthCheckType != LENGTH_CHECK_ZERO, length, OUT_OPT_LENGTH_SHORT ) \
int readSetExt( INOUT_PTR STREAM *stream, 
				/* PARAMCHECK */ int *length,
				IN_ENUM( LENGTH_CHECK ) \
					const LENGTH_CHECK_TYPE lengthCheckType )
	{
...
PARAMCHECK( lengthCheckType == LENGTH_CHECK_ZERO, length, OUT_OPT_LENGTH_SHORT_Z ) \
PARAMCHECK( lengthCheckType != LENGTH_CHECK_ZERO, length, OUT_OPT_LENGTH_SHORT ) \
int readConstructedExt( INOUT_PTR STREAM *stream, 
						/* PARAMCHECK */ int *length,
...
RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int readOctetStringHole( INOUT_PTR STREAM *stream, 
...
RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int readBitStringHole( INOUT_PTR STREAM *stream, 
					   OUT_OPT_LENGTH_SHORT_MIN( minLength ) int *length,
					   IN_LENGTH_SHORT const int minLength, 
					   IN_TAG const int tag )
	{
...
int readGenericHoleExt( INOUT_PTR STREAM *stream, 
						/* PARAMCHECK */ int *length,
						IN_LENGTH_SHORT_Z const int minLength, 
						IN_TAG_ENCODED const int tag,
						IN_ENUM( LENGTH_CHECK ) \
...
int readRawObjectAlloc( INOUT_PTR STREAM *stream, 
	...
	*objectLengthPtr = 0;
	...
	REQUIRES_S( isShortIntegerRangeNZ( objectLength ) );
