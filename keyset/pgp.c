...
void pgpFreeEntry( INOUT_PTR PGP_INFO *pgpInfo )
	...
	if( pgpInfo->keyData != NULL )
		{
		REQUIRES_V( isShortIntegerRangeNZ( pgpInfo->keyDataLen ) ); 
...
static BOOLEAN matchKeyID( IN_PTR const PGP_KEYINFO *keyInfo, 
						   IN_BUFFER( requiredIDlength ) const BYTE *requiredID,
						   IN_LENGTH_KEYID const int requiredIDlength,
						   IN_BOOL const BOOLEAN isPGPkeyID )
	{
	CRYPT_CONTEXT iLocalContext;
	MESSAGE_CREATEOBJECT_INFO createInfo;
	MESSAGE_DATA msgData;
	...
	REQUIRES_B( isBooleanValue( isPGPkeyID ) );
...
static PGP_INFO *findEntry( const PGP_INFO *pgpInfo,
	...
	LOOP_MED( i = 0, i < noPgpObjects, i++ )
		{
		const PGP_INFO *pgpInfoPtr;

		ENSURES_N( LOOP_INVARIANT_MED( i, 0, noPgpObjects - 1 ) );

		/* If there's no entry at this position, continue */
		pgpInfoPtr = &pgpInfo[ i ];
	...
	*iCryptHandle = CRYPT_ERROR;
	...
	if( ( keyIDtype == CRYPT_KEYID_NAME ) && \
		...
		LOOP_EXT( i = 0, i < PGP_KEYID_SIZE, i++, PGP_KEYID_SIZE + 1 )
			...
			ENSURES( LOOP_INVARIANT_EXT( i, 0, PGP_KEYID_SIZE - 1,
										 PGP_KEYID_SIZE + 1 ) );
...
static int setItemFunction( INOUT_PTR KEYSET_INFO *keysetInfoPtr,
	...
	switch( itemType )
		...
		case KEYMGMT_ITEM_PUBLICKEY:
			/* This is already enforced by the REQUIRES() precondition, the 
			   following check is present just to document the behaviour */
			if( password != NULL )
	...
	if( encryptionOnlyKey )
		...
		REQUIRES( isShortIntegerRangeNZ( keyDataSize ) );
...
static int shutdownFunction( INOUT_PTR KEYSET_INFO *keysetInfoPtr )
	...
	if( keysetInfoPtr->subType == KEYSET_SUBTYPE_PGP_PRIVATE )
		...
		LOOP_EXT( i = 0, i < MAX_PGP_OBJECTS, i++, MAX_PGP_OBJECTS + 1 )
			{
			ENSURES( LOOP_INVARIANT_EXT( i, 0, MAX_PGP_OBJECTS - 1,
										 MAX_PGP_OBJECTS + 1 ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int initPublicFunction( INOUT_PTR KEYSET_INFO *keysetInfoPtr, 
	...
	REQUIRES( isShortIntegerRangeNZ( sizeof( PGP_INFO ) ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int initPrivateFunction( INOUT_PTR KEYSET_INFO *keysetInfoPtr, 
								STDC_UNUSED const char *name,
								STDC_UNUSED const int nameLength,
								IN_ENUM( CRYPT_KEYOPT ) \
									const CRYPT_KEYOPT_TYPE options )
	...
	REQUIRES( isShortIntegerRangeNZ( sizeof( PGP_INFO ) * MAX_PGP_OBJECTS ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int setAccessMethodPGPPublic( INOUT_PTR KEYSET_INFO *keysetInfoPtr )
	{
	assert( isWritePtr( keysetInfoPtr, sizeof( KEYSET_INFO ) ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int setAccessMethodPGPPrivate( INOUT_PTR KEYSET_INFO *keysetInfoPtr )
	{
	assert( isWritePtr( keysetInfoPtr, sizeof( KEYSET_INFO ) ) );
