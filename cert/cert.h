...
/* The maximum number of entries in a CRL.  With real-world CRL sizes going
   ...
   following should be sufficient for all but insanely-sized CRLs */

#define MAX_CRL_ENTRIES			10000

#if MAX_CRL_ENTRIES >= FAILSAFE_ITERATIONS_MAX
  #error The maximum CRL entry count must be less than FAILSAFE_ITERATIONS_MAX
#endif /* MAX_CRL_ENTRIES > FAILSAFE_ITERATIONS_MAX */
...
/* Set the error locus and type.  This is used for certificate checking 
   ...
   parameter or only being available via a handle so that the usual
   setObjectErrorInfo() can't be used */
...
/* cryptlib object data is usually referred to via $(typename)InfoPtr,
   however with certificates we can have subjectCertInfoPtr and 
   issuerCertInfoPtr alongside the generic certInfoPtr, as well as type-
   specific names like userInfoPtr, revInfoPtr, and so on.  To deal with 
   this we define additional error-info access macros alongside the generic 
   CERTIFICATE_ERRINFO */

#define SUBJECTCERT_ERRINFO		&subjectCertInfoPtr->errorInfo
#define ISSUERCERT_ERRINFO		&issuerCertInfoPtr->errorInfo
#define PKIUSER_ERRINFO			&userInfoPtr->errorInfo
#define OCSPREQ_ERRINFO			&ocspRequestInfoPtr->errorInfo
#define RTCSREQ_ERRINFO			&rtcsRequestInfoPtr->errorInfo
#define CRL_ERRINFO				&crlInfoPtr->errorInfo
#define REVOCATION_ERRINFO		&revInfoPtr->errorInfo
#define CMS_ERRINFO				&attributeInfoPtr->errorInfo

/* Sometimes we need to perform a check on a certificate object that isn't 
   the one being processed, for example an issuer certificate being used to
   sign the actual certificate of interest.  Since the checking functions 
   can set error information for this ancillary certificate object, we need 
   to reset the error information after we've checked it */

#define resetErrorInfo( certInfoPtr ) \
		{ \
		clearObjectErrorInfo( certInfoPtr ); \
		clearErrorInfo( &( certInfoPtr )->errorInfo ); \
		}
...
/* The structure used to store the current policy set if we're doing full
   ...
   length match is checked in cert/chk_cert.c, where MAX_OID_SIZE is 
   visible).
...
typedef CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
		int ( *READCERT_FUNCTION )( INOUT_PTR STREAM *stream, 
									INOUT_PTR CERT_INFO *certInfoPtr );
typedef CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
		int ( *WRITECERT_FUNCTION )( INOUT_PTR STREAM *stream, 
