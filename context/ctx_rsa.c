...
static int initContext( OUT_PTR CONTEXT_INFO *contextInfo,
						OUT_PTR PKC_INFO *pkcInfo )
	{
	int status;
...
static int selfTest( void )
	...
	if( cryptStatusError( status ) || \
		memcmp( buffer, randomTestData, 128 ) )
		{
		staticDestroyContext( &contextInfo );
		return( CRYPT_ERROR_FAILED );
		}
...
/* Decrypt/sign a single block of data using the Chinese Remainder Theorem 
...
static int decryptFn( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
	...
	/* Move the data from the buffer into a bignum.  We need to make an 
	   ...
	   total length of 36 bytes.  In addition we can't use 
	   BIGNUM_CHECK_VALUE_PKC because the data may be formatted using PKCS 
	   #1 which will fail the validity check */
	status = importBignum( data, buffer, length, 36, CRYPT_MAX_PKCSIZE, 
						   &pkcInfo->rsaParam_n, BIGNUM_CHECK_NONE );
	if( cryptStatusError( status ) )
		return( status );
	...
	/* p2 = p2 - q2; if p2 < 0 then p2 = p2 + p.  In some extremely rare
	   ...
	   positive */
	CK( BN_sub( p2, p2, q2 ) );
	LOOP_SMALL_WHILE( bnStatusOK( bnStatus ) && BN_is_negative( p2 ) )
		{
...
static const CAPABILITY_INFO capabilityInfo = {
	...
	decryptFn, encryptFn, readPublicKeyRsaFunction, writePublicKeyRsaFunction
	};
