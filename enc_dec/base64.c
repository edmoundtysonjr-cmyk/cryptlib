...
static BOOLEAN checkBase64( INOUT_PTR STREAM *stream, 
							IN_BOOL const BOOLEAN isPEM )
	{
	...
	if( isPEM )
		...
		LOOP_LARGE( i = 0, i < ( BASE64_MAX_LINESIZE - \
			...
			ENSURES_B( LOOP_INVARIANT_LARGE( i, 0, 
											 BASE64_MAX_LINESIZE - \
													BASE64_MIN_LINESIZE ) );

			ch = byteToInt( decode( buffer[ i ] ) );
...
static int checkPEMHeader( INOUT_PTR STREAM *stream, 
	...
	*headerLength = 0;
	...
	LOOP_EXT_REV_CHECKINC( length >= 4, length--, LINEBUFFER_SIZE + 1 )
		{
		ENSURES( LOOP_INVARIANT_EXT_REV_XXX( length, 4, LINEBUFFER_SIZE,
											 LINEBUFFER_SIZE + 1 ) );
	...
	ENSURES( LOOP_BOUND_EXT_REV_OK( LINEBUFFER_SIZE + 1 ) );
	...
	LOOP_MED( lineCount = 0, lineCount < MAX_HEADER_LINES, lineCount++ )
		...
		if( isSSH && length > 0 )
			{
			/* Check whether we've hit the ':' separated embedded in the 
			   string */
			if( strFindCh( buffer, length, ':' ) < 0 )
				break;
			}
		if( isPGP && length <= 0 )
			{
			/* We've hit the blank-line delimiter */
			break;
			}
		}
...
static int checkEOL( IN_BUFFER( srcLen ) const BYTE *src, 
	...
	REQUIRES( isBufsizeRangeNZ( srcLen ) );
...
int base64checkHeader( IN_BUFFER( dataLength ) const BYTE *data, 
	...
	int position DUMMY_INIT, length, status;
	...
	REQUIRES( isBufsizeRangeMin( dataLength, MIN_CERTSIZE ) );
...
static int writeAccumulator( INOUT_PTR STREAM *stream, 
...
int base64decode( OUT_BUFFER( destMaxLen, *destLen ) void *dest, 
	...
	REQUIRES( isBufsizeRangeMin( destMaxLen, 10 ) );
	REQUIRES( isBufsizeRangeMin( srcLen, 10 ) );
	REQUIRES( isEnumRangeOpt( format, CRYPT_CERTFORMAT ) );

	/* Clear return values */
	REQUIRES( rangeCheck( destMaxLen, 10, MAX_BUFFER_SIZE ) ); 
	...
	*destLen = 0;
	...
	LOOP_EXT( srcIndex = 0,
			  ...
			  srcIndex++ /* Also incremented in loop body */, 
			  MAX_BUFFER_SIZE )
		{
		int value;
		...
		ENSURES( LOOP_INVARIANT_EXT_XXX( srcIndex, 0, srcLen - 1,
										 MAX_BUFFER_SIZE ) );
				 /* srcIndex may skip more than one character if we hit an 
				    EOL or similar */

		/* Process special-case characters */
		value = byteToInt( decode( src[ srcIndex ] ) );
		if( value == BERR )
			{
...
static const HEADER_INFO *getHeaderInfo( IN_ENUM_OPT( CRYPT_CERTTYPE ) \
	...
	LOOP_MED( index = 0, 
			  index < FAILSAFE_ARRAYSIZE( headerInfo, HEADER_INFO ) && \
					headerInfo[ index ].type != CRYPT_CERTTYPE_NONE,
			  index++ )
		{
		ENSURES_N( LOOP_INVARIANT_MED( index, 0, 
									   FAILSAFE_ARRAYSIZE( headerInfo, \
														   HEADER_INFO ) - 1 ) );
...
int base64encode( OUT_BUFFER( destMaxLen, *destLen ) char *dest, 
	...
	assert( destMaxLen >= 10 && isWritePtrDynamic( dest, destMaxLen ) );
	assert( isWritePtr( destLen, sizeof( int ) ) );
	assert( srcLen >= 8 && isReadPtrDynamic( src, srcLen ) );

	REQUIRES( isBufsizeRangeMin( destMaxLen, 10 ) && \
			  destMaxLen > srcLen );
	REQUIRES( isBufsizeRangeMin( srcLen, 8 ) );
	REQUIRES( isEnumRangeOpt( certType, CRYPT_CERTTYPE ) );
