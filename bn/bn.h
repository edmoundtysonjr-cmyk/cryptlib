...
/* Some of the temporary bignums used in multiplies grow to enormous sizes 
   ...
   twice/four times the normal size.
	...
	There is one special case where that won't work and that's when
	BIGNUM_BASE_ALLOCSIZE isn't a power of 2.  In this case the BN_mul() 
	code tries to allocate an oversize bignum sized at the closest power of 
	2 above BIGNUM_BASE_ALLOCSIZE (or at least its BIGNUM_ALLOC_WORDS_EXT2 
	equivalent), which exceeds BIGNUM_ALLOC_WORDS_EXT2.  To deal with this 
	we make BIGNUM_ALLOC_WORDS_EXT2 larger than we would for a standard 
	power-of-2 BIGNUM_BASE_ALLOCSIZE */

#define BIGNUM_ALLOC_WORDS_EXT		( BIGNUM_ALLOC_WORDS * 2 )
#if ( BIGNUM_BASE_ALLOCSIZE != 128 ) && \
	( BIGNUM_BASE_ALLOCSIZE != 256 ) && \
	( BIGNUM_BASE_ALLOCSIZE != 512 ) && \
	( BIGNUM_BASE_ALLOCSIZE != 1024 )
  #define BIGNUM_ALLOC_WORDS_EXT2	( BIGNUM_ALLOC_WORDS * 6 )
#else
  #define BIGNUM_ALLOC_WORDS_EXT2	( BIGNUM_ALLOC_WORDS * 4 )
#endif /* Non power-of-2 bignums */
...
/* Bignum flags.  BN_FLG_SCRATCH is set for bignums that aren't actually used 
   as bignums but merely as scratch storage for data in the 'd' array (used 
   by low-level internal functions which directly manipulate the bignum 
   data), which would fail a sanity check as standard bignums.  BN_FLG_INUSE 
   is used to verify that the BIGNUM_EXT_xxx's aren't double-allocated or 
   double-freed */
...
#define BN_FLG_INUSE		0x10	/* _EXT bignum is in use */
#define BN_FLG_ALLOC_EXT	0x20	/* Double-size bignum */
#define BN_FLG_ALLOC_EXT2	0x40	/* Quad-size bignum */
#define BN_FLG_MAX			0x7F	/* Maximum possible flag value */
...
/* Make sure that we're not using conflicting defines */

#if defined( SIXTY_FOUR_BIT_LONG ) && defined( SIXTY_FOUR_BIT ) 
  #error SIXTY_FOUR_BIT_LONG cant also define SIXTY_FOUR_BIT.
#endif /* SIXTY_FOUR_BIT_LONG && SIXTY_FOUR_BIT */
#if defined( SIXTY_FOUR_BIT_LONG ) && defined( THIRTY_TWO_BIT )
  #error SIXTY_FOUR_BIT_LONG cant also define THIRTY_TWO_BIT.
#endif /* SIXTY_FOUR_BIT_LONG && THIRTY_TWO_BIT */
#if defined( SIXTY_FOUR_BIT ) && defined( THIRTY_TWO_BIT )
  #error SIXTY_FOUR_BIT cant also THIRTY_TWO_BIT.
#endif /* SIXTY_FOUR_BIT && THIRTY_TWO_BIT */

/* Make sure that we have at least one option defined */

#if !defined( SIXTY_FOUR_BIT_LONG ) && !defined( SIXTY_FOUR_BIT ) && \
	!defined( THIRTY_TWO_BIT )
  #error At least one of SIXTY_FOUR_BIT_LONG, SIXTY_FOUR_BIT, or THIRTY_TWO_BIT must be defined.
#endif /* !( SIXTY_FOUR_BIT_LONG || SIXTY_FOUR_BIT || THIRTY_TWO_BIT */

/* LP64 architectures where long and long long are 64 bits, e.g. x86-64 
   Unix, Alpha, PPC64, Sparc64, and RISC-V64 */
...
/* Support functions to mask the OpenSSL memory-allocation mess.  bn_expand()
   ...
   An additional significant benefit to removing all of the on-demand 
   resizing that the original OpenSSL code did is that it closes a giant 
   timing side-channel that was present in the code there.
   ...
   IAR, which has a lot less braindamage than MQX, also redefines NULL in
   this manner (see "Implementation-defined behavior for Standard C: J.3.12 
   Library Functions" of the IAR C/C++ Development Guide), so we make an 
   exception for that as well */
...
#elif defined( __xlc__ ) || defined( __TANDEM )
...
/* The purpose, and much of the functioning, of the BN_CTX pool/stack/
   ...
   The BN_CTX array size is determined via the diagnostic code in ctx_bn.c,
   which for all operations indicates that the highest-used array index is 
   35 */

#define BN_CTX_ARRAY_SIZE		36
...
BOOLEAN BN_mod_mul( INOUT_PTR BIGNUM *r, 
					IN_PTR const BIGNUM *a, 
					IN_PTR const BIGNUM *b,
					IN_PTR const BIGNUM *m, INOUT_PTR BN_CTX *ctx );
CHECK_RETVAL_BOOL STDC_NONNULL_ARG( ( 1, 2, 3, 4 ) ) \
