...
static int selfTest( void )
	...
	ALIGN_DATA( hashState, SHA1_STATE_SIZE, 8 );
	void *hashStatePtr = ALIGN_GET_PTR( hashState, 8 );
	LOOP_INDEX i;
	int status;

	/* Test SHA-1 against values given in FIPS 180-1 */
	memset( hashStatePtr, 0, SHA1_STATE_SIZE );	/* Keep static analysers happy */
	LOOP_SMALL( i = 0, 
...
void shaHashBuffer( INOUT_PTR HASHINFO hashInfo,
	...
	assert( ( hashState != HASH_STATE_END && \
			...
			( hashState == HASH_STATE_END && \
			  ...
			  outBufMaxLength >= 20 ) );
	assert( ( inBuffer == NULL && inLength == 0 ) || \
			( inLength > 0 && isReadPtrDynamic( inBuffer, inLength ) ) );
...
void shaHashBufferAtomic( OUT_BUFFER_C( outBufMaxLength, 20 ) BYTE *outBuffer,
	...
	assert( isWritePtrDynamic( outBuffer, outBufMaxLength ) && \
			outBufMaxLength >= 20 );
	assert( inLength > 0 && isReadPtrDynamic( inBuffer, inLength ) );
