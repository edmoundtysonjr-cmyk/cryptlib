...
static int processKeyexchangeAction( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	int keyexAlgorithm DUMMY_INIT, keyexFormat = CRYPT_PKCFORMAT_DEFAULT;
	int status;
		...
		if( cryptStatusError( status ) || iCryptDevice != iKeyexDevice )
			...
			retExt( CRYPT_ERROR_INVALID,
					( CRYPT_ERROR_INVALID, errorInfo,
					  "%s object must be associated with the same crypto "
					  "device as the key exchange object",
	...
	/* Evaluate the size of the exported action.  If it's a conventional key
	   ...
	   reason to use the cryptlib format.  Note that this assumes that the 
	   first action is the one that we'll be exporting the key for, which is 
	...
	status = iCryptExportKey( NULL, 0, &preActionListPtr->encodedSize, 
						...
						actionListPtr->iCryptHandle, 
						preActionListPtr->iCryptHandle, errorInfo );
	if( cryptStatusOK( status ) )
		{
		status = krnlSendMessage( preActionListPtr->iCryptHandle,
	...
	if( cryptStatusOK( status ) && keyexAlgorithm == CRYPT_ALGO_RSA )
		{
	...
	/* If there are any key exchange actions that will result in indefinite-
	   ...
	   In theory we could opportunistically generate a deterministic encoding 
	   by counting the number of keyex actions and, if there's only one 
	   present and it's not overly long (which is almost always the case) 
	   emitting it into the currently-unused space at the end of the envelope
	   buffer, and then memmove()ing it into place in cms_env.c.  However 
	   this isn't really worth the effort as it's only ever going to be used
	   with the virtually nonexistent Elgamal and RSA-OAEP keyex mechanisms,
			...
			if( status != OK_SPECIAL )
				...
				retExtErr( status,
						   ( status, ENVELOPE_ERRINFO, &localErrorInfo, 
							 "Couldn't calculate key exchange size for "
							 "envelope header" ) );
				}
...
static int processSigningCerts( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int processSignatureAction( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
								   INOUT_PTR ACTION_LIST *actionListPtr )
	{
	...
	if( cryptStatusError( status ) )
		...
		retExtErr( status,
				   ( status, ENVELOPE_ERRINFO, &localErrorInfo,
					 "Couldn't calculate signature size for envelope "
					 "trailer" ) );
		}
...
int cmsPreEnvelopeSign( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	...
	if( TEST_FLAG( envelopeInfoPtr->flags, ENVELOPE_FLAG_ATTRONLY ) )
		...
		if( envelopeInfoPtr->payloadSize > 0 )
			{
			setObjectErrorInfo( envelopeInfoPtr, CRYPT_ENVINFO_DATASIZE,
