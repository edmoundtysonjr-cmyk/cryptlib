...
static int readAddressAndPort( INOUT_PTR SESSION_INFO *sessionInfoPtr, 
	...
	REQUIRES( isShortIntegerRangeMin( hostInfoMaxLen, 16 ) );

	/* Clear return value */
	REQUIRES( isShortIntegerRangeNZ( hostInfoMaxLen ) ); 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2, 3, 5 ) ) \
static int getAddressAndPort( INOUT_PTR SESSION_INFO *sessionInfoPtr, 
							  INOUT_PTR STREAM *stream,
							  OUT_BUFFER( hostInfoMaxLen, *hostInfoLen ) \
									char *hostInfo, 
							  IN_LENGTH_SHORT_MIN( 16 ) \
...
static int sendChannelResponse( INOUT_PTR SESSION_INFO *sessionInfoPtr,
								const long channelNo,
								IN_BOOL const BOOLEAN isSuccessful )
	{
	int status;
	...
	REQUIRES( isBooleanValue( isSuccessful ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int sendGlobalResponse( INOUT_PTR SESSION_INFO *sessionInfoPtr,
							   IN_BOOL const BOOLEAN isSuccessful )
	{
	int status;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int sendOpenResponseFailed( INOUT_PTR SESSION_INFO *sessionInfoPtr,
								   const long channelNo )
	{
	int status;
...
int processChannelOpen( INOUT_PTR SESSION_INFO *sessionInfoPtr, 
	...
	LOOP_SMALL( i = 0, 
				i < FAILSAFE_ARRAYSIZE( channelInfo, CHANNEL_TYPE_INFO ) && \
					channelInfo[ i ].channelName != NULL,
				i++ )
		{
		ENSURES( LOOP_INVARIANT_SMALL( i, 0, 
									   FAILSAFE_ARRAYSIZE( channelInfo, \
														   CHANNEL_TYPE_INFO ) - 1 ) );
...
int processChannelRequest( INOUT_PTR SESSION_INFO *sessionInfoPtr,
	...
	LOOP_MED( i = 0, 
			  i < FAILSAFE_ARRAYSIZE( requestInfo, REQUEST_TYPE_INFO ) && \
					requestInfo[ i ].requestName != NULL,
			  i++ )
		{
		ENSURES( LOOP_INVARIANT_MED( i, 0, 
									 FAILSAFE_ARRAYSIZE( requestInfo, \
														 REQUEST_TYPE_INFO ) - 1 ) );
