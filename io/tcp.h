...
/****************************************************************************
*																			*
*							FreeRTOS with FreeRTOS Sockets					*
*																			*
****************************************************************************/
...
/* FreeRTOS native sockets are another one of those implementations that
   define their own gratuitously incompatible names for all standard socket 
   functions */
...
#define select					FreeRTOS_select
#define setsockopt				FreeRTOS_setsockopt
#define shutdown				FreeRTOS_shutdown
#define socket					FreeRTOS_socket

#define FD_ZERO					FreeRTOS_FD_CLR
#define FD_ISSET				FreeRTOS_FD_ISSET
#define FD_SET					FreeRTOS_FD_SET

/* Unfortunately alongside the gratuitously incompatible naming FreeRTOS 
   also creates some gratuitously incompatible functions that look like 
   standard sockets functions but aren't.  The worst of these is 
   FreeRTOS_select(), which works as follows:

	SocketSet_t socketSet;

	socketSet = FreeRTOS_CreateSocketSet();
	if( socketSet == NULL ) -> error;
	FreeRTOS_FD_SET( socketSet, socket, eSELECT_READ | eSELECT_EXCEPT );
	result = FreeRTOS_select( socketSet, timeInTicks );
	FreeRTOS_DeleteSocketSet( socketSet );
	if( result != 0 && \
		FreeRTOS_FD_ISSET ( socket, socketSet ) )
		{
		if( result & eSELECT_EXCEPT ) -> error;
		if( !( result & eSELECT_READ ) -> error;
		// OK to read
		}

   There's no way that this can be turned into anything matching the 
   standard sockets API without writing an entire wrapper for it, but then
   the API also doesn't really support nonblocking sockets, IPv6, or a pile
   of other things, so we don't try and go any further for now */

#error The FreeRTOS native sockets implementation is too incomplete to use for now, use LWIP instead.
...
/* MQX has a select() that looks mostly like a normal select() but isn't,
   ...
   select() is given as milliseconds rather than a struct timeval, which 
   isn't defined in MQX.  To deal with this we map the standard names to 
   the MQX ones and call the MQX select via a wrapper that sets up the 
...
/* MQX uses MQX-specific unified error-information routines */

#define getErrorCode( socket )	RTCS_get_errno()
#define getHostErrorCode( socket ) RTCS_get_errno()
...
#include <limits.h>		/* Needed by ioLib.h */
#include <ioLib.h>
#include <selectLib.h>
#include <hostLib.h>
#include <sockLib.h>
...
/* Winsock uses Windows-specific unified error-information routines */

#define clearErrorState()           WSASetLastError( 0 )
#define getErrorCode( socket )		WSAGetLastError()
#define getHostErrorCode( socket )	WSAGetLastError()
...
/* Zephyr redefines the entire standard sockets API using its own wrappers
   that overlay the BSD sockets API (this is to deal with naming conflicts
   such as the generic close() vs. socket close() problem), but there's a 
   built-in mechanism that maps the Posix names back to the Zepher ones, 
   enabled with CONFIG_NET_SOCKETS_POSIX_NAMES */

#define CONFIG_NET_SOCKETS_POSIX_NAMES
#include <net/socket.h>

/* Zephyr doesn't have any h_errno with which to implement 
   getHostErrorCode(), the best that we can do is map it to a standard errno 
   read */

#define getErrorCode( socket )	errno
#define getHostErrorCode		getErrorCode
...
#include "lwip/netdb.h"
  ...
  #pragma message( "Warning: This appears to be LWIP 1.x, IPv6 and other functionality isn't available" )
  #define LWIP_1x
#else
  #define USE_IPv6
  #define USE_IPv6_DNSAPI
...
/* LWIP 1.x has small pieces of IPv6 (getaddrinfo(), freeaddrinfo()) but 
   none of the surrounding functions (gai_strerror()) or defines 
   (AI_PASSIVE, NI_NUMERICxxx, IPPROTO_IPV6, IPV6_V6ONLY, etc), so we 
   remove the two xxxaddrinfo()s (which exist as macros) and rename the 
   LWIP addrinfo struct to something else so that we can replace it with 
   our own one */
...
/* LWIP 2.x now has most of the IPv6 stuff but not getnameinfo().  We check 
   for NI_NUMERICHOST being defined as an indicator that this has been added 
   and tell the user to */

#ifdef NI_NUMERICHOST
  #error This version of LWIP appears to support getnameinfo(), please update io/tcp.h to use this directly.
#else
  #define NI_NUMERICHOST	0
  #define NI_NUMERICSERV	0
  #define getnameinfo		lwip_getnameinfo
  #define gai_strerror		lwip_gai_strerror	
...
/* The size of a v4 and v6 IP address and the number of IP addresses that we 
   try to connect to for a given host, used if we're providing an emulated
...
#elif defined( __FreeRTOS__ ) && defined( USE_FREERTOS_SOCKETS )
  /* FreeRTOS native sockets don't really do nonblocking sockets (see the
...
#elif defined( __FreeRTOS__ ) && defined( USE_FREERTOS_SOCKETS )
  /* FreeRTOS with native sockets doesn't implenent Nagle at all so there's
	 no need to disable it, to avoid having special handling for this in the
	 main code we define a non-valid option for setsockopt() that gets 
	 turned into a no-op */
  #define DISABLE_NAGLE_LEVEL		0		/* Not used by FreeRTOS */
  #define DISABLE_NAGLE_OPTION		12345
#elif defined( __MQXRTOS__ )
...
/* Error code handling */
  ...
  #define clearErrorState()			errno = 0
#endif /* OS-specific error state handling */
...
/* The handling of size parameters to socket functions is, as with most
   ...
   decided that it'd be better to use size_t, but then people complained 
   that this wasn't binary-compatible with existing usage because on 64-bit
   ...
   To resolve this mess (where it's possible), we try and use socklen_t if 
   we detect its presence (as usual every system does this differently, all 
   of the values below are needed to handle different systems), otherwise we 
   use int where we know it's safe to do so, and failing that we fall back 
   to size_t */
...
/* IPv6 emulation functions used to provide a single consistent interface.
  ...
  #elif defined( __FreeRTOS__ ) && defined( USE_FREERTOS_SOCKETS )
	#define OUT_OF_MEMORY_ERROR		pdFREERTOS_ERRNO_ENOMEM
	#define HOST_NOT_FOUND_ERROR	pdFREERTOS_ERRNO_EADDRNOTAVAIL
  #elif defined( __MQXRTOS__ )
...
#elif defined( __FreeRTOS__ ) && defined( USE_FREERTOS_SOCKETS )
  /* FreeRTOS sockets currently (2022) only support nonblocking I/O via the 
     global ipconfigSOCK_DEFAULT_RECEIVE_BLOCK_TIME setting which means that 
	 either all sockets have to be blocking or all sockets have to be 
	 nonblocking.  Hopefully this will be fixed in the future but for now we
	 have to assume the caller has set it as required */
...
#elif defined( __VxWorks__ )
  /* VxWorks should in theory use the FIONBIO case earlier but it defines
     ioctl() as 'int ioctl( int fd, int function, int arg )' rather than
	 a pointer for the last argument so we have to special-snowflake the
	 calls */
  #define getSocketNonblockingStatus( socket, value ) \
			{ \
			value = ioctlsocket( socket, FIONBIO, 0 ); \
			value = isSocketError( value ) ? TRUE : FALSE; \
			}
  #define setSocketNonblocking( socket ) \
...
int getAddressInfo( INOUT_PTR NET_STREAM_INFO *netStream, 
					OUT_PTR_PTR_COND struct addrinfo **addrInfoPtrPtr,
					IN_BUFFER_OPT( nameLen ) const char *name, 
					IN_LENGTH_Z const int nameLen, 
					IN_PORT const int port, 
					IN_BOOL const BOOLEAN isServer,
					IN_BOOL const BOOLEAN isStreamSocket );
STDC_NONNULL_ARG( ( 1 ) ) \
...
/* Prototypes for functions in dns_srv.c */
  ...
  int findHostInfo( INOUT_PTR NET_STREAM_INFO *netStream, 
