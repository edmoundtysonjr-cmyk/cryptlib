...
/* Since this is only incomplete test code, we don't handle error messages
   yet */
...
static int mapError( CRYPTOAPI_INFO *cryptoapiInfo, const int defaultError )
	...
	LOOP_LARGE_REV( messageLength = errorStringLen,
					messageLength > 0 && \
						( errorInfo->errorString[ messageLength - 1 ] == '\n' || \
						  errorInfo->errorString[ messageLength - 1 ] == '\r' ),
					messageLength-- )
		{
		ENSURES( LOOP_INVARIANT_REV( messageLength, 1, errorStringLen ) );
		}
...
static CRYPT_ALGO_TYPE capiToCryptlibID( const ALG_ID algID )
	...
	LOOP_MED( i = 0, 
			  i < FAILSAFE_ARRAYSIZE( algoMapTbl, MAP_TABLE ) && \
					algoMapTbl[ i ].source != CRYPT_ALGO_NONE,
			  i++ )
		{
		ENSURES_EXT( LOOP_INVARIANT_MED( i, 0, 
										 FAILSAFE_ARRAYSIZE( algoMapTbl, \
															 MAP_TABLE ) - 1 ),
					 CRYPT_ALGO_NONE );
...
static BYTE *copyMPI( BYTE *dest, const BYTE *src, const int srcLen,
	...
	LOOP_MAX( i = 0, i < srcLen, i++ )
		{
		ENSURES_N( LOOP_INVARIANT_MAX( i, 0, srcLen - 1 ) );
...
static int createExportKey( const HCRYPTPROV hProv, HCRYPTKEY *hPrivateKey, 
	...
	*keyBlobPtr++ = 1;
	...
	if( result )
		...
		*hPrivateKey = 0;
	REQUIRES( isShortIntegerRangeNZ( keyBlobLen ) ); 
...
static int importPlainKey( const HCRYPTPROV hProv, 
	...
	*dwPtr = CALG_RSA_KEYX;
	...
	LOOP_LARGE_REV( i = keyDataSize - 1, i >= 0, i-- )
		{
		ENSURES( LOOP_INVARIANT_REV( i, 0, keyDataSize - 1 ) );
	...
	*keyBlobPtr++ = 0;
	...
	REQUIRES( isShortIntegerRangeNZ( blobSize ) ); 
...
static int getCertificate( const CRYPTOAPI_INFO *cryptoapiInfo,
	...
	switch( keyIDtype )
		...
		case CRYPT_IKEYID_ISSUERANDSERIALNUMBER:
			...
			if( cryptStatusOK( status ) )
				status = getStreamObjectLength( &stream, &length, 3 );
			if( cryptStatusOK( status ) && !isShortIntegerRangeNZ( length ) )
				status = CRYPT_ERROR_BADDATA;
			if( cryptStatusOK( status ) )
				status = sMemGetDataBlock( &stream, &dataPtr, length );
...
static int getCertificateFromKey( CRYPTOAPI_INFO *cryptoapiInfo,
								  const HCRYPTKEY hKey,
								  IN_BOOL const BOOLEAN isSigningKey,
								  PCCERT_CONTEXT *pCertContextPtr )
	{
	PCCERT_CONTEXT pCertContext;
...
static int getFirstItemFunction( DEVICE_INFO *deviceInfoPtr, 
								 ...
								 const int options )
	{
	CRYPTOAPI_INFO *cryptoapiInfo = deviceInfoPtr->deviceCryptoAPI;
	PCCERT_CONTEXT pCertContext;
	PCCERT_CHAIN_CONTEXT pChainContext;
	MESSAGE_CREATEOBJECT_INFO createInfo;
	int status;
	...
	assert( isShortIntegerRangeMin( keyIDlength, 4 ) );
...
static int getNextItemFunction( DEVICE_INFO *deviceInfoPtr, 
								CRYPT_CERTIFICATE *iCertificate,
	...
	*iCertificate = CRYPT_ERROR;
	...
	setMessageCreateObjectIndirectInfo( &createInfo, 
							...
							CRYPT_CERTTYPE_CERTIFICATE, DEVICE_ERRINFO );
	status = krnlSendMessage( SYSTEM_OBJECT_HANDLE, 
							  IMESSAGE_DEV_CREATEOBJECT_INDIRECT,
							  &createInfo, OBJECT_TYPE_CERTIFICATE );
	if( cryptStatusError( status ) )
...
static int rsaSign( CONTEXT_INFO *contextInfoPtr, void *buffer, int length )
	...
	LOOP_LARGE( i = 2, i < length, i++ )
		{
		ENSURES( LOOP_INVARIANT_LARGE( i, 2, length - 1 ) );
		if( bufPtr[ i ] == 0 )
			break;
		}
	ENSURES( LOOP_BOUND_OK );
...
static int rsaDecrypt( CONTEXT_INFO *contextInfoPtr, void *buffer, int length )
	...
	LOOP_LARGE( i = 2, i < length - resultLength - 1, i++ )
		{
		ENSURES( LOOP_INVARIANT_LARGE( i, 2, length - resultLength ) );
		bufPtr[ i ] = 0xA5;
		}
	ENSURES( LOOP_BOUND_OK );
	bufPtr[ i ] = 0;
	...
	do
		...
		LOOP_LARGE( i = 0, 
					i < FAILSAFE_ARRAYSIZE( mechanismInfo, MECHANISM_INFO ) && \
						mechanismInfo[ i ].cryptAlgo != CRYPT_ALGO_NONE,
					i++ )
			{
			ENSURES( LOOP_INVARIANT_LARGE( i, 0, 
										   FAILSAFE_ARRAYSIZE( mechanismInfo, \
															   MECHANISM_INFO ) - 1 ) );
