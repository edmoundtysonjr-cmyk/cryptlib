...
static int processCertWrapper( INOUT_PTR STREAM *stream, 
							   OUT_LENGTH_SHORT_Z int *objectOffset, 
							   OUT_DATALENGTH_Z int *objectLength, 
							   IN_LENGTH_SHORT_Z const int objectStartPos )
	{
	...
	REQUIRES( isShortIntegerRange( objectStartPos ) );
	...
	*objectOffset = *objectLength = 0;
	...
	if( length > 0 )
		...
		if( length == CRYPT_UNUSED )
			...
			LOOP_SMALL( i = 0, i < 3, i++ )
				{
				ENSURES( LOOP_INVARIANT_SMALL( i, 0, 2 ) );
	...
	status = getStreamObjectLength( stream, &innerLength, MIN_CERTSIZE );
	if( cryptStatusOK( status ) )
		{
		innerOffset = stell( stream );
		ENSURES( isShortIntegerRangeNZ( innerOffset ) );
...
int getCertObjectInfo( INOUT_PTR STREAM *stream,
	...
	static const MAP_TABLE minLengthMapTable[] = {
				...
				{ CRYPT_ICERTTYPE_CMP_CERTSEQUENCE, MIN_CERTSIZE },
				{ CRYPT_ICERTTYPE_TLS_CERTCHAIN, MIN_CERTSIZE },
				{ CRYPT_ICERTTYPE_TLS13_CERTCHAIN, MIN_CERTSIZE },
				{ CRYPT_CERTTYPE_CRL, 64 },
				{ CRYPT_CERTTYPE_OCSP_RESPONSE, 64 },
				{ CRYPT_CERTTYPE_REQUEST_CERT, 64 },
				{ CRYPT_CERTTYPE_RTCS_REQUEST, 32 },
				{ CRYPT_CERTTYPE_RTCS_RESPONSE, 24 },
	...
	*objectType = CRYPT_CERTTYPE_NONE;
	...
	/* If it's an TLS certificate chain then there's no recognisable 
	   tagging, however the caller will have told us what it is */
	if( formatHint == CRYPT_ICERTTYPE_TLS_CERTCHAIN || \
	...
	*objectLength = length;
	offset = stell( stream );
	ENSURES( isShortIntegerRange( offset ) );
	...
	/* If it's a PKCS #7 certificate chain then there'll be an object 
	   identifier present, check the wrapper */
	...
	/* If it's a PKCS #12 mess then there'll be a version number, 3, 
	   present */
	if( tag == BER_INTEGER )
		{
		long value;
