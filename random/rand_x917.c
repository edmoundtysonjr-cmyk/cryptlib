...
static int fipsTest( INOUT_PTR RANDOM_INFO *testRandomInfo,
	...
	REQUIRES( isBooleanValue( isX931 ) );
	...
	LOOP_EXT( i = 0, i < VST_ITERATIONS, i++, VST_ITERATIONS + 1 )
		...
		ENSURES( LOOP_INVARIANT_EXT( i, 0, VST_ITERATIONS - 1,
									 VST_ITERATIONS + 1 ) );
...
int fipsTestX917( INOUT_PTR RANDOM_INFO *testRandomInfo )
	...
	if( cryptStatusOK( status ) )
		{
		LOOP_EXT( i = 0, i < 10000 && cryptStatusOK( status ), i++, 
				  10001 )
			{
			ENSURES( LOOP_INVARIANT_EXT( i, 0, 9999, 10001 ) );
