...
BOOLEAN sanityCheckSession( const SESSION_INFO *sessionInfoPtr )
	...
	if( !isEnumRangeOpt( sessionInfoPtr->errorLocus, CRYPT_ATTRIBUTE ) || \
		!isEnumRangeOpt( sessionInfoPtr->errorType, CRYPT_ERRTYPE ) )
		{
		DEBUG_PUTS(( "sanityCheckSession: Error info" ));
		return( FALSE );
		}
...
static BOOLEAN sanityCheckProtocolInfo( const PROTOCOL_INFO *protocolInfo )
	...
	if( !isFlagRangeZ( protocolInfo->clientReqAttrFlags, SESSION_NEEDS ) || \
		!isFlagRangeZ( protocolInfo->serverReqAttrFlags, SESSION_NEEDS ) )
		{
...
CHECK_RETVAL_BOOL STDC_NONNULL_ARG( ( 1 ) ) \
static BOOLEAN checkSessionFunctions( IN_PTR const SESSION_INFO *sessionInfoPtr )
	{
		...
		if( message != MESSAGE_DELETEATTRIBUTE && \
			...
			if( message == MESSAGE_SETATTRIBUTE || \
				...
				if( status == CRYPT_ERROR_INITED )
					{
					setObjectErrorInfo( sessionInfoPtr, messageValue, 
...
static int openSession( OUT_HANDLE_OPT CRYPT_SESSION *iCryptSession,
						...
						OUT_PTR_PTR_OPT SESSION_INFO **sessionInfoPtrPtr )
	{
	...
	*sessionInfoPtrPtr = sessionInfoPtr;
...
#ifdef USE_TLS
			case CRYPT_SESSION_TLS:
				sessionInfoPtr->sessionTLS = \
								( TLS_INFO * ) sessionInfoPtr->storage;
				break;
#endif /* USE_TLS */
	...
	/* Check that the protocol information is OK */
