...
static int sendChannelClose( INOUT_PTR SESSION_INFO *sessionInfoPtr,
							 const long channelNo,
							 IN_ENUM( CHANNEL ) const CHANNEL_TYPE channelType,
							 IN_BOOL const BOOLEAN closeLastChannel );
...
int processChannelControlMessage( INOUT_PTR SESSION_INFO *sessionInfoPtr,
	...
	switch( sshInfo->packetType )
		...
		default:
			...
			retExt( CRYPT_ERROR_BADDATA,
					( CRYPT_ERROR_BADDATA, SESSION_ERRINFO, 
					  "%s", stringBuffer ) );	/* Redundant %s needed for gcc */
			}
		}
...
static int sendChannelClose( INOUT_PTR SESSION_INFO *sessionInfoPtr,
	...
	REQUIRES( isBooleanValue( closeLastChannel ) );
...
int closeChannel( INOUT_PTR SESSION_INFO *sessionInfoPtr,
	...
	if( closeAllChannels )
		...
		LOOP_MED( noChannels = 0, 
				  ...
				  noChannels++ )
			{
			ENSURES( LOOP_INVARIANT_MED( noChannels, 0, SSH_MAX_CHANNELS ) );
		...
		ENSURES( LOOP_BOUND_OK );
		ENSURES( noChannels <= SSH_MAX_CHANNELS );
		}
	else
		{
