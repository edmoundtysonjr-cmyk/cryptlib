...
/* Check an encoded certificate for a matching key usage.  The semantics of 
   ...
   likely to be usable for purpose X.  If it gets it wrong then the 
   certificate-level checking will catch the problem */
...
static BOOLEAN checkCertUsage( IN_BUFFER( certLength ) const BYTE *certificate, 
	...
	REQUIRES_B( isShortIntegerRangeMin( certLength, MIN_CRYPT_OBJECTSIZE ) );
	REQUIRES_B( isFlagRange( requestedUsage, KEYMGMT ) );
	REQUIRES_B( requestedUsage & KEYMGMT_MASK_USAGEOPTIONS );

	/* If there's not enough data to contain any extensions, there can't be 
	...
	/* Scan the payload portion of the certificate for the keyUsage 
	   extension.  The certificate is laid out approximately as:
		...
		[ junk ][ DN ][ validity ][ DN ][ pubKey ][ attrs ][ sig ]
...
/* Check whether an item is present in a keyset without fetching any data */

CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 4 ) ) \
int presenceCheck( INOUT_PTR DBMS_INFO *dbmsInfo,
	...
	/* Check whether this item is present.  We don't care about the result 
	   data, all we want to know is whether it's there or not so we just do 
	   a check rather than a fetch of any data */
...
int getItemData( INOUT_PTR DBMS_INFO *dbmsInfo, 
	...
	*iCertificate = CRYPT_ERROR;
	...
	LOOP_MED( itemCount = 0, itemCount < 32, itemCount++ )
		{
		ENSURES( LOOP_INVARIANT_MED( itemCount, 0, 31 ) );
		...
		if( cryptStatusError( status ) )
			...
			/* Convert the error code to a more appropriate value if
			   necessary.  Note that we don't use retExt() at this point
			   because the extended error information has already been set in
			   the hidden dbmsInfo parameter sent to dbmsQuery(), which 
			   retExt() would overwrite */
	...
	if( cryptStatusError( status ) )
		...
		retExtErr( status, 
				   ( status, KEYSET_ERRINFO, getDbmsErrorInfo( dbmsInfo ),
					 "Certificate database read operation failed" ) );
		}
...
static int getItemFunction( INOUT_PTR KEYSET_INFO *keysetInfoPtr,
	...
	*iCryptHandle = CRYPT_ERROR;
	...
	if( flags & KEYMGMT_FLAG_CHECK_ONLY )
		{
		status = presenceCheck( dbmsInfo, itemType, keyIDtype, keyID, 
								keyIDlength );
		if( cryptStatusError( status ) )
			{
			retExtErr( status, 
					   ( status, KEYSET_ERRINFO, getDbmsErrorInfo( dbmsInfo ),
						 "Certificate database presence-check operation "
						 "failed" ) );
			}
	...
	status = iCryptImportCertIndirect( iCryptHandle, 
									   keysetInfoPtr->objectHandle,
	...
	if( cryptStatusError( status ) )
		...
		retExtErr( status,
				   ( status, KEYSET_ERRINFO, &localErrorInfo,
				     "Couldn't import certificate chain from database" ) );
		}
...
static int setSpecialItemFunction( INOUT_PTR KEYSET_INFO *keysetInfoPtr,
	...
	if( dataLength == 6 && strSame( data, "cancel", dataLength ) )
		...
		if( cryptStatusError( status ) )
			...
			retExtErr( status, 
					   ( status, KEYSET_ERRINFO, getDbmsErrorInfo( dbmsInfo ),
						 "Certificate database query cancel operation "
						 "failed" ) );
			}
		return( CRYPT_OK );
		}
	...
	if( cryptStatusError( status ) )
		...
		retExtErr( status, 
				   ( status, KEYSET_ERRINFO, getDbmsErrorInfo( dbmsInfo ),
					 "Certificate database query operation failed" ) );
		}
	return( CRYPT_OK );
	}
...
int initDBMSread( INOUT_PTR KEYSET_INFO *keysetInfoPtr )
	{
	assert( isWritePtr( keysetInfoPtr, sizeof( KEYSET_INFO ) ) );
