...
STDC_NONNULL_ARG( ( 1 ) ) \
static void cleanupReqResp( INOUT_PTR SESSION_INFO *sessionInfoPtr,
							IN_BOOL const BOOLEAN preTransaction )
	{
	...
	REQUIRES_V( isBooleanValue( preTransaction ) );
...
int initSessionNetConnectInfo( IN_PTR const SESSION_INFO *sessionInfoPtr,
	...
	if( sessionInfoPtr->subProtocol == CRYPT_SUBPROTOCOL_EAPTTLS || \
		...
		connectInfo->subProtocol = sessionInfoPtr->subProtocol;
		connectInfo->authName = userNamePtr->value;
		connectInfo->authNameLength = userNamePtr->valueLength;
		connectInfo->authKey = keyPtr->value;
		connectInfo->authKeyLength = keyPtr->valueLength;
		}
#endif /* USE_EAP */
...
/* Write a fixed-size value, meaning a variable-length integer that some
   protocol requires be padded with leading zeroes to a fixed size */
...
int writeFixedsizeValue( INOUT_PTR STREAM *stream,
	...
	REQUIRES( dataLen >= 20 && dataLen <= fixedSize && \
			  dataLen <= CRYPT_MAX_PKCSIZE );
	...
	return( swrite( stream, data, dataLen ) );
	}
...
static CRYPT_ATTRIBUTE_TYPE checkClientParameters( IN_PTR const SESSION_INFO *sessionInfoPtr )
	{
...
static int activateConnection( INOUT_PTR SESSION_INFO *sessionInfoPtr )
	{
	...
	if( errorAttribute != CRYPT_ATTRIBUTE_NONE )
		{
		setObjectErrorInfo( sessionInfoPtr, errorAttribute, 
	...
	if( sessionInfoPtr->receiveBuffer == NULL )
		...
		REQUIRES( isBufsizeRangeMin( sessionInfoPtr->receiveBufSize, \
									 MIN_BUFFER_SIZE ) );
		REQUIRES( sessionInfoPtr->sendBufSize == CRYPT_UNUSED || \
				  isBufsizeRangeMin( sessionInfoPtr->sendBufSize, \
									 MIN_BUFFER_SIZE ) );
	...
	ENSURES( sessionInfoPtr->receiveBuffer != NULL && \
			 isBufsizeRangeMin( sessionInfoPtr->receiveBufSize, \
								MIN_BUFFER_SIZE ) );
	...
	if( sessionInfoPtr->subProtocol != CRYPT_SUBPROTOCOL_NONE && \
		...
		/* Record the fact that the layered protocol has been 
		   activated */
		SET_FLAG( sessionInfoPtr->flags, SESSION_FLAG_SUBPROTOCOL_ACTIVE );
		}
		...
		if( sessionInfoPtr->subProtocol != CRYPT_SUBPROTOCOL_NONE )
			...
			switch( sessionInfoPtr->subProtocol )
				...
				case CRYPT_SUBPROTOCOL_PEAP:
					/* These subprotocol types affect the network transport
...
int closeSession( INOUT_PTR SESSION_INFO *sessionInfoPtr )
	...
	if( sessionInfoPtr->subProtocol != CRYPT_SUBPROTOCOL_NONE )
		...
		/* If protocol management is handled by an outer protocol, don't 
		   perform a session shutdown.  This is a bit specific to the outer 
		   protocol type, in this case it's EAP over RADIUS with the outer 
		   layers taking precedence over the TLS tunnel that's being carried.  
		   What this means is that a session is regarded as complete once 
		   the outer layers report Access-Accept (RADIUS) or Success (EAP)
		   regardless of the state of the TLS tunnel carried over them, so 
		   there's no way to continue the outer session in order to perform 
		   the TLS shutdown */
		if( sessionInfoPtr->subProtocol == CRYPT_SUBPROTOCOL_EAPTTLS || \
			sessionInfoPtr->subProtocol == CRYPT_SUBPROTOCOL_PEAP )
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int defaultClientStartupFunction( INOUT_PTR SESSION_INFO *sessionInfoPtr )
	{
	...
	if( cryptStatusError( status ) )
		return( status );
	DEBUG_PRINT_COND( connectInfo.nameLength > 0,
					  ( "Connecting to server '%.*s' on port %d.\n",
					    connectInfo.nameLength, connectInfo.name, 
						connectInfo.port ));
	DEBUG_PRINT_COND( connectInfo.nameLength <= 0,
					  ( "Connecting via externally-supplied socket.\n" ));
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int defaultServerStartupFunction( INOUT_PTR SESSION_INFO *sessionInfoPtr )
	{
	NET_CONNECT_INFO connectInfo;
	STREAM_PROTOCOL_TYPE protocolType = STREAM_PROTOCOL_TCP;
	...
	DEBUG_PRINT(( "Received connection from '%.*s'.\n", 
				  nameLen, sessionInfoPtr->receiveBuffer ));
...
STDC_NONNULL_ARG( ( 1 ) ) \
static void defaultShutdownFunction( INOUT_PTR SESSION_INFO *sessionInfoPtr )
	{
	assert( isWritePtr( sessionInfoPtr, sizeof( SESSION_INFO ) ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int defaultGetAttributeFunction( INOUT_PTR SESSION_INFO *sessionInfoPtr,
										OUT_PTR void *data,
										IN_ATTRIBUTE \
