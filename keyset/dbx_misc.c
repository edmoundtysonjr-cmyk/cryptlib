...
int makeKeyID( OUT_BUFFER( keyIdMaxLen, *keyIdLen ) char *keyID, 
	...
	/* Clear return values */
	REQUIRES( isShortIntegerRangeNZ( keyIdMaxLen ) ); 
...
int getKeyID( OUT_BUFFER( keyIdMaxLen, *keyIdLen ) char *keyID, 
	...
	REQUIRES( isShortIntegerRangeMin( keyIdMaxLen, 16 ) );
...
int extractCertData( IN_HANDLE const CRYPT_CERTIFICATE iCryptCert, 
	...
	REQUIRES( isShortIntegerRangeMin( certDataMaxLength, 
	...
	/* Clear return values */
	REQUIRES( isShortIntegerRangeNZ( certDataMaxLength ) ); 
...
int resetErrorInfo( INOUT_PTR DBMS_INFO *dbmsInfo )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 3 ) ) \
static int createDatabase( INOUT_PTR DBMS_INFO *dbmsInfo, 
						   IN_BOOL const BOOLEAN hasPermissions, 
	...
	REQUIRES( isBooleanValue( hasPermissions ) );
...
int setAccessMethodDBMS( INOUT_PTR KEYSET_INFO *keysetInfoPtr,
