...
extern int keyReadOK;		/* keyfile.c:testGetPGPPrivateKey() */
extern int doubleCertOK;	/* keyfile.c:testDoubleCertFile() */
...
static int readFileFromTemplate( const C_STR fileTemplate, const int count,
								 ...
								 const int bufSize )
	{
	BYTE fileName[ FILENAME_BUFFER_SIZE ];
		...
		switch( cryptEnvInfo )
			...
			case CRYPT_ENVINFO_PRIVATEKEY:
				...
				if( stringEnvInfo == NULL )
					...
					if( cryptStatusError( status ) )
						{
						printExtError( envelope, "cryptSetAttribute()", 
									   status, __LINE__ );
				...
				if( cryptStatusError( status ) )
					{
					printExtError( envelope, "cryptSetAttributeString()", 
								   status, __LINE__ );
...
static int pushData( const CRYPT_ENVELOPE envelope, const BYTE *buffer,
	...
	if( cryptStatusError( status ) )
		...
		/* A detached signature doesn't have any content so an inability to 
...
int testEnvelopeData( void )
	{
	if( !envelopeData( "env_data_ndef", FALSE, DATA_AMOUNT_SMALL, 0, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Indefinite-length */
	if( !envelopeData( "env_data", TRUE, DATA_AMOUNT_SMALL, 0, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Datasize */
	if( !envelopeData( "env_data.pgp", TRUE, DATA_AMOUNT_SMALL, 0, CRYPT_FORMAT_PGP ) )
		return( FALSE );	/* PGP format */
	return( envelopeData( "env_data_lge.pgp", TRUE, DATA_AMOUNT_MEDIUM, 0, CRYPT_FORMAT_PGP ) );
...
static int envelopeDataMultiple( const int blockSize, 
								 const BOOLEAN useDataSize,
	...
	if( useEncryption && \
		...
		return( FALSE );
	if( useDataSize && \
		!addEnvInfoNumeric( cryptEnvelope, CRYPT_ENVINFO_DATASIZE,
							( BUFFER_SIZE / blockSize ) * blockSize ) )
		return( FALSE );
	if( !createDeenvelope( &decryptEnvelope ) )
		return( FALSE );
	for( i = 0; i < BUFFER_SIZE / blockSize; i++ )
		{
		/* Envelope the data */
		status = cryptPushData( cryptEnvelope, inBufPtr, blockSize, &count );
		if( cryptStatusError( status ) )
			return( FALSE );
		inBufPtr += count;
...
int testEnvelopeDataMultiple( void )
	...
	for( i = 1; i < 8192; i++ )
		{
		/* We run the tests once with and once without an explicit length,
		   since this changes the blocking within the envelope */
		fprintf( outputStream, "%d.\r", i );
		if( !envelopeDataMultiple( i, FALSE, FALSE ) )
			return( FALSE );
		if( !envelopeDataMultiple( i, TRUE, FALSE ) )
			return( FALSE );
		}
...
int testEnvelopeCompress( void )
	...
	if( !envelopeCompress( "env_copr_ndef", FALSE, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Indefinite length */
	if( !envelopeCompress( "env_copr", TRUE, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Datasize */
	return( envelopeCompress( "env_copr.pgp", TRUE, CRYPT_FORMAT_PGP ) );
	}						/* PGP format */
...
int testEnvelopeSessionCrypt( void )
	{
	if( !envelopeSessionCrypt( "env_session_ndef", FALSE, FALSE, DATA_AMOUNT_SMALL, 0, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Indefinite length */
	if( !envelopeSessionCrypt( "env_session", TRUE, FALSE, DATA_AMOUNT_SMALL, 0, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Datasize */
	if( !envelopeSessionCrypt( "env_session_strmciph", TRUE, TRUE, DATA_AMOUNT_SMALL, 0, CRYPT_FORMAT_CRYPTLIB ) )
	...
	return( envelopeSessionCrypt( "env_session.pgp", TRUE, FALSE, DATA_AMOUNT_SMALL, 0, CRYPT_FORMAT_PGP ) );
#endif /* 0 */
	return( TRUE );
	}
...
int testEnvelopeSessionCryptLargeBuffer( void )
	{
	return( envelopeSessionCrypt( "env_session_lge", TRUE, FALSE, DATA_AMOUNT_LARGE, 0, CRYPT_FORMAT_CRYPTLIB ) );
...
int testEnvelopeSessionCryptMultiple( void )
	...
	for( i = 1; i < 8192; i++ )
		{
		fprintf( outputStream, "%d.\r", i );
		if( !envelopeDataMultiple( i, FALSE, TRUE ) )
			return( FALSE );
		}
...
int testEnvelopeCrypt( void )
	{
	if( !envelopeCrypt( "env_kek_ndef", FALSE, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Indefinite length */
...
int testEnvelopePasswordCrypt( void )
	{
	if( !envelopePasswordCrypt( "env_password_ndef", FALSE, FALSE, FALSE, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Indefinite length */
	if( !envelopePasswordCrypt( "env_password", TRUE, FALSE, FALSE, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Datasize */
	if( !envelopePasswordCrypt( "env_password_mult", TRUE, FALSE, TRUE, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Datasize, multiple keys */
	if( !envelopePasswordCrypt( "env_password.pgp", TRUE, FALSE, FALSE, CRYPT_FORMAT_PGP ) )
		return( FALSE );	/* PGP format */
	if( !envelopePasswordCrypt( "env_password_altciph.pgp", TRUE, TRUE, FALSE, CRYPT_FORMAT_PGP ) )
		return( FALSE );	/* PGP format, alt.cipher */
	return( envelopePasswordCrypt( "env_password_altciph", TRUE, TRUE, FALSE, CRYPT_FORMAT_CRYPTLIB ) );
	}						/* IV-less alt.cipher */
...
static int envelopeBoundaryTest( const BOOLEAN usePassword,
	...
	if( strategy == 0 )	
		{
		/* Different strategies produce the same output so we only dump the 
		   output once */
		debugDump( usePassword ? "env_ovfl_password" : "env_ovfl", 
				   encBuffer, count );
		}
...
static int envelopePKCCrypt( const char *dumpFileName,
	...
	{
	CRYPT_ENVELOPE cryptEnvelope;
	CRYPT_KEYSET cryptKeyset DUMMY_INIT;
	CRYPT_HANDLE cryptKey DUMMY_INIT;
...
int testEnvelopePKCCrypt( void )
	...
	if( cryptStatusError( cryptQueryCapability( CRYPT_ALGO_IDEA, NULL ) ) )
		...
		{
		if( !envelopePKCCrypt( "env_pkc_ndef", FALSE, KEYFILE_PGP, FALSE, FALSE, FALSE, FALSE, CRYPT_FORMAT_CRYPTLIB, CRYPT_UNUSED, CRYPT_UNUSED ) )
		...
		if( !envelopePKCCrypt( "env_pkc.pgp", TRUE, KEYFILE_PGP, FALSE, FALSE, FALSE, FALSE, CRYPT_FORMAT_PGP, CRYPT_UNUSED, CRYPT_UNUSED ) )
			return( FALSE );	/* PGP format */
		if( !envelopePKCCrypt( "env_pkc_recip.pgp", TRUE, KEYFILE_PGP, TRUE, FALSE, FALSE, FALSE, CRYPT_FORMAT_PGP, CRYPT_UNUSED, CRYPT_UNUSED ) )
			return( FALSE );	/* PGP format, recipient */
		if( !envelopePKCCrypt( "env_pkc_indkey.pgp", TRUE, KEYFILE_PGP, FALSE, FALSE, FALSE, TRUE, CRYPT_FORMAT_PGP, CRYPT_UNUSED, CRYPT_UNUSED ) )
			return( FALSE );	/* PGP format, decrypt key provided directly */
#ifdef USE_3DES
		if( !envelopePKCCrypt( "env_pkc_3des.pgp", TRUE, KEYFILE_PGP, TRUE, FALSE, TRUE, FALSE, CRYPT_FORMAT_PGP, CRYPT_UNUSED, CRYPT_UNUSED ) )
	...
	if( !envelopePKCCrypt( "env_crt_ndef", FALSE, KEYFILE_X509, FALSE, FALSE, FALSE, FALSE, CRYPT_FORMAT_CRYPTLIB, CRYPT_UNUSED, CRYPT_UNUSED ) )
...
#ifdef USE_OAEP
	status = cryptGetAttribute( CRYPT_UNUSED, CRYPT_OPTION_PKC_FORMAT, &value );
	if( cryptStatusOK( status ) )
		{
	...
	if( cryptStatusError( status ) )
		return( FALSE );
	if( !envelopePKCCrypt( "env_crt_oaep", TRUE, KEYFILE_X509, FALSE, FALSE, FALSE, FALSE, CRYPT_FORMAT_CRYPTLIB, CRYPT_UNUSED, CRYPT_UNUSED ) )
		return( FALSE );	/* Datasize hint */
	cryptSetAttribute( CRYPT_UNUSED, CRYPT_OPTION_PKC_FORMAT, value );
#endif /* USE_OAEP */
	if( !envelopePKCCrypt( "env_crt_indkey", TRUE, KEYFILE_X509, FALSE, FALSE, FALSE, TRUE, CRYPT_FORMAT_CRYPTLIB, CRYPT_UNUSED, CRYPT_UNUSED ) )
		return( FALSE );	/* Datasize, certificate, decrypt key provided directly */
	if( !envelopePKCCrypt( "env_crt_recip", TRUE, KEYFILE_X509, TRUE, FALSE, FALSE, FALSE, CRYPT_FORMAT_CRYPTLIB, CRYPT_UNUSED, CRYPT_UNUSED ) )
		return( FALSE );	/* Datasize, cerficate, recipient */
	return( envelopePKCCrypt( "env_crt_password", TRUE, KEYFILE_X509, FALSE, TRUE, FALSE, FALSE, CRYPT_FORMAT_CRYPTLIB, CRYPT_UNUSED, CRYPT_UNUSED ) );
...
#if 0
	if( !envelopePKCCrypt( "env_pkc_ex.pgp", TRUE, KEYFILE_NONE, FALSE, FALSE, FALSE, FALSE, CRYPT_FORMAT_PGP, cryptContext, decryptKeyset ) )
		return( FALSE );	/* PGP format */
#endif
	return( envelopePKCCrypt( "env_pkc_ex", TRUE, KEYFILE_NONE, FALSE, FALSE, FALSE, FALSE, CRYPT_FORMAT_CRYPTLIB, cryptContext, decryptKeyset ) );
	}						/* Datasize, raw key */
...
static int envelopePKCIterated( const CRYPT_FORMAT_TYPE formatType )
	...
	for( i = 0; i < NO_ITERS; i++ )
		...
		fprintf( origOutputStream, "%d.\r", i );
		memset( globalBuffer, 0, BUFFER_SIZE );
...
int testEnvelopePKCCryptAlgo( void )
	...
	if( cryptStatusOK( cryptQueryCapability( CRYPT_ALGO_ELGAMAL, NULL ) ) && \
		!envelopeAlgoCrypt( "env_pkc_elgamal", CRYPT_ALGO_ELGAMAL ) )
		return( FALSE );
	return( TRUE );
	}
...
static int envelopePKCCryptMulti( const char *dumpFileName,
	...
	{
	CRYPT_ENVELOPE cryptEnvelope;
	CRYPT_HANDLE cryptKey1, cryptKey2 DUMMY_INIT;
	int count, status;
...
int testEnvelopePKCCryptMulti( void )
	...
	if( cryptStatusError( cryptQueryCapability( CRYPT_ALGO_IDEA, NULL ) ) )
		...
		{
		if( !envelopePKCCryptMulti( "env_pkc_mult_key1.pgp", 1, CRYPT_FORMAT_PGP ) )
			return( FALSE );	/* PGP format, multiple recip, key #1 decrypts */
		if( !envelopePKCCryptMulti( "env_pkc_mult_key2.pgp", 2, CRYPT_FORMAT_PGP ) )
			return( FALSE );	/* PGP format, multiple recip, key #2 decrypts */
		}
	if( !envelopePKCCryptMulti( "env_pkc_mult_key1", 1, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Multiple recipients, key #1 decrypts */
	return( envelopePKCCryptMulti( "env_pkc_mult_key2", 2, CRYPT_FORMAT_CRYPTLIB ) );
...
static int envelopeSign( const void *data, const int dataLength,
	...
	if( keyFileType == KEYFILE_OPENPGP_HASH || keyFileType == KEYFILE_PGP )
		...
		{
		filenameFromTemplate( filenameBuffer, USER_PRIVKEY_FILE_TEMPLATE, 
							  ( keyFileType == KEYFILE_X509 ) ? 1 : 2 );
...
int testEnvelopeSign( void )
	...
	if( cryptStatusError( cryptQueryCapability( CRYPT_ALGO_IDEA, NULL ) ) )
		...
		{
		if( !envelopeSign( ENVELOPE_TESTDATA, ENVELOPE_TESTDATA_SIZE, "env_sig_ndef", KEYFILE_PGP, FALSE, FALSE, FALSE, CRYPT_CONTENT_NONE, CRYPT_FORMAT_CRYPTLIB ) )
			return( FALSE );	/* Indefinite length, raw key */
		if( !envelopeSign( ENVELOPE_TESTDATA, ENVELOPE_TESTDATA_SIZE, "env_sig", KEYFILE_PGP, TRUE, FALSE, FALSE, CRYPT_CONTENT_NONE, CRYPT_FORMAT_CRYPTLIB ) )
			return( FALSE );	/* Datasize, raw key */
		if( !envelopeSign( ENVELOPE_TESTDATA, ENVELOPE_TESTDATA_SIZE, "env_sig_content", KEYFILE_PGP, TRUE, FALSE, FALSE, CRYPT_CONTENT_COMPRESSEDDATA, CRYPT_FORMAT_CRYPTLIB ) )
	...
	if( !envelopeSign( ENVELOPE_TESTDATA, ENVELOPE_TESTDATA_SIZE, "env_csg", KEYFILE_X509, TRUE, FALSE, FALSE, CRYPT_CONTENT_NONE, CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );	/* Datasize, certificate */
	if( !envelopeSign( ENVELOPE_TESTDATA, ENVELOPE_TESTDATA_SIZE, "env_csg_smime", KEYFILE_X509, TRUE, FALSE, FALSE, CRYPT_CONTENT_NONE, CRYPT_FORMAT_SMIME ) )
		return( FALSE );	/* Datasize, certificate, S/MIME semantics */
	if( !envelopeSign( ENVELOPE_TESTDATA, ENVELOPE_TESTDATA_SIZE, "env_csg_key", KEYFILE_X509, TRUE, FALSE, TRUE, CRYPT_CONTENT_NONE, CRYPT_FORMAT_CRYPTLIB ) )
...
static int envelopeSignIterated( const CRYPT_FORMAT_TYPE formatType )
	{
	CRYPT_CONTEXT cryptPubKey, cryptPrivKey;
	FILE *origOutputStream = outputStream;
	int i, status;
	...
	for( i = 0; i < NO_ITERS; i++ )
		...
		status = createEnvelope( &cryptEnvelope, formatType );
		assert( status == TRUE );
		status = addEnvInfoNumeric( cryptEnvelope, CRYPT_ENVINFO_SIGNATURE, 
									cryptPrivKey );
		assert( status == TRUE );
		cryptSetAttribute( cryptEnvelope, CRYPT_ENVINFO_DATASIZE,
						   ENVELOPE_TESTDATA_SIZE );
		...
		/* De-envelope the data */
		count = envelopeSigCheck( globalBuffer, count, CRYPT_UNUSED, 
								  cryptPubKey, KEYFILE_NONE, FALSE, 
								  formatType );
		if( count <= 0 )
			{
			fclose( outputStream );
	...
	fprintf( outputStream, "Tested %d iterations of %s signed enveloping.\n\n", 
			 NO_ITERS, 
			 ( formatType == CRYPT_FORMAT_CRYPTLIB ) ? "cryptlib" : "PGP" );
	return( TRUE );
	}
...
int testEnvelopeSignIterated( void )
	...
	if( !envelopeSignIterated( CRYPT_FORMAT_CRYPTLIB ) )
		return( FALSE );
#ifdef USE_PSS
	...
	cryptSetAttribute( CRYPT_UNUSED, CRYPT_OPTION_PKC_FORMAT, value );
#endif /* USE_PSS */
	return( envelopeSignIterated( CRYPT_FORMAT_PGP ) );
	}
...
static int envelopeAlgoSign( const char *dumpFileName,
	...
	switch( cryptAlgo )
		...
		case CRYPT_ALGO_DSA:
			status = loadDSAContexts( CRYPT_UNUSED, &sigCheckKey, &sigKey );
...
int testEnvelopeSignHashUpgrade( void )
	{
	return( envelopeSign( ENVELOPE_TESTDATA, ENVELOPE_TESTDATA_SIZE, "env_csg_hashupgrd", KEYFILE_X509_ALT, TRUE, FALSE, FALSE, CRYPT_CONTENT_NONE, CRYPT_FORMAT_CRYPTLIB ) );
	}
...
int testEnvelopeSignOverflow( void )
	...
	if( !envelopeSignOverflow( buffer, 8192 - 280, "env_sig_ovfl1.pgp", CRYPT_FORMAT_PGP, "signature" ) )
		return( FALSE );	/* PGP format, raw key */
	if( !envelopeSignOverflow( buffer, 8384 - 6, "env_sig_ovfl2.pgp", CRYPT_FORMAT_PGP, "seg.boundary" ) )
		return( FALSE );	/* PGP format, raw key */
	if( !envelopeSignOverflow( buffer, 8192 - 1152, "env_csg_ovfl1", CRYPT_FORMAT_SMIME, "signing certs" ) )
		return( FALSE );	/* Datasize, certificate, S/MIME semantics */
	return( envelopeSignOverflow( buffer, 8192 - 1280, "env_csg_ovfl2", CRYPT_FORMAT_SMIME, "signature" ) );
...
static int envelopeAuthent( const void *data, const int dataLength,
	...
	if( usePKC )
		...
		filenameFromTemplate( filenameBuffer, USER_PRIVKEY_FILE_TEMPLATE, 1 );
		status = getPublicKey( &cryptKey, filenameBuffer, 
							   USER_PRIVKEY_LABEL );
		if( cryptStatusError( status ) )
			return( FALSE );
		if( !addEnvInfoNumeric( cryptEnvelope, CRYPT_ENVINFO_PUBLICKEY,
	...
	debugDump( ( formatType == CRYPT_FORMAT_PGP ) ? "env_mdc.pgp" : \
			   useAuthEnc ? ( usePKC ? "env_authenc_pkc" : "env_authenc" ) : \
			   useDatasize ? "env_mac" : \
			   usePKC ? "env_mac_pkc" : "env_mac_ndef", globalBuffer, count );
...
static int cmsEnvelopeSigCheck( const void *signedData, 
					const int signedDataLength, const void *detachedData, 
					const int detachedDataLength, 
					const CRYPT_CONTEXT sigCheckContext, 
					const CRYPT_CONTEXT hashContext, 
	...
	if( hashContext != CRYPT_UNUSED )
		...
		/* Try and add it a second time and make sure that the duplicate add
		   is detected */
		status = cryptSetAttribute( cryptEnvelope, CRYPT_ENVINFO_HASH,
									hashContext );
		if( status != CRYPT_ERROR_INITED )
			{
			fprintf( outputStream, "Duplicate add of externally-generated "
					 "hash to envelope wasn't detected, status %d, "
					 "line %d.\n", status, __LINE__ );
			return( FALSE );
			}
		}
	...
	if( count == CRYPT_ERROR_NOTAVAIL && \
		!( detachedData != NULL || hashContext != CRYPT_UNUSED || \
		   hasTimestamp || checkData ) )
		{
	...
	if( !cryptStatusError( count ) && hashContext == CRYPT_UNUSED )
		{
		/* There's detached data associated with the signature and it hasn't
		   already been processed via an externally-supplied hash context, 
		   hash it explicitly */
		if( detachedData != NULL )
			{
			count = pushData( cryptEnvelope, detachedData,
							  detachedDataLength, NULL, 0 );
			}
		else
			{
			count = popData( cryptEnvelope, globalBuffer, BUFFER_SIZE );
	...
	if( testRefCount )
		{
		CRYPT_CERTIFICATE cryptCert1, cryptCert2 DUMMY_INIT, cryptCert3 DUMMY_INIT;
		int value;
	...
	/* Make sure that the result matches what we pushed */
	if( detachedData == NULL && hashContext == CRYPT_UNUSED && checkData )
		{
		if( !compareData( ENVELOPE_TESTDATA, ENVELOPE_TESTDATA_SIZE, 
						  globalBuffer, count ) )
			return( FALSE );
...
static int cmsEnvelopeSign( const BOOLEAN useDatasize,
	...
	{
	CRYPT_ENVELOPE cryptEnvelope;
	CRYPT_CONTEXT cryptContext, cryptContext2 DUMMY_INIT, hashContext = CRYPT_UNUSED;
	...
	if( cryptStatusOK( status ) && useTimestamp )
		...
		status = cryptSetAttributeString( cryptSession,
										  ...
										  TIMESTAP_SERVER_NAME,
										  paramStrlen( TIMESTAP_SERVER_NAME ) );
		if( cryptStatusError( status ) )
			{
	...
	debugDump( detachedSig ?
			   ...
			   useTimestamp ? \
				 ( useDatasize ? "smi_sig_tsp" : "smi_sig_tsp_ndef" ) : \
			   useNonDataContent ? \
				 ( useDatasize ? "smi_sig_nondata" : "smi_sig_nondata_ndef" ) : \
			   dualSig ? \
				 ( useDatasize ? "smi_sig_dual" : "smi_sig_dual_ndef" ) : \
			   useDatasize ? "smi_sig" : "smi_sig_ndef", globalBuffer, count );
	...
	status = cmsEnvelopeSigCheck( globalBuffer, count,
					detachedSig ? ENVELOPE_TESTDATA : NULL, 
					detachedSig ? ENVELOPE_TESTDATA_SIZE : 0, 
					isPGP || isRawCMS ? cryptContext : CRYPT_UNUSED, 
					hashContext, useTimestamp, TRUE, useAttributes, 
					useNonDataContent ? TRUE : FALSE, testRefCount );
	if( externalHashLevel > 0 )
		cryptDestroyContext( hashContext );
	if( isPGP || isRawCMS )
		cryptDestroyContext( cryptContext );
...
static int cmsImportSignedData( const char *fileName, const int fileNo )
	...
	/* Check the signature on the data.  File 4 is file 1 modified to have 
	   an unrecognised SignerInfo version in the first of two signatures so
	   it's checked for alongside file 1 */
	status = cmsEnvelopeSigCheck( bufPtr, count, NULL, 0, CRYPT_UNUSED, 
								  CRYPT_UNUSED, 
								  ( fileNo == 6 ) ? TRUE : FALSE, FALSE, 
								  ( fileNo == 1 || fileNo == 4 ) ? \
...
int testCMSEnvelopePKCCrypt( void )
	...
	if( !cmsEnvelopeCrypt( "smi_pkc_ndef", FALSE, FALSE, FALSE, FALSE, CRYPT_UNUSED, CRYPT_UNUSED, NULL, NULL ) )
	...
	if( cryptStatusError( status ) )
		return( FALSE );
	if( !cmsEnvelopeCrypt( "smi_pkc_oaep", TRUE, FALSE, FALSE, FALSE, CRYPT_UNUSED, CRYPT_UNUSED, NULL, NULL ) )
		return( FALSE );	/* Datasize hint */
	cryptSetAttribute( CRYPT_UNUSED, CRYPT_OPTION_PKC_FORMAT, value );
#endif /* USE_OAEP */
	...
	status = cmsEnvelopeCrypt( "smi_pkc_strm", TRUE, TRUE, FALSE, FALSE, CRYPT_UNUSED, CRYPT_UNUSED, NULL, NULL );
	cryptSetAttribute( CRYPT_UNUSED, CRYPT_OPTION_ENCR_ALGO, value );
	if( !status )			/* Datasize and stream cipher */
		return( status );

	/* Test enveloping with a cipher with an alternative cipher with a 
	   different block size than the default AES */
	status = cryptGetAttribute( CRYPT_UNUSED, CRYPT_OPTION_ENCR_ALGO, &value );
	if( cryptStatusError( status ) )
		return( FALSE );
	cryptSetAttribute( CRYPT_UNUSED, CRYPT_OPTION_ENCR_ALGO, CRYPT_ALGO_3DES );
	status = cmsEnvelopeCrypt( "smi_pkc_3des", TRUE, FALSE, TRUE, FALSE, CRYPT_UNUSED, CRYPT_UNUSED, NULL, NULL );
	cryptSetAttribute( CRYPT_UNUSED, CRYPT_OPTION_ENCR_ALGO, value );
	return( status );		/* Datasize and alternative blocksize cipher */
	}
...
int testCMSEnvelopePKCCryptEx( const CRYPT_HANDLE encryptContext,
	...
	status = cmsEnvelopeCrypt( "smi_pkc_device", TRUE, FALSE, FALSE, \
...
int testCMSEnvelopePKCCryptDoubleCert( void )
	...
	status = cmsEnvelopeCrypt( "smi_pkc_dual", TRUE, FALSE, FALSE, FALSE, 
...
int testCMSEnvelopeSignedDataImport( void )
	...
	/* There are many encoding variations possible for signed data so we try
	   ...
	   cases.  The files are:
		...
		File 4: File with v4+v1 signature, created from File 1.
		
		File 5: File with v4+v4 signature, created from File 1.
		
		File 6: File with timestamp, created by testSessionEnvTSP() */
	for( i = 1; i <= 6; i++ )
		{
		int count;

		filenameFromTemplate( fileName, SMIME_SIG_FILE_TEMPLATE, i );
		count = cmsImportSignedData( fileName, i );
		if( i == 5 )
			{
			if( count )
				return( FALSE );
...
int testEnvelopePasswordCryptImport( void )
	...
	/* Process the password-protected data.  The files are:
		...
		File 3: CMS Enveloped data with v1 PWRI created from File 2 */
	for( i = 1; i <= 3; i++ )
		{
		int count;
		...
		if( count <= 0 )
			return( FALSE );
		count = cmsEnvelopeDecrypt( globalBuffer, count, CRYPT_UNUSED, 
									TEST_PASSWORD, FALSE );
		if( i == 3 )
			{
			if( count > 0 )
...
int testPGPEnvelopePasswordCryptImport( void )
	...
	/* Process the OpenPGP data, all with password "password".  The files are:
		...
		File 8+9: AES-256, with the maximum amount of S2K iterations 
				  possible in the case of File 9, which seems to be the 
				  default with some GPG 2.2.x versions.  Create with:

			gpg -c --s2k-count 65536 test.txt
			gpg -c test.txt 
			
		File 10: SKE packet with version of 5 created from File 9, leading 
				 to an unprocessable message */
#ifdef USE_3DES
	...
	count = readFileFromTemplate( PGP_ENC_FILE_TEMPLATE, 3, 
								  "OpenPGP password-encrypted data "
								  "(3DES+non-iterated hash)",
								  globalBuffer, BUFFER_SIZE );
	if( count <= 0 )
		return( FALSE );
	if( !envelopePasswordDecrypt( globalBuffer, count ) )
	...
	if( !compareData( ENVELOPE_COMPRESSEDDATA, ENVELOPE_TESTDATA_SIZE, 
		...
		return( FALSE );
#endif /* USE_3DES */
	count = readFileFromTemplate( PGP_ENC_FILE_TEMPLATE, 8, 
								  "GPG AES-256 password-encrypted data",
								  globalBuffer, BUFFER_SIZE );
	if( count <= 0 )
		return( FALSE );
	count = envelopePasswordDecrypt( globalBuffer, count );
	...
	if( !compareData( ENVELOPE_PGP_TESTDATA, ENVELOPE_TESTDATA_SIZE, 
					  globalBuffer, ENVELOPE_TESTDATA_SIZE ) )
		return( FALSE );
	count = readFileFromTemplate( PGP_ENC_FILE_TEMPLATE, 9, 
								  "GPG password-encrypted data with "
								  "ludicrous-speed iterations",
								  globalBuffer, BUFFER_SIZE );
	if( count <= 0 )
		return( FALSE );
	count = envelopePasswordDecrypt( globalBuffer, count );
	...
	if( !compareData( ENVELOPE_PGP_TESTDATA, ENVELOPE_TESTDATA_SIZE, 
					  globalBuffer, ENVELOPE_TESTDATA_SIZE ) )
		return( FALSE );
	count = readFileFromTemplate( PGP_ENC_FILE_TEMPLATE, 10, 
								  "OpenPGP password-encrypted data with "
								  "unknown SKE version",
								  globalBuffer, BUFFER_SIZE );
	if( count <= 0 )
		return( FALSE );
...
static int createIncompleteHash( CRYPT_CONTEXT *hashContext,
	...
	if( cryptStatusOK( status ) && blockSize != 0 )
		{
		status = cryptSetAttribute( *hashContext, CRYPT_CTXINFO_BLOCKSIZE, 
									blockSize );
		}
	if( cryptStatusOK( status ) )
		status = cryptEncrypt( *hashContext, ( void * ) data, dataLength );
	if( cryptStatusError( status ) )
		{
		fputs( "Couldn't create external hash of data.\n\n", outputStream );
		return( FALSE );
...
int testPGPEnvelopeSignedDataImport( void )
	...
	if( !createIncompleteHash( &hashContext, CRYPT_ALGO_SHA1, 
							   ENVELOPE_PGP_TESTDATA, 
	...
	/* Process alternative OpenPGP detached signed data, created by 
	   Sequioa-PGP for testing interoperability with unknown signature 
	   ...
	   signatures */
	count = readFileFromTemplate( PGP_SIG_FILE_TEMPLATE, 5, 
								  "OpenPGP detached v4+v4 signature", 
								  globalBuffer, BUFFER_SIZE );
	if( count <= 0 )
		return( FALSE );
	if( !createIncompleteHash( &hashContext, CRYPT_ALGO_SHA2, 
							   "Hello World :)", 14, 64 ) )
		return( FALSE );
	count = envelopeSigCheck( globalBuffer, count, hashContext,
							  CRYPT_UNUSED, KEYFILE_OPENPGP_HASH_ALT, TRUE,
							  CRYPT_FORMAT_PGP );
	cryptDestroyContext( hashContext );
	if( count <= 0 )
		return( FALSE );
	...
	if( count <= 0 )
		return( FALSE );
	cryptDestroyContext( hashContext );
	fputs( "Import of OpenPGP detached test signatures succeeded.\n\n", 
		   outputStream );

	/* Process OpenPGP signed data with unprocessable signature packets.  
	   The files are:

		Signed-8: Version 5 signature on data created from Signed-3.
		Signed-9: Version 5 standalone signature created from Signed-4 */
	count = readFileFromTemplate( PGP_SIG_FILE_TEMPLATE, 8, 
								  "OpenPGP v5 signature", 
								  globalBuffer, BUFFER_SIZE );
	if( count <= 0 )
		return( FALSE );
	count = envelopeSigCheck( globalBuffer, count, CRYPT_UNUSED,
...
int testPGPEnvelopeCompressedDataImport( void )
	...
	/* Process booby-traped compressed data import.  This is a PGP quine 
	   ...
	   time, but we test it anyway just to show that it's not affected.
...
static BOOLEAN getFileData( const char *fileName, BYTE **bufPtrPtr, 
							int *bufSize )
	{
	BYTE *bufPtr = globalBuffer;
	int count;
...
void xxxDataImport( const char *fileName )
	...
	if( !getFileData( fileName, &bufPtr, &count ) )
		return;
...
static void signedDataImport( const BYTE *sigData, const int sigLength, 
							  const BYTE *detachedData, 
							  const int detachedLength )
	{
	int status;

	status = cmsEnvelopeSigCheck( sigData, sigLength, detachedData, 
								  detachedLength, CRYPT_UNUSED, CRYPT_UNUSED, 
...
void xxxDetachedSignedDataImport( const char *dataFileName, 
	...
	/* If it's a detached signature, get the extra data */
	if( dataFileName != NULL )
		{
		detachedBufPtr = detachedBuffer;
		detachedCount = getFileSize( dataFileName ) + 10;
		assert( detachedCount <= 2048 );
		detachedCount = readFileData( dataFileName, "S/MIME test data", 
									  detachedBufPtr, detachedCount, 64,
									  FALSE );
		assert( detachedCount > 64 );
		}

	if( !getFileData( sigFileName, &sigBufPtr, &sigCount ) )
		return;
...
void xxxSignedDataImport( const char *fileName )
	{
	xxxDetachedSignedDataImport( NULL, fileName );
	}
...
void xxxSignedEncryptedDataImport( const char *fileName, const char *keyset,
	...
	if( !getFileData( fileName, &bufPtr, &count ) )
		return;
	signedDataImport( bufPtr, count, NULL, 0 );
	encryptedDataImport( bufPtr, count, keyset, password );
	if( bufPtr != globalBuffer )
		free( bufPtr );
	}
