...
static int pbkdf2Hash( OUT_BUFFER_FIXED( outLength ) BYTE *out, 
	...
	LOOP_MAX( i = 0, i < iterations - 1, i++ )
		...
		ENSURES( LOOP_INVARIANT_MAX( i, 0, iterations - 2 ) );
		...
		LOOP_EXT_ALT( j = 0, j < outLength, j++, CRYPT_MAX_HASHSIZE + 1 )
			{
			ENSURES( LOOP_INVARIANT_EXT_ALT( j, 0, outLength - 1,
											 CRYPT_MAX_HASHSIZE + 1 ) );
...
static int deriveHKDF( STDC_UNUSED void *dummy, 
	...
	/* Convert the HMAC algorithm into the equivalent hash algorithm for use 
	   with the PRF.  Although the user has specified the algorithm in terms 
	   of an HMAC we're synthesising it from the underlying hash algorithm 
	   since this allows us to perform the PRF setup once and reuse the 
	   initial value for any future hashing */
	status = mapValue( mechanismInfo->hashAlgo, &value, mapTbl, 
					   FAILSAFE_ARRAYSIZE( mapTbl, MAP_TABLE ) );
	if( cryptStatusError( status ) )
		return( status );
	...
	/* Extract the master secret value from the generic-secret context and 
	   convert it into the KDF key:

		KDF-Key = HMAC( key )
		
	   Note that this is not a typo: The key is HMAC'd using the salt as the 
	   HMAC key, not the other way round */
	getHashAtomicParameters( hashAlgo, mechanismInfo->hashParam, 
							 &hashFunctionAtomic, &hashSize );
	getHashParameters( hashAlgo, mechanismInfo->hashParam, &hashFunction, 
					   NULL, &hashBlockSize );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 2 ) ) \
int kdfHKDF( STDC_UNUSED void *dummy, 
			 INOUT_PTR MECHANISM_KDF_INFO *mechanismInfo )
	{
	MECHANISM_DERIVE_INFO mechanismDeriveInfo;
	MESSAGE_DATA msgData;
...
static int add64( INOUT_BUFFER_FIXED( destLen ) BYTE *dest,
	...
	LOOP_EXT_REV( ( destIndex = P12_BLOCKSIZE - 1, 
					srcIndex = P12_BLOCKSIZE - 1 ),
				  destIndex >= 0, ( destIndex--, srcIndex-- ), 
				  P12_BLOCKSIZE + 1 )
		{
		int value;

		ENSURES( LOOP_INVARIANT_REV( srcIndex, 0, P12_BLOCKSIZE - 1 ) );
		ENSURES( LOOP_INVARIANT_SECONDARY( destIndex, 0, 
										   P12_BLOCKSIZE - 1 ) );
		
		value = dest[ destIndex ] + src[ srcIndex ] + carry;
		dest[ destIndex ] = intToByte( value );
		carry = value >> 8;
		}
	ENSURES( LOOP_BOUND_EXT_REV_OK( P12_BLOCKSIZE + 1 ) );
...
static int initDSP( OUT_BUFFER( dspMaxLen, *dspLen ) BYTE *dsp,
	...
	BYTE *dspPtr = dsp;
	LOOP_INDEX i, keyIndex, bmpIndex;
	int status;
	...
	/* Clear return values */
	REQUIRES( isShortIntegerRangeNZ( dspMaxLen ) ); 
	...
	*dspLen = 0;
	...
	LOOP_EXT( i = 0, i < P12_BLOCKSIZE, i++, P12_BLOCKSIZE + 1 )
		{
		ENSURES( LOOP_INVARIANT_EXT( i, 0, P12_BLOCKSIZE - 1,
									 P12_BLOCKSIZE + 1 ) );
	...
	LOOP_EXT( ( keyIndex = 0, bmpIndex = 0 ), 
			  keyIndex < keyLength, 
			  ( keyIndex++, bmpIndex += 2 ), CRYPT_MAX_TEXTSIZE + 1 )
		{
		ENSURES( LOOP_INVARIANT_EXT( keyIndex, 0, keyLength - 1,
									 CRYPT_MAX_TEXTSIZE + 1 ) );
		ENSURES( LOOP_INVARIANT_SECONDARY( bmpIndex, 0, 
										   ( keyLength * 2 ) - 2 ) );
...
int derivePKCS12( STDC_UNUSED void *dummy, 
	...
	LOOP_MED( keyIndex = 0, 
		...
		LOOP_MAX_ALT( i = 1, i < mechanismInfo->iterations, i++ )
			{
			ENSURES( LOOP_INVARIANT_MAX_ALT( i, 1, 
											 mechanismInfo->iterations - 1 ) );
		...
		if( cryptStatusError( status ) )
			break;
		LOOP_LARGE_ALT( dspIndex = P12_BLOCKSIZE, 
						dspIndex < dspLen, 
						dspIndex += P12_BLOCKSIZE )
			{
			ENSURES( LOOP_INVARIANT_LARGE_XXX_ALT( dspIndex, P12_BLOCKSIZE, 
												   dspLen - 1 ) );
...
static int tlsPrfInit( INOUT_PTR TLS_PRF_INFO *prfInfo, 
					   IN_BUFFER( keyLength ) const void *key, 
					   IN_LENGTH_SHORT const int keyLength,
					   IN_BUFFER( saltLength ) const void *salt, 
...
static int tlsPrfHash( INOUT_BUFFER_FIXED( outLength ) BYTE *out, 
					   ...
					   INOUT_PTR TLS_PRF_INFO *prfInfo, 
					   IN_BUFFER( saltLength ) const void *salt, 
	...
	LOOP_EXT( i = 0, i < outLength, i++, CRYPT_MAX_HASHSIZE + 1 )
		{
		ENSURES( LOOP_INVARIANT_EXT( i, 0, outLength - 1,
									 CRYPT_MAX_HASHSIZE + 1 ) );
...
/* Perform TLS 1.0 - 1.1 key derivation.  This implements the function 
   described as 'PRF()' in the TLS spec */
...
int deriveTLS( STDC_UNUSED void *dummy, 
	...
	LOOP_MED_INITCHECK( md5Index = shaIndex = 0, md5Index < dataOutLength )
		...
		ENSURES( LOOP_INVARIANT_MED_XXX( md5Index, 0, dataOutLength - 1 ) );
				 /* md5Index is incremented by the number of output bytes */
		ENSURES( LOOP_INVARIANT_SECONDARY( shaIndex, 0, dataOutLength ) );
...
static int pgpPrfHash( OUT_BUFFER_FIXED( outLength ) BYTE *out, 
	...
	REQUIRES( hashSize >= MIN_HASHSIZE && hashSize <= CRYPT_MAX_HASHSIZE );
	REQUIRES( isShortIntegerRangeMin( keyLength, 2 ) );
...
int derivePGP( STDC_UNUSED void *dummy, 
	...
	LOOP_EXT( i = 0, byteCount > 0 && cryptStatusOK( status ), i++, 
		...
		{
		/* There's not really a proper LOOP_INVARIANT possible for this 
		   case, i is really just a boolean value used to indicate the first 
		   iteration and byteCount is supplied by pgpPrfHash() and again is 
	...
	if( cryptStatusOK( status ) && secondByteCount > 0 )
		...
		LOOP_EXT( i = 0, secondByteCount > 0 && cryptStatusOK( status ), i++,
				  GPG_FAILSAFE_ITERATIONS_MAX + 1 )
			{
			/* As before there's not really a proper LOOP_INVARIANT possible 
			   for this case */
			ENSURES( LOOP_INVARIANT_EXT_GENERIC( GPG_FAILSAFE_ITERATIONS_MAX + 1 ) );
...
int deriveCMP( STDC_UNUSED void *dummy, 
	...
	LOOP_MAX( iterations = 1, 
			  iterations < mechanismInfo->iterations, 
			  iterations++ )
		{
		ENSURES( LOOP_INVARIANT_MAX( iterations, 1, 
									 mechanismInfo->iterations - 1 ) );
...
/* Perform HOTP_TOTP key derivation */

CHECK_RETVAL STDC_NONNULL_ARG( ( 2 ) ) \
int deriveHOTP( STDC_UNUSED void *dummy, 
				INOUT_PTR MECHANISM_DERIVE_INFO *mechanismInfo )
	{
	HASH_FUNCTION_ATOMIC hashFunctionAtomic;
	HASH_FUNCTION hashFunction;
	...
	/* HMAC the counter, present in the mechanism info as the salt, using the 
	   key */
...
#define MECHANISM_OUTPUT_SIZE_TLS	48
#define MECHANISM_INPUT_SIZE_TLS	48
#define MECHANISM_SALT_SIZE_TLS		64
...
static const MECHANISM_TEST_INFO deriveMechanismTestInfo[] = {
	...
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HOTP, ( FUNCTION_CAST ) deriveHOTP },
	  { MKDATA( "359152" ), 6,
		MKDATA( "12345678901234567890" ), 20, CRYPT_ALGO_SHA1, 0, 
		"\x00\x00\x00\x00\x00\x00\x00\x02", 8, 1 } },
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HOTP, ( FUNCTION_CAST ) deriveHOTP },
	  { MKDATA( "969429" ), 6,
		MKDATA( "12345678901234567890" ), 20, CRYPT_ALGO_SHA1, 0, 
		"\x00\x00\x00\x00\x00\x00\x00\x03", 8, 1 } },
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HOTP, ( FUNCTION_CAST ) deriveHOTP },
	  { MKDATA( "338314" ), 6,
		MKDATA( "12345678901234567890" ), 20, CRYPT_ALGO_SHA1, 0, 
		"\x00\x00\x00\x00\x00\x00\x00\x04", 8, 1 } },
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HOTP, ( FUNCTION_CAST ) deriveHOTP },
	  { MKDATA( "254676" ), 6,
		MKDATA( "12345678901234567890" ), 20, CRYPT_ALGO_SHA1, 0, 
		"\x00\x00\x00\x00\x00\x00\x00\x05", 8, 1 } },
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HOTP, ( FUNCTION_CAST ) deriveHOTP },
	  { MKDATA( "287922" ), 6,
		MKDATA( "12345678901234567890" ), 20, CRYPT_ALGO_SHA1, 0, 
		"\x00\x00\x00\x00\x00\x00\x00\x06", 8, 1 } },
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HOTP, ( FUNCTION_CAST ) deriveHOTP },
	  { MKDATA( "162583" ), 6,
		MKDATA( "12345678901234567890" ), 20, CRYPT_ALGO_SHA1, 0, 
		"\x00\x00\x00\x00\x00\x00\x00\x07", 8, 1 } },
  #endif /* 0 */
  #if 0		/* TOTP tests that just changed the counter value to a time-based value */
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HOTP, ( FUNCTION_CAST ) deriveHOTP },
	  { MKDATA( "081804" ), 6,
		MKDATA( "12345678901234567890" ), 20, CRYPT_ALGO_SHA1, 0, 
		"\x00\x00\x00\x00\x02\x35\x23\xEC", 8, 1 } },
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HOTP, ( FUNCTION_CAST ) deriveHOTP },
	  { MKDATA( "050471" ), 6,
		MKDATA( "12345678901234567890" ), 20, CRYPT_ALGO_SHA1, 0, 
		"\x00\x00\x00\x00\x02\x35\x23\xED", 8, 1 } },
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HOTP, ( FUNCTION_CAST ) deriveHOTP },
	  { MKDATA( "005924" ), 6,
		MKDATA( "12345678901234567890" ), 20, CRYPT_ALGO_SHA1, 0, 
		"\x00\x00\x00\x00\x02\x73\xEF\x07", 8, 1 } },
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HOTP, ( FUNCTION_CAST ) deriveHOTP },
	  { MKDATA( "279037" ), 6,
		MKDATA( "12345678901234567890" ), 20, CRYPT_ALGO_SHA1, 0, 
		"\x00\x00\x00\x00\x03\xF9\x40\xAA", 8, 1 } },
  #endif /* 0 */
#endif /* USE_TLS || USE_SSH */
#ifdef USE_TLS
...
int deriveSelftest( STDC_UNUSED void *dummy, 
	...
	LOOP_LARGE( i = 0, 
		...
		ENSURES( LOOP_INVARIANT_LARGE( i, 0, 
									   FAILSAFE_ARRAYSIZE( deriveMechanismTestInfo, \
														   MECHANISM_TEST_INFO ) - 1 ) );
...
static const MECHANISM_TEST_INFO kdfMechanismTestInfo[] = {
	...
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HKDF, ( FUNCTION_CAST ) testDeriveHKDF },
	  { MKDATA( "\x3C\xB2\x5F\x25\xFA\xAC\xD5\x7A\x90\x43\x4F\x64\xD0\x36\x2F\x2A"
		...
		MKDATA( "\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B"
				"\x0B\x0B\x0B\x0B\x0B\x0B" ), 22,
		CRYPT_ALGO_HMAC_SHA2, 32,
		MKDATA( "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C" ), 13, 1 } },
	...
	{ { MESSAGE_DEV_DERIVE, MECHANISM_DERIVE_HKDF, ( FUNCTION_CAST ) testDeriveHKDF },
	  { MKDATA( "\x08\x5A\x01\xEA\x1B\x10\xF3\x69\x33\x06\x8B\x56\xEF\xA5\xAD\x81"
		...
		MKDATA( "\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B\x0B" ), 11,
		CRYPT_ALGO_HMAC_SHA1, 0,
		MKDATA( "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C" ), 13, 1 } },
...
int kdfSelftest( STDC_UNUSED void *dummy, 
	...
	LOOP_LARGE( i = 0, 
		...
		ENSURES( LOOP_INVARIANT_LARGE( i, 0, 
									   FAILSAFE_ARRAYSIZE( kdfMechanismTestInfo, \
														   MECHANISM_TEST_INFO ) - 1 ) );
