...
BOOLEAN sanityCheckSessionWrite( const SESSION_INFO *sessionInfoPtr )
	...
	if( sessionInfoPtr->type != CRYPT_SESSION_SSH && \
		...
		sessionInfoPtr->type != CRYPT_SESSION_TLS_SERVER )
		{
	...
	if( !isBufsizeRangeMin( sessionInfoPtr->sendBufSize, MIN_BUFFER_SIZE ) )
		{
...
static int flushData( INOUT_PTR SESSION_INFO *sessionInfoPtr )
	...
	if( cryptStatusError( status ) && status != CRYPT_ERROR_TIMEOUT )
		...
		/* Now things get a bit complicated, the write was interrupted so 
		   we've probably got incompletely-finalised buffer states.  This 
		   means that an attempt to perform a clean shutdown, for example
		   by sending an SSH/TLS close notification, will run into problems
		   with the session's internal data-handling state not being valid.  
		   
		   Trying to flush out what's still there in order to send the close 
		   will fail since we've already run into a fatal networking error, 
		   and resetting the state to allow the close-notification send will 
		   also fail at the receiving end, assuming we could send it, 
		   because half of the previous packet will be missing when the 
		   close arrives.

		   This is another one of those can't-occur conditions that it's 
		   difficult to plan for, but in order to avoid internal sanity-
		   check failures we reset the buffer state values to clear things
		   for the assembly of a close notification that'll never be sent */
		sessionInfoPtr->sendBufPos = sessionInfoPtr->sendBufStartOfs;
	...
	/* We've sent everything, reset the buffer state values */
	sessionInfoPtr->sendBufPos = sessionInfoPtr->sendBufStartOfs;
	sessionInfoPtr->partialWrite = FALSE;
	sessionInfoPtr->sendBufPartialBufPos = 0;
...
int putSessionData( INOUT_PTR SESSION_INFO *sessionInfoPtr, 
	...
	*bytesCopied = 0;
	...
	if( cryptStatusError( status ) )
		return( status );
	LOOP_LARGE_WHILE( length >= availableBuffer )
		{
		ENSURES( LOOP_INVARIANT_LARGE_XXX( length, availableBuffer, 
										   MAX_INTLENGTH ) );
...
int writePkiDatagram( INOUT_PTR SESSION_INFO *sessionInfoPtr, 
	...
	if( cryptStatusError( status ) )
		{
		/* Turn the low-level network error message into a more useful high-
		   level: low-level error message */
		sNetGetErrorInfo( &sessionInfoPtr->stream,
						  &sessionInfoPtr->errorInfo );
		retExtErr( status,
