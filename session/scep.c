...
BOOLEAN sanityCheckSCEPProtocolInfo( IN_PTR \
	...
	if( protocolInfo->transIDsize < 0 || \
		...
		protocolInfo->nonceSize < 0 || \
		protocolInfo->nonceSize > CRYPT_MAX_HASHSIZE )
		{
		DEBUG_PUTS(( "sanityCheckSCEPProtocolInfo: Session state information" ));
		return( FALSE );
		}

	/* Check certificate objects */
	if( !( protocolInfo->iClientTempCert == CRYPT_ERROR || \
		   isHandleRangeValid( protocolInfo->iClientTempCert ) ) || \
		!( protocolInfo->iPkiUser == CRYPT_ERROR || \
		   isHandleRangeValid( protocolInfo->iPkiUser ) ) )
		{
		DEBUG_PUTS(( "sanityCheckSCEPProtocolInfo: Certificate objects" ));
		return( FALSE );
		}

	/* Check password-related information */
	if( !isBooleanValue( protocolInfo->clientSignOnlyKey ) || \
		!isBooleanValue( protocolInfo->caSignOnlyKey ) || \
		protocolInfo->userPasswordSize < 0 || \
		protocolInfo->userPasswordSize > CRYPT_MAX_TEXTSIZE )
		{
		DEBUG_PUTS(( "sanityCheckSCEPProtocolInfo: Password information" ));
		return( FALSE );
		}
...
void initSCEPprotocolInfo( OUT_PTR SCEP_PROTOCOL_INFO *protocolInfo )
	...
	protocolInfo->iClientTempCert = protocolInfo->iPkiUser = CRYPT_ERROR;
	}
...
void destroySCEPprotocolInfo( INOUT_PTR SCEP_PROTOCOL_INFO *protocolInfo )
	...
	if( protocolInfo->iClientTempCert != CRYPT_ERROR )
		{
		krnlSendNotifier( protocolInfo->iClientTempCert, 
...
static BOOLEAN checkPrivateKey( IN_HANDLE const CRYPT_CONTEXT iCryptContext,
								IN_BOOL const BOOLEAN isCertKey )
	{
	BOOLEAN_INT isInited;
	int value, status;

	REQUIRES_B( isHandleRangeValid( iCryptContext ) );
	REQUIRES_B( isBooleanValue( isCertKey ) );
...
static int processUserName( INOUT_PTR SESSION_INFO *sessionInfoPtr,
	...
	if( userNameLength == 12 && !memcmp( userName, "[Autodetect]", 12 ) )
		...
		LOOP_MED( i = 0, i < 16, i++ )
			{
			ENSURES( LOOP_INVARIANT_MED( i, 0, 15 ) );
...
int createScepAttributes( INOUT_PTR SESSION_INFO *sessionInfoPtr,
	...
	REQUIRES( sanityCheckSessionSCEP( sessionInfoPtr ) );
	REQUIRES( sanityCheckSCEPProtocolInfo( protocolInfo ) );
...
static int setAttributeFunction( INOUT_PTR SESSION_INFO *sessionInfoPtr,
	...
	if( type == CRYPT_SESSINFO_CMP_REQUESTTYPE )
		...
		if( requestType == CRYPT_REQUESTTYPE_INITIALISATION )
			...
			if( sessionInfoPtr->privateKey != CRYPT_ERROR && \
				...
				retExt( CRYPT_ARGERROR_NUM1,
						( CRYPT_ARGERROR_NUM1, SESSION_ERRINFO,
						  "Private key already contains an associated "
						  "certificate for '%s'",
			...
			if( sessionInfoPtr->privateKey != CRYPT_ERROR && \
				...
				retExt( CRYPT_ARGERROR_NUM1,
						( CRYPT_ARGERROR_NUM1, SESSION_ERRINFO,
						  "Private key doesn't contain an associated "
						  "certificate" ) );
				}
			if( findSessionInfo( sessionInfoPtr, \
				...
				retExt( CRYPT_ARGERROR_NUM1,
						( CRYPT_ARGERROR_NUM1, SESSION_ERRINFO,
						  "Session already contains a password which "
						  "implies a non-renewal operation" ) );
				}
			}
		...
		if( scepInfo->requestType != CRYPT_REQUESTTYPE_NONE )
			...
			if( scepInfo->requestType == CRYPT_REQUESTTYPE_INITIALISATION )
				...
				if( isInited )
					...
					retExt( CRYPT_ARGERROR_NUM1,
							( CRYPT_ARGERROR_NUM1, SESSION_ERRINFO,
							  "PKCS #10 request for '%s' must be unsigned "
							  "for SCEP use",
							  getCertHolderName( cryptCert, certName, 
												 CRYPT_MAX_TEXTSIZE ) ) );
					}
				}
			else
				...
				if( !isInited )
					...
					retExt( CRYPT_ARGERROR_NUM1,
							( CRYPT_ARGERROR_NUM1, SESSION_ERRINFO,
							  "PKCS #10 request for '%s' must be signed for "
							  "SCEP renewal use",
							  getCertHolderName( cryptCert, certName, 
												 CRYPT_MAX_TEXTSIZE ) ) );
					}
	...
	if( type == CRYPT_SESSINFO_CACERTIFICATE )
		...
		if( !checkSCEPCACert( cryptCert, KEYMGMT_FLAG_NONE ) )
			...
			retExt( CRYPT_ARGERROR_NUM1,
					( CRYPT_ARGERROR_NUM1, SESSION_ERRINFO,
					  "CA certificate for '%s' isn't valid for SCEP use",
					  getCertHolderName( cryptCert, certName, 
										 CRYPT_MAX_TEXTSIZE ) ) );
			}
		}
...
static int checkAttributeFunction( INOUT_PTR SESSION_INFO *sessionInfoPtr,
	...
	if( type == CRYPT_SESSINFO_REQUEST )
		...
		if( cryptStatusError( status ) )
			{
			setObjectErrorInfo( sessionInfoPtr, CRYPT_CERTINFO_COMMONNAME,
								CRYPT_ERRTYPE_ATTR_ABSENT );
			return( CRYPT_ARGERROR_NUM1 );
			}
