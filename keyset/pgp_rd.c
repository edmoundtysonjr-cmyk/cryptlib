...
static int getMPIsize( INOUT_PTR STREAM *stream, 
...
static int readPrivateKeyDecryptionInfo( INOUT_PTR STREAM *stream, 
										 INOUT_PTR PGP_KEYINFO *keyInfo )
	{
	...
	status = readPgpAlgo( stream, &keyInfo->cryptAlgo, 
						  &keyInfo->cryptParam, PGP_ALGOCLASS_PWCRYPT );
	if( cryptStatusError( status ) )
		{
	...
	status = readPgpS2K( stream, &keyInfo->hashAlgo, &keyInfo->hashParam, 
...
static int readRSAKeyComponents( INOUT_PTR STREAM *stream, 
								 INOUT_PTR PGP_KEYINFO *keyInfo,
...
CHECK_RETVAL_SPECIAL STDC_NONNULL_ARG( ( 1, 2, 3 ) ) \
static int readDLPKeyComponents( INOUT_PTR STREAM *stream, 
								 INOUT_PTR PGP_KEYINFO *keyInfo,
								 OUT_INT_Z int *pubKeyComponentLength )
	{
	int length, totalLength = 1, status;
...
CHECK_RETVAL_SPECIAL STDC_NONNULL_ARG( ( 1, 2, 3 ) ) \
static int readECCKeyComponents( INOUT_PTR STREAM *stream, 
								 INOUT_PTR PGP_KEYINFO *keyInfo,
								 OUT_INT_Z int *pubKeyComponentLength )
	{
...
CHECK_RETVAL_SPECIAL STDC_NONNULL_ARG( ( 1, 2, 3 ) ) \
static int readPublicKeyComponents( INOUT_PTR STREAM *stream, 
									INOUT_PTR PGP_KEYINFO *keyInfo,
									OUT_INT_Z int *pubKeyComponentLength )
	{
...
static int readKey( INOUT_PTR STREAM *stream, 
	...
	if( keyInfo->pkcAlgo != CRYPT_ALGO_NONE )
		...
		LOOP_MED( noUserIDs = 0, 
			...
			{
			ENSURES( LOOP_INVARIANT_MED( noUserIDs, 0, 31 ) );
	...
	REQUIRES( isIntegerRangeNZ( pubKeyPayloadPos ) );
	...
	if( cryptStatusOK( status ) )
		...
		status = sMemGetDataBlockAbs( stream, pubKeyPos, 
									  &keyInfo->pubKeyData, 
									  keyInfo->pubKeyDataLen );
		}
	if( cryptStatusError( status ) )
	...
	if( cryptStatusOK( status ) )
		...
		status = sMemGetDataBlockAbs( stream, pubKeyPayloadPos, 
									  &pubKeyPayload, pubKeyPayloadLen );
		}
	if( cryptStatusError( status ) )
		{
		assert( DEBUG_WARN );
	...
	/* Complete the packet header that we read earlier on by adding the
	   length information */
	packetHeader[ 1 ] = intToByte( ( 1 + 4 + length ) >> 8 );
...
static int processPacketGroup( INOUT_PTR STREAM *stream, 
							   INOUT_PTR PGP_INFO *pgpInfo,
							   IN_PTR_OPT const KEY_MATCH_INFO *keyMatchInfo,
							   INOUT_PTR_PTR_OPT \
	...
	if( keyMatchInfo == NULL )
		return( CRYPT_OK );
	ENSURES( matchedKeyInfoPtrPtr != NULL );
...
static int processKeyringPackets( INOUT_PTR STREAM *stream, 
								  IN_ARRAY( maxNoPgpObjects ) \
									PGP_INFO *pgpInfo, 
								  IN_LENGTH_SHORT const int maxNoPgpObjects,
								  OUT_BUFFER_FIXED( bufSize ) BYTE *buffer, 
								  IN_LENGTH_FIXED( KEYRING_BUFSIZE ) \
									const int bufSize,
								  IN_PTR_OPT \
									const KEY_MATCH_INFO *keyMatchInfo,
								  INOUT_PTR_PTR_OPT \
									PGP_KEYINFO **matchedKeyInfoPtrPtr,
								  OUT_PTR BOOLEAN *unhandledDataPresent,
								  INOUT_PTR ERROR_INFO *errorInfo )
	{
...
int pgpScanPubKeyring( INOUT_PTR STREAM *stream, 
	...
	BYTE ALIGN_STACK_DATA streamBuffer[ SAFEBUFFER_SIZE( STREAM_BUFSIZE ) + 8 ];
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2, 4 ) ) \
int pgpReadPrivKeyring( INOUT_PTR STREAM *stream, 
