...
static int copyToEnvelopeAlt( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int writeSignatureInfoPacket( INOUT_PTR STREAM *stream, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int writeHeaderPacket( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
	...
	/* Reset the segmentation state.  Although PGP doesn't segment the 
	   ...
	   payload hashing.  We also set the block size mask to all ones since 
	   we can begin and end data segments on arbitrary boundaries */
...
static int writeKeyex( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	{
	LOOP_INDEX_PTR ACTION_LIST *lastActionPtr;
	int status = CRYPT_OK;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int createSessionKey( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int preEnvelopeEncrypt( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
	CRYPT_DEVICE iCryptDevice = CRYPT_ERROR;
	ACTION_LIST *actionListPtr = \
					DATAPTR_GET( envelopeInfoPtr->actionList );
	LOOP_INDEX_PTR ACTION_LIST *actionListCursor;
	int status;
		...
		if( iCryptDevice != CRYPT_ERROR )
			...
			if( cryptStatusError( status ) || iCryptDevice != iKeyexDevice )
				...
				retExt( CRYPT_ERROR_INVALID,
						( CRYPT_ERROR_INVALID, ENVELOPE_ERRINFO,
						  "Session key object must be associated with the same "
						  "crypto device as the key exchange object" ) );
				}
			}
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int preEnvelopeInit( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
	int status = CRYPT_OK;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int emitMDC( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
