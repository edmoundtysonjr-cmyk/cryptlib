...
/* An 'action list' that defines what we need to do to the content when
   ...
   The flags are:
	...
	FLAG_ADDEDEXTERNALLY: Whether an action was added externally.  This is 
		used to mark hash actions for detached signatures that were added by
		the caller rather than being created as part of the signature-
		verification process.  When we process a detached signature we 
		normally create a hash context to hash the detached content, but if
		there's already a caller-supplied hash context present that was used
		to hash the content then we don't need to to this.  In other words
		instead of returning a CRYPT_ERROR_DUPLICATE we quietly skip it and
		continue.
...
#define ACTION_FLAG_ADDEDEXTERNALLY	0x04	/* Whether action added by caller */
#define ACTION_FLAG_HASHCOMPLETE	0x08	/* Whether hash is complete */
#define ACTION_FLAG_MAX				0x0F	/* Maximum possible flag value */
...
typedef struct {
	...
	int keySetupParam;				/* Optional algorithm parameter */
...
/* Envelope information flags.  These are:
	...
	FLAG_ATTRSKIPPED: The data being de-enveloped contained attributes that 
			were skipped on read due to being unrecognised.  This allows
			special-case actions to be taken if the envelope data is reached 
			without there being any attributes present.
...
#define ENVELOPE_FLAG_ATTRSKIPPED	0x10	/* Env.attributes were skipped */
#define ENVELOPE_FLAG_ZSTREAMINITED	0x20	/* Whether zlib stream has been inited */
#define ENVELOPE_FLAG_AUTHENC		0x40	/* Use authenticated encryption */
#define ENVELOPE_FLAG_MAX			0x7F	/* Maximum possible flag value */
...
typedef CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
		int ( *ENV_ADDINFO_FUNCTION )( INOUT_PTR struct EI *envelopeInfoPtr,
									   IN_ATTRIBUTE \
...
typedef CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
		int ( *ENV_CHECKMISSINGINFO_FUNCTION )( INOUT_PTR struct EI *envelopeInfoPtr,
...
typedef CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
		int ( *ENV_PROCESSPREAMBLE_FUNCTION )( INOUT_PTR struct EI *envelopeInfoPtr );
typedef CHECK_RETVAL_SPECIAL STDC_NONNULL_ARG( ( 1 ) ) \
		int ( *ENV_PROCESSPOSTAMBLE_FUNCTION )( INOUT_PTR struct EI *envelopeInfoPtr,
												IN_BOOL const BOOLEAN isFlush );
typedef CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
		int ( *ENV_COPYTOENVELOPE_FUNCTION )( INOUT_PTR struct EI *envelopeInfoPtr, 
											  IN_BUFFER_OPT( length ) \
													const BYTE *buffer, 
											  IN_DATALENGTH_Z const int length );
typedef CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2, 4 ) ) \
		int ( *ENV_COPYFROMENVELOPE_FUNCTION )( INOUT_PTR struct EI *envelopeInfoPtr, 
...
typedef CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
		int ( *ENV_PROCESSEXTRADATA_FUNCTION )( INOUT_PTR struct EI *envelopeInfoPtr, 
												IN_BUFFER( length ) \
													const void *buffer, 
												IN_DATALENGTH_Z const int length );
typedef CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
...
typedef struct EI {
	...
	int defaultHashParam;				/* Hash/MAC parameter */
...
int getEnvelopeAttribute( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
						  OUT_INT_Z int *valuePtr, 
						  IN_ATTRIBUTE const CRYPT_ATTRIBUTE_TYPE attribute );
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int getEnvelopeAttributeS( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
						   INOUT_PTR MESSAGE_DATA *msgData, 
						   IN_ATTRIBUTE const CRYPT_ATTRIBUTE_TYPE attribute );
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int setEnvelopeAttribute( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
						  IN_INT_Z const int value, 
						  IN_ATTRIBUTE const CRYPT_ATTRIBUTE_TYPE attribute );
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int setEnvelopeAttributeS( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
...
#endif /* !CONFIG_CONSERVE_MEMORY_EXTRA */
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int addKeysetInfo( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int initEnvelopeEncryption( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
							...
							IN_BOOL const BOOLEAN copyContext );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int hashEnvelopeData( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
					  IN_BUFFER( dataLength ) const void *data, 
...
STDC_NONNULL_ARG( ( 1 ) ) \
void initEnvResourceHandling( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr );
STDC_NONNULL_ARG( ( 1 ) ) \
