...
CHECK_RETVAL_BOOL \
static BOOLEAN checkSerialNumber( IN_PTR_OPT const void *serialNumber,
	...
	/* If there's no serial number present then we're done */
...
CHECK_RETVAL_BOOL \
static BOOLEAN checkDataPointers( IN_PTR_OPT const void *certificatePtr,
								  IN_LENGTH_Z const int certificateSize,
								  IN_PTR_OPT const void *dataPtr,
								  IN_PTR_OPT const void *dataStoragePtr,
								  IN_LENGTH_SHORT_Z const int dataSize,
								  IN_DATAPTR const DATAPTR dataObject )
	{
	...
	REQUIRES_B( ( certificatePtr == NULL && certificateSize == 0 ) || \
				( certificatePtr != NULL && \
				  isIntegerRangeNZ( certificateSize ) ) );
...
static BOOLEAN sanityCheckCertificate( const CERT_INFO *certInfoPtr )
	...
	if( certInfo->chainEnd != 0 )
		...
		{
		if( certInfo->chainPos != -1 )
			{
			DEBUG_PUTS(( "sanityCheckCert: Spurious chain info" ));
			return( FALSE );
			}
		}
	...
	if( certInfo->hashAlgo != CRYPT_ALGO_NONE )
		...
		if( !isHashAlgo( certInfo->hashAlgo ) || \
			certInfo->hashParam < MIN_HASHSIZE || \
			certInfo->hashParam > CRYPT_MAX_HASHSIZE )
			{
...
static BOOLEAN sanityCheckCertRev( const CERT_INFO *certInfoPtr )
	...
	if( revInfo->hashAlgo != CRYPT_ALGO_NONE )
		{
		if( !isHashAlgo( revInfo->hashAlgo ) || \
...
BOOLEAN sanityCheckCert( IN_PTR const CERT_INFO *certInfoPtr )
	...
	if( certInfoPtr->certificate == NULL )
		...
		switch( certInfoPtr->type )
			...
			case CRYPT_CERTTYPE_CRL:
				/* CRLs can get arbitrarily large so we allow large upper 
				   bounds on the length */
				if( !isIntegerRangeMin( certInfoPtr->certificateSize, 
			...
			case CRYPT_CERTTYPE_RTCS_REQUEST:
				/* RTCS requests are very compact so we allow a smaller 
				   lower bound */
				if( !isShortIntegerRangeMin( certInfoPtr->certificateSize, 32 ) )
					{
					DEBUG_PUTS(( "sanityCheckCert: Certificate data" ));
					return( FALSE );
					}
			...
			case CRYPT_CERTTYPE_RTCS_RESPONSE:
				/* RTCS responses are even more compact */
				if( !isShortIntegerRangeMin( certInfoPtr->certificateSize, 24 ) )
					{
					DEBUG_PUTS(( "sanityCheckCert: Certificate data" ));
					return( FALSE );
					}
			...
			default:
				if( !isShortIntegerRangeMin( certInfoPtr->certificateSize, 
										MIN_CRYPT_OBJECTSIZE ) )
					{
					DEBUG_PUTS(( "sanityCheckCert: Certificate data" ));
					return( FALSE );
	...
	if( !checkVarStruct( certInfoPtr ) )
		{
		DEBUG_PUTS(( "sanityCheckCert: VarStruct" ));
		return( FALSE );
		}
	...
	if( certInfoPtr->publicKeyInfo == NULL )
		...
		{
		if( !isShortIntegerRangeMin( certInfoPtr->publicKeyInfoSize, 
									 MIN_PKCSIZE_ECC ) )
			{
	...
	if( certInfoPtr->subjectDNsize == 0 )
		...
		if( !isShortIntegerRangeMin( certInfoPtr->subjectDNsize, 
									 MIN_DN_SIZE ) )
			{
			if( !nullDN )
				{
	...
	if( certInfoPtr->issuerDNsize == 0 )
		...
		if( !isShortIntegerRangeMin( certInfoPtr->issuerDNsize, 
									 MIN_DN_SIZE ) )
			{
	...
	if( !isEnumRangeOpt( certInfoPtr->errorLocus, CRYPT_ATTRIBUTE ) || \
		!isEnumRangeOpt( certInfoPtr->errorType, CRYPT_ERRTYPE ) )
		{
		DEBUG_PUTS(( "sanityCheckCert: Error info" ));
		return( FALSE );
		}
