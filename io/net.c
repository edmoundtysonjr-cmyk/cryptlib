...
BOOLEAN sanityCheckNetStream( const NET_STREAM_INFO *netStream )
	...
	if( !isEnumRange( netStream->protocol, STREAM_PROTOCOL ) || \
		!isEnumRangeOpt( netStream->subProtocol, CRYPT_SUBPROTOCOL ) || \
	...
	if( !TEST_FLAG( netStream->nFlags, STREAM_NFLAG_USERSOCKET ) )
		...
		if( netStream->port < MIN_PORT_NUMBER || \
			netStream->port > MAX_DEST_PORT_NUMBER )
			{
	...
	if( netStream->clientAddressLen < 0 || \
		...
		{
		DEBUG_PUTS(( "sanityCheckNetStream: Client address" ));
		return( FALSE );
		}
	if( netStream->clientAddressBinaryLen != 0 && \
		netStream->clientAddressBinaryLen != 4 && \
		netStream->clientAddressBinaryLen != 16 )
		{
		DEBUG_PUTS(( "sanityCheckNetStream: Client address (binary)" ));
		return( FALSE );
		}
	...
	if( netStream->writeBuffer == NULL )
		...
		if( !isBufsizeRangeNZ( netStream->writeBufSize ) )
			{
	...
	if( !FNPTR_ISVALID( netStream->connectFunctionOpt ) || \
		...
		!FNPTR_ISVALID( netStream->getMetadataFunctionOpt ) )
		{
...
static int checkForProxy( INOUT_PTR NET_STREAM_INFO *netStream, 
...
static int openNetworkConnection( INOUT_PTR NET_STREAM_INFO *netStream, 
								  IN_ENUM( NET_OPTION ) \
...
static int initStream( OUT_PTR STREAM *stream, 
...
/* Set up pointers to stream-internal buffers if required:
	...
	v				v
	+---------------+---------+----------+---------+--------+------------+--------------+
	|NET_STREAM_INFO|Read buf.|Write buf.|Host name|Loc.name|Subtype data|Transport data|
	+---------------+---------+----------+---------+--------+------------+--------------+
			|				  ^			 ^		   ^		^			 ^
			|				  |writeBuffr|host	   |location|subtypeInfo |transportInfo
			|				  |			 |		   |		|			 |
			+-----------------+----------+---------+--------+------------+ */

CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int initStreamStorage( INOUT_PTR STREAM *stream,
							  INOUT_PTR NET_STREAM_INFO *netStream,
							  IN_DATALENGTH_Z const int netStreamAllocSize,
							  IN_ENUM( STREAM_PROTOCOL ) \
									const STREAM_PROTOCOL_TYPE protocol,
	...
	REQUIRES( isEnumRange( protocol, STREAM_PROTOCOL ) );
	REQUIRES( isBufsizeRange( netStreamAllocSize ) );
	...
	if( protocol != STREAM_PROTOCOL_TCP && protocol != STREAM_PROTOCOL_UDP )
		...
		stream->buffer = SAFEBUFFER_PTR( netStream->storage );
						 /* The assignment above will generate a compiler 
						    warning about running off the end of the 
							variable-size VARSTRUCT array */
		...
		netStream->writeBuffer = \
					...
					/* The assignment above will generate a compiler warning 
					   about running off the end of the variable-size 
					   VARSTRUCT array */
...
STDC_NONNULL_ARG( ( 1 ) ) \
static void cleanupStream( INOUT_PTR STREAM *stream, 
						   IN_BOOL const BOOLEAN cleanupTransport )
	{
	...
	REQUIRES_V( isBooleanValue( cleanupTransport ) );
...
static int processConnectOptions( INOUT_PTR STREAM *stream, 
								  INOUT_PTR NET_STREAM_INFO *netStream,
...
static int completeConnect( INOUT_PTR STREAM *stream,
							INOUT_PTR NET_STREAM_INFO *netStreamTemplate, 
							IN_PTR const NET_CONNECT_INFO *connectInfo, 
							IN_PTR_OPT const URL_INFO *urlInfo,
							IN_ENUM( STREAM_PROTOCOL ) \
								const STREAM_PROTOCOL_TYPE protocol,
	...
	/* If we're using an optional transport protocol-negotiation step, 
	   activate that too */
