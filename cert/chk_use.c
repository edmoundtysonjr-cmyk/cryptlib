...
#define ALGO_TYPE_NONE			0x00
#define ALGO_TYPE_SIGN			0x01
#define ALGO_TYPE_CRYPT			0x02
#define ALGO_TYPE_KEYAGREEMENT	0x04
#define ALGO_TYPE_FLAG_NONE		0x00	/* Defines for range checking */
...
static int getExtendedKeyUsageFlags( IN_DATAPTR const DATAPTR_ATTRIBUTE attributes,
	...
	{
	LOOP_INDEX i;
	int keyUsage = CRYPT_KEYUSAGE_NONE;
	...
	*errorLocus = CRYPT_ATTRIBUTE_NONE;
	...
	LOOP_MED( i = 0, 
		...
		ENSURES( LOOP_INVARIANT_MED( i, 0, \
									 FAILSAFE_ARRAYSIZE( extendedUsageInfo, \
														 EXT_USAGE_INFO ) - 1 ) );
	...
	*errorLocus = CRYPT_ATTRIBUTE_NONE;
	...
	LOOP_SMALL( i = 0,
		...
		ENSURES( LOOP_INVARIANT_SMALL( i, 0, \
									   FAILSAFE_ARRAYSIZE( nsCertTypeInfo, \
														   NS_CERT_TYPE_INFO ) - 1 ) );
...
int getKeyUsageFromExtKeyUsage( IN_PTR const CERT_INFO *certInfoPtr,
	...
	assert( isWritePtr( errorCertInfoPtr, sizeof( CERT_INFO ) ) );
	...
	*keyUsage = CRYPT_KEYUSAGE_NONE;
	...
	errorLocus = &errorCertInfoPtr->errorLocus;
	errorType = &errorCertInfoPtr->errorType;
	errorInfo = &errorCertInfoPtr->errorInfo;
	...
	if( isKeyexAlgo( certInfoPtr->publicKeyAlgo ) )
...
/* Check that a certificate/key is valid for a particular purpose.  This 
   ...
   This function has a somewhat unusual function signature in that error 
   information is returned in a separate error-information certInfoPtr.  
   This is necessary because it's frequently used to check that an issuer 
   certificate is OK, but we want to set the error information in the 
   subject certificate, not the issuer */
...
int checkKeyUsage( IN_PTR const CERT_INFO *certInfoPtr,
	...
	if( certInfoPtr->type == CRYPT_CERTTYPE_CERTIFICATE || \
		...
		if( specificUsage != CRYPT_KEYUSAGE_NONE && \
			...
			retExt( CRYPT_ERROR_INVALID,
					( CRYPT_ERROR_INVALID, errorInfo, 
					  "Key in %s isn't trusted for the requested usage",
	...
	if( complianceLevel < CRYPT_COMPLIANCELEVEL_REDUCED )
		{
		ENSURES( CFI_CHECK_SEQUENCE_1( "trustedUsage" ) );
		return( CRYPT_OK );
		}
	...
	/* Get the certificate's keyUsage.  If we're running at a reduced 
	   ...
	   versa then we synthesise the required value from the other value in 
	   order to pass the checks that follow */
	...
	if( DATAPTR_ISSET( attributePtr ) )
		...
		/* If the CA key usages are set then make sure that the CA flag is 
		...
		/* If the CA flag is set then make sure that the keyUsage is set in 
	...
	if( flags & CHECKKEY_FLAG_CA )
		...
		if( !isCA )
			...
			retExt( CRYPT_ERROR_INVALID,
					( CRYPT_ERROR_INVALID, errorInfo, 
					  "Non-CA %s contains CA keyUsage attributes",
					  getCertTypeNameLC( certInfoPtr->type ) ) );
			}
		if( !( caKeyUsage & specificUsage ) )
			...
			retExt( CRYPT_ERROR_INVALID,
					( CRYPT_ERROR_INVALID, errorInfo, 
					  "Key in CA %s isn't trusted for the requested usage",
					  getCertTypeNameLC( certInfoPtr->type ) ) );
			}
		}
	...
	if( complianceLevel < CRYPT_COMPLIANCELEVEL_STANDARD )
		{
		ENSURES( CFI_CHECK_SEQUENCE_6( "trustedUsage", 
									   "CRYPT_COMPLIANCELEVEL_REDUCED", 
	...
	if( !isGeneralCheck )
		{
		ENSURES( CFI_CHECK_SEQUENCE_7( "trustedUsage", 
									   "CRYPT_COMPLIANCELEVEL_REDUCED", 
									   "selfSigned", "keyUsage", "caKeyUsage", 
									   "trustedUsage", 
	...
	if( isCA )
		...
		if( !caKeyUsage )
			...
			retExt( CRYPT_ERROR_INVALID,
					( CRYPT_ERROR_INVALID, errorInfo, 
					  "CA %s doesn't contain CA keyUsage attributes",
					  getCertTypeNameLC( certInfoPtr->type ) ) );
			}
		}	
	else
	...
	if( specificUsage != CRYPT_KEYUSAGE_NONE )
		...
		if( !usageOK )
			...
			retExt( CRYPT_ERROR_INVALID,
					( CRYPT_ERROR_INVALID, errorInfo, 
					  "Key in %s isn't valid for the requested usage",
					  getCertTypeNameLC( certInfoPtr->type ) ) );
			}
		}
	...
	if( ( keyUsage & CRYPT_KEYUSAGE_ENCIPHERONLY ) && \
		...
		retExt( CRYPT_ERROR_INVALID,
				( CRYPT_ERROR_INVALID, errorInfo, 
				  "%s contains mutually exclusive keyUsage flags",
				  getCertTypeName( certInfoPtr->type ) ) );
		}
	...
	if( ( ( ( keyUsage & KEYUSAGE_CRYPT ) && \
		  ...
		  ( ( keyUsage & KEYUSAGE_KEYAGREEMENT ) && \
			  !isKeyexAlgo( certInfoPtr->publicKeyAlgo ) ) ) ) 
		{
		setErrorValues( CRYPT_CERTINFO_KEYUSAGE, CRYPT_ERRTYPE_ATTR_VALUE );
		retExt( CRYPT_ERROR_INVALID,
				( CRYPT_ERROR_INVALID, errorInfo, 
				  "%s contains keyUsage attribute that the key isn't "
				  "actually capable of", 
				  getCertTypeName( certInfoPtr->type ) ) );
		}
	...
	/* Mask out any non-relevant usages such as certificate signing, which 
	...
	if( ( flags & CHECKKEY_FLAG_PRIVATEKEY ) && \
		...
		if( currentTime <= MIN_TIME_VALUE )
			...
			retExt( CRYPT_ERROR_INVALID,
					( CRYPT_ERROR_INVALID, errorInfo, 
					  "System time is broken, can't check private key "
					  "validity" ) );
			}
	...
	if( isCA && ( keyUsage & extKeyUsage & KEYUSAGE_CRYPT ) )
		...
		retExt( CRYPT_ERROR_INVALID,
				( CRYPT_ERROR_INVALID, errorInfo, 
				  "CA %s contains non-permitted encryption keyUsage flag",
				  getCertTypeNameLC( certInfoPtr->type ) ) );
		}
	...
	if( ( keyUsage & extKeyUsage ) != extKeyUsage )
		...
		retExt( CRYPT_ERROR_INVALID,
				( CRYPT_ERROR_INVALID, errorInfo, 
				  "%s extended key usage isn't consistent with actual key "
				  "usage", getCertTypeName( certInfoPtr->type ) ) );
		}
	...
	if( ( keyUsage & ( CRYPT_KEYUSAGE_ENCIPHERONLY | \
		...
		retExt( CRYPT_ERROR_INVALID,
				( CRYPT_ERROR_INVALID, errorInfo, 
				  "%s contains encipherOnly/decipherOnly keyUsage flag but "
				  "not keyAgreement keyUsage flag",
				  getCertTypeName( certInfoPtr->type ) ) );
		}
	CFI_CHECK_UPDATE( "extUsageChecks" );

	ENSURES( CFI_CHECK_SEQUENCE_13( "trustedUsage", 
									"CRYPT_COMPLIANCELEVEL_REDUCED", 
									"selfSigned", "keyUsage", "caKeyUsage", 
									"trustedUsage", 
									"CRYPT_COMPLIANCELEVEL_STANDARD", 
									...
									"extUsageChecks" ) );
#else
	ENSURES( CFI_CHECK_SEQUENCE_10( "trustedUsage", 
									"CRYPT_COMPLIANCELEVEL_REDUCED", 
									"selfSigned", "keyUsage", "caKeyUsage", 
									"trustedUsage", 
									"CRYPT_COMPLIANCELEVEL_STANDARD", 
