...
static int checkResponder( INOUT_PTR CERT_INFO *certInfoPtr,
	...
	/* Activate the session and get the response information.  Since the 
	   session activation can take a fair amount of time, we re-verify 
	   (sanity-check) the certificate object data after it's completed */
	status = krnlSendMessage( iCryptSession, IMESSAGE_SETATTRIBUTE,
	...
	if( cryptStatusError( status ) )
		{
		retExtObj( status,
				   ( status, CERTIFICATE_ERRINFO, iCryptSession,
				     "%s responder query failed", 
					 ( sessionType == SUBTYPE_SESSION_RTCS ) ? \
					   "RTCS" : "OCSP" ) );
		}
	...
	if( sessionType == SUBTYPE_SESSION_RTCS )
		...
		if( cryptStatusOK( status ) && \
			( responderStatus != CRYPT_CERTSTATUS_VALID ) )
		...
		if( cryptStatusOK( status ) && \
			( responderStatus != CRYPT_OCSPSTATUS_NOTREVOKED ) )
			status = CRYPT_ERROR_INVALID;
		}
	...
	ENSURES( CFI_CHECK_SEQUENCE_3( "IMESSAGE_SETATTRIBUTE", "activateSession", 
	...
	if( cryptStatusError( status ) )
		...
		retExt( status,
				( status, CERTIFICATE_ERRINFO, 
				  "%s responder query reported certificate status "
				  "invalid (%d)", 
				  ( sessionType == SUBTYPE_SESSION_RTCS ) ? \
					"RTCS" : "OCSP", responderStatus ) );
		}
...
/* Check a certificate object against a signing certificate object, 
   ...
   as follows:
	...
	basicCheckDone: Used if checkCertBasic() has already been done by the 
			caller.  This is because when we're coming from the certificate-
			chain code then we're processing a certificate in the middle of 
			the chain and it hasn't been done yet, but if we're coming from 
			the standalone certificate-checking code then it's already been 
			done */
...
int checkCertDetails( INOUT_PTR CERT_INFO *subjectCertInfoPtr,
	...
	REQUIRES( isBooleanValue( trustAnchorCheck ) );
	REQUIRES( isBooleanValue( shortCircuitCheck ) );
	REQUIRES( isBooleanValue( basicCheckDone ) );

	/* Clear any existing error state that may be present for the 
	   certificate.  This can be warning-level information like the key not
	   being implicitly trusted that we don't want leaking through in case 
	   we encounter an error that doesn't set error state details */
	clearErrorInfo( SUBJECTCERT_ERRINFO );
	subjectCertInfoPtr->errorLocus = CRYPT_ATTRIBUTE_NONE;
	subjectCertInfoPtr->errorType = CRYPT_ERRTYPE_NONE;
	...
	if( TEST_FLAG( subjectCertInfoPtr->flags, CERT_FLAG_SIGCHECKED ) || \
		...
		{
		ENSURES( CFI_CHECK_SEQUENCE_2( "checkCertBasic", "checkCert" ) );
	...
	/* If it's an intermediate or leaf certificate and it's not a v3
	   ...
	   leaf certificate doesn't need it.  
	   
	   Since there's no point to this check, our conforming implementation 
	   chooses not to reject certificates based on it */
#if defined( USE_CERTLEVEL_PKIX_FULL ) && 0
	if( !trustAnchorCheck && \
		subjectCertInfoPtr->type == CRYPT_CERTINFO_CERTIFICATE && \
		...
		retExt( CRYPT_ERROR_INVALID,
				( CRYPT_ERROR_INVALID, SUBJECTCERT_ERRINFO, 
				  "Intermediate or leaf certificate isn't a v3 "
				  "certificate" ) );
		}
	...
	if( cryptStatusError( status ) )
		...
		int subjectIDlength, issuerIDlength DUMMY_INIT;
		...
		if( status != CRYPT_ERROR_BADDATA || issuerCertInfoPtr == NULL )
			{
			retExtErr( status,
					   ( status, SUBJECTCERT_ERRINFO, &localErrorInfo,
		...
		/* Get the subject certificate's issuerID and the issuer 
		   ...
		   difference in error codes.  
		   
		   Since the overall error status at this point is 
		   CRYPT_ERROR_BADDATA (see the status-code check above) we return 
		   that instead of any ephemeral status values returned from further 
		   function calls */
		...
		if( cryptStatusError( status ) )
			{
			retExtErr( CRYPT_ERROR_BADDATA,
					   ( CRYPT_ERROR_BADDATA, SUBJECTCERT_ERRINFO, 
						 &localErrorInfo,
						 "%s signature check failed", 
						 getCertTypeName( subjectCertInfoPtr->type ) ) );
			}
		...
		if( ( issuerIDlength != subjectIDlength ) || \
			...
			{
			DEBUG_DIAG(( "%s signature check failed due to mismatched key "
						 "in CA certificate",
						 getCertTypeName( subjectCertInfoPtr->type ) ));
			assert( DEBUG_WARN );
			retExtErr( CRYPT_ERROR_SIGNATURE,
					   ( CRYPT_ERROR_SIGNATURE, SUBJECTCERT_ERRINFO, 
						 &localErrorInfo,
						 "%s signature check failed due to mismatched key "
						 "in CA certificate",
						 getCertTypeName( subjectCertInfoPtr->type ) ) );
			}
	...
	ENSURES( CFI_CHECK_SEQUENCE_4( "checkCertBasic", "checkCert", 
								   "IMESSAGE_USER_TRUSTMGMT", 
								   "checkX509signature" ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int checkSelfSignedCert( INOUT_PTR CERT_INFO *certInfoPtr,
								IN_PTR_OPT \
	...
	if( TEST_FLAG( certInfoPtr->flags, CERT_FLAG_SELFSIGNED ) )
		...
		if( certInfoPtr->type == CRYPT_CERTTYPE_CERTIFICATE || \
			...
			if( cryptStatusOK( status ) )
				{
				ENSURES( CFI_CHECK_SEQUENCE_1( "checkCertBasic" ) );
		...
		if( cryptStatusError( status ) )
			...
			retExt( CRYPT_ARGERROR_VALUE,
					( CRYPT_ARGERROR_VALUE, CERTIFICATE_ERRINFO, 
					  "No trusted CA certificate available to validate %s",
	...
	ENSURES( CFI_CHECK_SEQUENCE_3( "checkCertBasic", 
								   "IMESSAGE_USER_TRUSTMGMT", 
								   "checkCertDetails" ) );
...
/* Check the validity of a certificate object against another object, 
   ...
   session.  This single enormous function is an indication of how complex
   any PKI validation process quickly becomes.
   ...
   situation is created by a small number of TLS implementations, most 
   ...
   To check the chain { B - C - D } itself will require an explicit call for
   that, which allows returning error information in the case of a check
   failure in { B - C - D }.  In other words checking both { A } against
   { B - C - D } and { B - C - D } itself requires two calls, the first to
   check { A } against { B } and the second to check { B - C - D } */
...
int checkCertValidity( INOUT_PTR CERT_INFO *certInfoPtr, 
	...
	if( certInfoPtr->type == CRYPT_CERTTYPE_CERTCHAIN )
		...
		/* Get the certificate check-object type for error reporting 
		   purposes */
		status = krnlSendMessage( iSigCheckObject, IMESSAGE_GETATTRIBUTE, 
								  &certSubType, CRYPT_CERTINFO_CERTTYPE );
		if( cryptStatusError( status ) )
			{
		...
		if( checkObjectType != OBJECT_TYPE_CERTIFICATE || \
			...
			retExt( CRYPT_ARGERROR_VALUE,
					( CRYPT_ARGERROR_VALUE, CERTIFICATE_ERRINFO, 
					  "%s can only be checked against a certificate",
					  getCertTypeName( certInfoPtr->type ) ) );
			}
		ENSURES( checkObjectType == OBJECT_TYPE_CERTIFICATE );
		...
		if( !checkContextCapability( iSigCheckObject, 
			...
			retExt( CRYPT_ARGERROR_VALUE,
					( CRYPT_ARGERROR_VALUE, CERTIFICATE_ERRINFO, 
					  "%s being used for checking isn't a CA certificate",
					  getCertTypeName( certSubType ) ) );
			}
		status = krnlSendMessage( certInfoPtr->ownerHandle, 
								  IMESSAGE_USER_TRUSTMGMT, 
		...
		if( cryptStatusError( status ) )
			...
			retExt( CRYPT_ARGERROR_VALUE,
					( CRYPT_ARGERROR_VALUE, CERTIFICATE_ERRINFO, 
					  "%s being used for checking isn't trusted",
					  getCertTypeName( certSubType ) ) );
			}
		...
		if( cryptStatusError( status ) )
			...
			retExt( CRYPT_ARGERROR_VALUE,
					( CRYPT_ARGERROR_VALUE, CERTIFICATE_ERRINFO, 
					  "Check of %s using trusted CA certificate failed",
					  getCertTypeNameLC( certSubType ) ) );
			}
	...
	if( TEST_FLAG( certInfoPtr->flags, CERT_FLAG_SELFSIGNED ) )
		...
		if( cryptStatusError( status ) )
			...
			retExt( CRYPT_ARGERROR_VALUE,
					( CRYPT_ARGERROR_VALUE, CERTIFICATE_ERRINFO, 
					  "Key being used to check the self-signed %s differs "
					  "from the one in the checking %s", 
					  getCertTypeNameLC( certInfoPtr->type ),
					  ( checkObjectType == OBJECT_TYPE_CERTIFICATE ) ? \
						"certificate" : "context" ) );
			}
		...
		if( checkObjectType == OBJECT_TYPE_CERTIFICATE )
			...
			if( cryptStatusError( status ) )
				...
				retExt( CRYPT_ARGERROR_VALUE,
						( CRYPT_ARGERROR_VALUE, CERTIFICATE_ERRINFO, 
						  "Certificate being used to check the self-signed "
						  "%s isn't the same as the self-signed %s", 
						  getCertTypeNameLC( certInfoPtr->type ), 
						  getCertTypeNameLC( certInfoPtr->type ) ) );
				}
			}
	...
	if( checkObjectType == OBJECT_TYPE_CERTIFICATE && \
		...
		if( cryptStatusError( status ) )
			...
			retExt( CRYPT_ARGERROR_VALUE,
					( CRYPT_ARGERROR_VALUE, CERTIFICATE_ERRINFO, 
					  "Certificate chain being used to check %s isn't "
					  "trusted", getCertTypeNameLC( certInfoPtr->type ) ) );
			}
	...
	ENSURES( CFI_CHECK_SEQUENCE_6( "IMESSAGE_GETATTRIBUTE", "checkObjectSubtype", 
