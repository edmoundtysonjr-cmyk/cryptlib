...
static int processUserManagement( INOUT_PTR USER_INFO *userInfoPtr, 
...
static int processTrustManagement( INOUT_PTR USER_INFO *userInfoPtr, 
	...
	/* iCertificate is an IN parameter for all except 
	   MESSAGE_TRUSTMGMT_GETISSUER, where it's an INOUT_PTR.  For this reason we
	   don't clear it as a return value */
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int userMessageFunction( INOUT_PTR TYPECAST( USER_INFO * ) \
									void *objectInfoPtr,
...
static int openUser( OUT_HANDLE_OPT CRYPT_USER *iCryptUser, 
					 ...
					 OUT_PTR_PTR_OPT USER_INFO **userInfoPtrPtr )
	{
