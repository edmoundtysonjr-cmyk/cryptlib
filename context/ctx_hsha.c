...
static int selfTest( void )
	...
	ALIGN_DATA( macState, SHA1_MAC_STATE_SIZE, 8 );
...
static int hash( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
	...
	if( noBytes > 0 )
		...
		LOOP_LARGE( i = 0, i < macInfo->userKeyLength, i++ )
			{
			ENSURES( LOOP_INVARIANT_LARGE( i, 0, macInfo->userKeyLength - 1 ) );
		...
		zeroise( hashBuffer, SHA_CBLOCK );
		SHA1_Update( shaInfo, digestBuffer, SHA_DIGEST_LENGTH );
		zeroise( digestBuffer, SHA_DIGEST_LENGTH );
...
static int initKey( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
	...
	/* Perform the start of the inner hash using the zero-padded key XOR'd
	   ...
	   this will ever be an issue but we do it just in case */
	REQUIRES( rangeCheck( macInfo->userKeyLength, 1, SHA_CBLOCK ) );
	memcpy( hashBuffer, macInfo->userKey,
			macInfo->userKeyLength );
	...
	LOOP_LARGE( i = 0, i < SHA_CBLOCK, i++ )
		{
		ENSURES( LOOP_INVARIANT_LARGE( i, 0, SHA_CBLOCK - 1 ) );
...
static const CAPABILITY_INFO capabilityInfo = {
	...
	MIN_KEYSIZE, bitsToBytes( 160 ), CRYPT_MAX_KEYSIZE,
