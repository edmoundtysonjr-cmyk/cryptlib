...
static int getKeyIDs( INOUT_PTR PKCS15_INFO *pkcs15infoPtr,
	...
	if( pkcs15infoPtr->uriLength <= 0 )
		{
		setMessageData( &msgData, pkcs15infoPtr->uri, CRYPT_MAX_TEXTSIZE );
		status = krnlSendMessage( iCryptContext, IMESSAGE_GETATTRIBUTE_S,
								  &msgData, CRYPT_IATTRIBUTE_HOLDERURI );
		if( cryptStatusOK( status ) )
			{
			/* Only present for certificates so an error isn't fatal */
			pkcs15infoPtr->uriLength = msgData.length;
			}
		}
...
static int getCertIDs( INOUT_PTR PKCS15_INFO *pkcs15infoPtr, 
					   IN_HANDLE const CRYPT_HANDLE iCryptCert, 
					   OUT_PTR BOOLEAN *isCA, 
					   OUT_PTR BOOLEAN *trustedImplicit, 
					   OUT_PTR int *trustedUsage )
	{
	int value, status;
	...
	*trustedUsage = CRYPT_UNUSED;
	...
	if( cryptStatusOK( status ) )
		...
		if( cryptStatusOK( status ) )
			*trustedImplicit = value;	/* BOOLEAN vs. int */
			...
			if( status == CRYPT_ERROR_NOTFOUND )
				{
				/* If it's not implicitly trusted, don't store a trust 
				   setting */
	...
	/* In addition to the various IDs we also opportunistically try and set 
	   the label to the certificate holder name.  This is to allow the
	   certificate to be addressed, and specifically deleted, since 
	   otherwise there's no way to manipulate standalone certificates in a 
	   keyset because only internal IDs are stored */
	if( pkcs15infoPtr->labelLength <= 0 )
		{
		MESSAGE_DATA msgData;
...
static int sizeofObjectIDs( const PKCS15_INFO *pkcs15infoPtr )
	...
	if( pkcs15infoPtr->uriLength > 0 )
		...
		identifierSize += \
			sizeofShortObject( \
				sizeofShortInteger( PKCS15_KEYID_URI ) + \
				sizeofShortObject( pkcs15infoPtr->uriLength ) );
		}
...
static int writeObjectIDs( INOUT_PTR STREAM *stream, 
	...
	if( pkcs15infoPtr->uriLength > 0 )
		{
		writeSequence( stream, sizeofShortInteger( PKCS15_KEYID_URI ) + \
							   sizeofShortObject( pkcs15infoPtr->uriLength ) );
		writeShortInteger( stream, PKCS15_KEYID_URI, DEFAULT_TAG );
		status = writeCharacterString( stream, pkcs15infoPtr->uri,
									   pkcs15infoPtr->uriLength, 
									   BER_STRING_UTF8 );
		}
...
int writeKeyAttributes( OUT_BUFFER( privKeyAttributeMaxLen, \
						...
						INOUT_PTR PKCS15_INFO *pkcs15infoPtr, 
						IN_HANDLE const CRYPT_HANDLE iCryptContext,
						IN_BOOL const BOOLEAN writeKeyIDs )
	{
	STREAM stream;
	...
	REQUIRES( isShortIntegerRangeMin( privKeyAttributeMaxLen, 16 ) );
	REQUIRES( isShortIntegerRangeMin( pubKeyAttributeMaxLen, 16 ) );
	REQUIRES( isHandleRangeValid( iCryptContext ) );

	/* Clear return values */
	REQUIRES( isShortIntegerRangeNZ( privKeyAttributeMaxLen ) ); 
	memset( privKeyAttributes, 0, min( 16, privKeyAttributeMaxLen ) );
	REQUIRES( isShortIntegerRangeNZ( pubKeyAttributeMaxLen ) ); 
...
int writeCertAttributes( OUT_BUFFER( certAttributeMaxLen, *certAttributeSize ) \
	...
	REQUIRES( isShortIntegerRangeMin( certAttributeMaxLen, 16 ) );
	REQUIRES( isHandleRangeValid( iCryptCert ) );

	/* Clear return values */
	REQUIRES( isShortIntegerRangeNZ( certAttributeMaxLen ) ); 
