...
static BOOLEAN testSafetyMechanisms( void )
	...
	/* Test the TMR value handling mechanisms */
	TMR_SET( tmrInt, 20 );
	if( !TMR_VALID( tmrInt ) || TMR_GET( tmrInt ) != 20 )
		return( FALSE );
	tmrIntA = 0;
	tmrIntB |= 0x800;
	tmrIntC |= 0x01;
	if( TMR_VALID( tmrInt ) || TMR_GET( tmrInt ) != 20 )
		return( FALSE );
	TMR_SCRUB( tmrInt );
	if( tmrIntA != 20 || tmrIntB != 20 || tmrIntC != 20 )
		return( FALSE );
	...
	if( !checkOverflowAdd( maxInt, 1 ) || \
		...
		checkOverflowAdd( maxIntSub10, 9 ) || \
		!checkOverflowAdd( maxIntSub10, 10 ) || \
	...
	if( checkOverflowMul( maxIntDiv10, 9) || \
		!checkOverflowMul( maxIntDiv10, 10 ) || \
		!checkOverflowMul( maxIntDiv10, 11 ) || \
		!checkOverflowMul( maxIntDiv10, maxInt ) )
		return( FALSE );
	...
	/* Test the bounded-loop mechanisms.  There are many variants of this, 
	   we only check the basic forms since they're all mapped to identical
	   constructs via macros */
	LOOP_SMALL( i = 0, i < 5, i++ )
		{
		/* Basic operation */
		if( !LOOP_INVARIANT_SMALL( i, 0, 4 ) )
			return( FALSE );
		}
	if( !LOOP_BOUND_OK )
		return( FALSE );
	LOOP_SMALL( i = 3, i < 5, i++ )
		{
		/* Start at nonzero value */
		if( !LOOP_INVARIANT_SMALL( i, 3, 4 ) )
			return( FALSE );
		}
	if( !LOOP_BOUND_OK )
		return( FALSE );
	LOOP_SMALL( i = 0, i < 20, i++ )
		{
		/* Detect bounds-exceeded */
		if( !LOOP_INVARIANT_SMALL( i, 0, 19 ) )
			return( FALSE );
		}
	if( LOOP_BOUND_OK )
		return( FALSE );
	...
	LOOP_SMALL_ALT( i = 0, i < 5, i++ )
		{
		if( !LOOP_INVARIANT_SMALL_ALT( i, 0, 4 ) )
			return( FALSE );
		}
	...
	LOOP_SMALL_ALT( i = 0, i < 20, i++ )
		{
		if( !LOOP_INVARIANT_SMALL_ALT( i, 0, 19 ) )
			return( FALSE );
		}
	if( LOOP_BOUND_OK_ALT )
		return( FALSE );
/*	LOOP_SMALL_ALT2( i = 0, i < 20, i++ )
		{
		ENSURES( LOOP_INVARIANT_SMALL_ALT2( i, 0, 19 ) );
		}
	if( LOOP_BOUND_OK_ALT2 )
		return( FALSE ); - No LOOP_SMALL_ALT2 */
	LOOP_SMALL_REV( i = 5, i >= 0, i-- )
		{
		if( !LOOP_INVARIANT_REV( i, 0, 5 ) )
			return( FALSE );
		}
	if( !LOOP_BOUND_SMALL_REV_OK )
		return( FALSE );
	LOOP_SMALL_REV( i = 20, i >= 0, i-- )
		{
		if( !LOOP_INVARIANT_REV( i, 0, 20 ) )
			return( FALSE );
		}
	if( LOOP_BOUND_SMALL_REV_OK )
		return( FALSE );
...
/*	LOOP_SMALL_REV_ALT( i = 5, i >= 0, i-- )
		{
		ENSURES( LOOP_INVARIANT_SMALL_ALT( i, 0, 5 ) );
		}
	if( !LOOP_BOUND_SMALL_REV_OK_ALT )
		return( FALSE );
	LOOP_SMALL_REV_ALT( i = 20, i >= 0, i-- )
		{
		ENSURES( LOOP_INVARIANT_REV_ALT( i, 0, 20 ) );
		}
	if( LOOP_BOUND_SMALL_REV_OK_ALT )
	...
	if( !CFI_CHECK_SEQUENCE_8( "BOOLEAN", "bitflags", "pointers", "TMR", 
							   "Overflow", "CFI", "loops", "buffers" ) )
		return( FALSE );
	if( CFI_CHECK_SEQUENCE_8( "BOOLEAN", "pointers", "bitflags", "TMR",
							  "Overflow", "CFI", "loops", "buffers" ) )
		return( FALSE );
	if( CFI_CHECK_SEQUENCE_8( "BOOLEAN", "bitflags", "loops", "buffers", 
							  "pointers", "TMR", "Overflow", "CFI" ) )
		return( FALSE );
...
/* Self-test code for several general crypto algorithms that are used
   internally all over cryptlib: MD5 (if enabled for TLS 1.0 and 1.1), 
   SHA-1, SHA-2, 3DES (if enabled), and AES */
...
int testFunctionality( void )
	{
	if( !testSafetyMechanisms() )
		{
		DEBUG_DIAG(( "Safety mechanism self-test failed" ));
		return( CRYPT_ERROR_FAILED );
		}
#ifndef CONFIG_NO_SELFTEST
	if( !testGeneralAlgorithms() )
		{
		DEBUG_DIAG(( "Essential algorithms self-test failed" ));
		return( CRYPT_ERROR_FAILED );
		}
#endif /* CONFIG_NO_SELFTEST */
	if( !testInternalFunctions() )
		{
		DEBUG_DIAG(( "Internal function self-test failed" ));
		return( CRYPT_ERROR_FAILED );
		}
...
int testKernel( void )
	...
	if( !testKernelMechanisms() )
		{
		DEBUG_DIAG(( "Kernel mechanism self-test failed" ));
		return( CRYPT_ERROR_FAILED );
		}
