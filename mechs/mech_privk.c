...
static int pgpReadDecryptMPI( INOUT_PTR STREAM *stream,
	...
	/* Get the MPI length and decrypt the payload data.  We have to be 
	   ...
	   Note that we use BIGNUM_CHECK_VALUE rather than 
	   BIGNUM_CHECK_VALUE_PKC because these are private-key components, not
	   the full-size public key values */
	...
	if( cryptStatusError( status ) )
		return( status );
	REQUIRES( isIntegerRangeNZ( mpiLength ) );
...
static int checkKeyIntegrity( IN_BUFFER( dataLength ) const void *data, 
	...
	/* Get the length of the encapsulated ASN.1 object.  Alongside using the
	   ...
	   basic sanity check on the returned length in case invalid decrypted 
	   data looks like a valid ASN.1 object, we need to have enough data 
	   for at least an ECC private key and can't have more than the total 
	   size minus one byte of padding */
	...
	if( length < ( MIN_PKCSIZE_ECC / 2 ) || length >= dataLength || \
		( dataLength - length ) > CRYPT_MAX_IVSIZE )
...
static int checkPgp2KeyIntegrity( IN_BUFFER( dataLength ) const void *data, 
	...
	REQUIRES( isShortIntegerRangeMin( dataLength, 16 ) );
	REQUIRES( isShortIntegerRangeMin( keyDataLength, 16 ) && \
...
typedef enum { PRIVATEKEY_WRAP_NONE, PRIVATEKEY_WRAP_NORMAL, 
			   PRIVATEKEY_WRAP_EXTENDED, PRIVATEKEY_WRAP_OLD, 
			   PRIVATEKEY_WRAP_LAST } PRIVATEKEY_WRAP_TYPE;

CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int privateKeyWrap( INOUT_PTR MECHANISM_WRAP_INFO *mechanismInfo,
						   IN_ENUM( PRIVATEKEY_WRAP ) \
							const PRIVATEKEY_WRAP_TYPE type )
	{
	const KEYFORMAT_TYPE formatType = \
			( type == PRIVATEKEY_WRAP_NORMAL ) ? KEYFORMAT_PRIVATE : \
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int privateKeyUnwrap( INOUT_PTR MECHANISM_WRAP_INFO *mechanismInfo,
							 IN_ENUM( PRIVATEKEY_WRAP ) \
								const PRIVATEKEY_WRAP_TYPE type )
	{
	const KEYFORMAT_TYPE formatType = \
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int privateKeyUnwrapPGP( INOUT_PTR MECHANISM_WRAP_INFO *mechanismInfo,
