...
BOOLEAN sanityCheckHttpDataInfo( const HTTP_DATA_INFO *httpDataInfo )
	...
	if( httpDataInfo->buffer == NULL )
		...
		{
		/* We can't check for httpDataInfo->bytesToWrite > 0 because this may
		   be a read */
		if( !isBufsizeRangeMin( httpDataInfo->bufSize, MIN_LINEBUF_SIZE ) || \
...
const HTTP_STATUS_INFO *getHTTPStatusInfo( IN_INT const int httpStatus )
	...
	/* Find the HTTP error string that corresponds to the HTTP status
	   value */
	LOOP_LARGE( i = 0, 
				i < FAILSAFE_ARRAYSIZE( httpStatusInfo, \
										HTTP_STATUS_INFO ) && \
					httpStatusInfo[ i ].httpStatus > 0 && \
					httpStatusInfo[ i ].httpStatus != httpStatus,
			  i++ )
		{
		ENSURES_N( LOOP_INVARIANT_LARGE( i, 0, 
										 FAILSAFE_ARRAYSIZE( httpStatusInfo, \
															 HTTP_STATUS_INFO ) - 1 ) );
		}
...
int retTextLineError( INOUT_PTR STREAM *stream, 
	...
	REQUIRES( isBooleanValue( isTextLineError ) );
#ifdef USE_ERRMSGS
	REQUIRES( netStream != NULL && sanityCheckNetStream( netStream ) );
#endif /* USE_ERRMSGS */
