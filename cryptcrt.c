	...
	switch( compareType )
		...
		case MESSAGE_COMPARE_ISSUERANDSERIALNUMBER:
			...
			if( !isShortIntegerRangeMin( length, 16 ) || \
				!isShortIntegerRangeMin( dataLength, 16 ) )
				return( FALSE );
			...
			if( cryptStatusOK( status ) )
				...
				ENSURES_B( isShortIntegerRangeNZ( dataLeft ) );
...
static int checkCertUsage( INOUT_PTR CERT_INFO *certInfoPtr, 
	...
	if( certInfoPtr->type == CRYPT_CERTTYPE_CERTREQUEST || \
		...
		if( checkType == MESSAGE_CHECK_PKC_SIGCHECK || \
			...
			return( CRYPT_OK );
		setObjectErrorInfo( certInfoPtr, CRYPT_CERTINFO_TRUSTED_USAGE, 
...
static int exportCertData( CERT_INFO *certInfoPtr, 
	...
	REQUIRES( ( certData == NULL && certDataMaxLength == 0 ) || \
			  ( certData != NULL && \
				isBufsizeRangeNZ( certDataMaxLength ) ) );
...
int iCryptImportCertIndirect( OUT_HANDLE_OPT CRYPT_CERTIFICATE *iCertificate,
	...
	*iCertificate = CRYPT_ERROR;
	...
	return( assembleCertChain( iCertificate, iCertSource, keyIDtype, 
							   keyID, keyIDlength, options, errorInfo ) );
	}
...
int iCryptVerifyID( IN_HANDLE const CRYPT_CERTIFICATE iCertificate,
	...
	DEBUG_DIAG_ERRMSG(( "Certificate fetched for given %s doesn't actually "
...
static int processCertAttribute( INOUT_PTR CERT_INFO *certInfoPtr,
								 IN_MESSAGE const MESSAGE_TYPE message,
								 INOUT_PTR void *messageDataPtr, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int certificateMessageFunction( INOUT_PTR TYPECAST( CERT_INFO * ) \
										void *objectInfoPtr,
									   IN_MESSAGE const MESSAGE_TYPE message,
									   void *messageDataPtr,
		...
		if( cryptStatusError( status ) )
			...
			if( cryptStatusError( status ) )
				...
				if( certInfoPtr->type != CRYPT_CERTTYPE_REQUEST_CERT )
					...
					retExt( CRYPT_ARGERROR_VALUE,
							( CRYPT_ARGERROR_VALUE, CERTIFICATE_ERRINFO,
							  "Non-signature-capable keys can only be used "
							  "to authenticate CRMF requests" ) );
					}
				status = krnlSendMessage( messageValue, IMESSAGE_CHECK, NULL,
				...
				if( cryptStatusError( status ) )
					...
					retExt( CRYPT_ARGERROR_VALUE,
							( CRYPT_ARGERROR_VALUE, CERTIFICATE_ERRINFO,
							  "Signing key can't be used to authenticate a "
							  "CRMF request" ) );
					}
				}
			}
...
int createCertificateIndirect( INOUT_PTR MESSAGE_CREATEOBJECT_INFO *createInfo,
	...
	REQUIRES( ( createInfo->arg2 == 0 && createInfo->strArg2 == NULL && \
			  ...
			  ( ( createInfo->arg2 == CRYPT_IKEYID_KEYID || \
				...
				isShortIntegerRangeMin( createInfo->strArgLen2, 3 ) ) );
	...
	status = importCert( createInfo->strArg1, createInfo->strArgLen1,
						 ...
						 createInfo->arg1, createInfo->errorInfo );
	if( cryptStatusError( status ) )
		{
		...
		DEBUG_DIAG_ERRMSG(( "Certificate import failed, status %s, error "
							"string:\n  %s", getStatusName( status ),
							getErrorInfoString( createInfo->errorInfo ) ));
		return( status );
		}
	createInfo->cryptHandle = iCertificate;
...
C_RET cryptGetCertExtension( C_IN CRYPT_CERTIFICATE certificate,
...
#endif /* EBCDIC_CHARS */
	int oidLen, binaryOidLen, value, dataLength, status;
	...
	*extensionLength = 0;
	...
	if( cryptStatusError( textToOID( asciiOID, 
		...
		return( CRYPT_ERROR_PARAM2 );
#else
	if( cryptStatusError( textToOID( oid, oidLen, binaryOID, MAX_OID_SIZE, 
									 &binaryOidLen ) ) )
		return( CRYPT_ERROR_PARAM2 );
#endif /* EBCDIC_CHARS */
...
C_RET cryptAddCertExtension( C_IN CRYPT_CERTIFICATE certificate,
	...
	char asciiOID[ CRYPT_MAX_TEXTSIZE + 1 + 8 ];
#endif /* EBCDIC_CHARS */
	int oidLen, binaryOidLen, value, status;
	...
	if( status == CRYPT_ERROR_INITED )
		...
		setObjectErrorInfo( certInfoPtr, CRYPT_ATTRIBUTE_NONE,
...
/* Functions to deal with pseudo-certificates, basic certificate-like 
   ...
   and as targets for kernel messages for protocols like TLS and CMS/PKCS #7 
   that require the use of certificate objects */
