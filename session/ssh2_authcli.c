   ...
   peculiarities) is:
	...
	processClientAuth:
		...
		if( required == further auth )
			send pubkey auth if available;
		return( processAuthFailure() ); */

#ifdef USE_SSH

/* Tables mapping SSH user-authentication mechanism names to sort-of 
   algorithm IDs.  Doing it this way is necessary because readAlgoString() 
   is set up to read algorithm names (which is what's usually communicated), 
   not mechanisms.  In this case the algorithm is set to CRYPT_ALGO_NONE and
   the information we want is communicated via the subAlgrithm.
   
   There are two mapping tables, one that favours password-based 
   authentication and one that favours pubkey-based authentication, 
   depending on whether the user has specified a password or a PKC as their 
   authentication choice.  This is required in order to handle SSH's weird 
   way of reporting authentication failures, see the comment in 
   reportAuthFailure() for details */

static const ALGO_STRING_INFO algoStringUserauthentPWTbl[] = {
	{ "password", 8, CRYPT_ALGO_NONE, SSH_AUTHTYPE_PASSWORD },
	{ "keyboard-interactive", 20, CRYPT_ALGO_NONE, SSH_AUTHTYPE_PAM },
	{ "publickey", 9, CRYPT_ALGO_NONE, SSH_AUTHTYPE_PUBKEY },
	{ NULL, 0, CRYPT_ALGO_NONE, SSH_AUTHTYPE_NONE }, 
		{ NULL, 0, CRYPT_ALGO_NONE, SSH_AUTHTYPE_NONE }
	};
static const ALGO_STRING_INFO algoStringUserauthentPubkeyTbl[] = {
	{ "publickey", 9, CRYPT_ALGO_NONE, SSH_AUTHTYPE_PUBKEY },
	{ "password", 8, CRYPT_ALGO_NONE, SSH_AUTHTYPE_PASSWORD },
	{ "keyboard-interactive", 20, CRYPT_ALGO_NONE, SSH_AUTHTYPE_PAM },
	{ NULL, 0, CRYPT_ALGO_NONE, SSH_AUTHTYPE_NONE }, 
...
static int readAuthFailureInfo( INOUT_PTR SESSION_INFO *sessionInfoPtr,
								IN_LENGTH_SHORT const int length,
								OUT_ENUM_OPT( SSH_AUTHTYPE ) \
									SSH_AUTHTYPE_TYPE *authType,
								OUT_BOOL BOOLEAN *furtherAuthRequired,
								IN_BOOL const BOOLEAN usedPasswordAuth )
	{
	SSH_AUTHTYPE_TYPE requiredAuthType DUMMY_INIT;
	STREAM stream;
	...
	*furtherAuthRequired = FALSE;
	...
	if( usedPasswordAuth )
		...
		status = readAlgoString( &stream, algoStringUserauthentPWTbl,
								 FAILSAFE_ARRAYSIZE( algoStringUserauthentPWTbl, \
													 ALGO_STRING_INFO ),
								 &algoParam, FALSE, SESSION_ERRINFO );
		}
	else
		{
		status = readAlgoString( &stream, algoStringUserauthentPubkeyTbl,
								 FAILSAFE_ARRAYSIZE( algoStringUserauthentPubkeyTbl, \
													 ALGO_STRING_INFO ),
								 &algoParam, FALSE, SESSION_ERRINFO );
		}
	if( cryptStatusOK( status ) )
		...
		requiredAuthType = algoParam;	/* int vs. enum */
	...
	if( cryptStatusOK( status ) && partialSuccess )
		...
		if( usedPasswordAuth )
			{
			/* Password auth, use the selection that prefers pubkey auth */
			status = readAlgoString( &stream, algoStringUserauthentPubkeyTbl,
									 FAILSAFE_ARRAYSIZE( algoStringUserauthentPubkeyTbl, \
														 ALGO_STRING_INFO ),
									 &algoParam, FALSE, SESSION_ERRINFO );
			...
			{
			/* Pubkey auth, use the selection that prefers password auth */
			status = readAlgoString( &stream, algoStringUserauthentPWTbl,
									 FAILSAFE_ARRAYSIZE( algoStringUserauthentPWTbl, \
														 ALGO_STRING_INFO ),
									 &algoParam, FALSE, SESSION_ERRINFO );
			}
		if( cryptStatusOK( status ) && requiredAuthType != algoParam )
			requiredAuthType = SSH_AUTHTYPE_AMBIGUOUS;
		}
...
static int reportAuthFailure( INOUT_PTR SESSION_INFO *sessionInfoPtr,
							  IN_ENUM( SSH_AUTHTYPE ) \
									const SSH_AUTHTYPE_TYPE providedAuthType,
							  IN_ENUM( SSH_AUTHTYPE ) \
									const SSH_AUTHTYPE_TYPE requiredAuthType,
							  IN_BOOL const BOOLEAN partialAuthOK )
	{
	const BOOLEAN needsPW = \
			( requiredAuthType == SSH_AUTHTYPE_PASSWORD || \
			  requiredAuthType == SSH_AUTHTYPE_PAM ) ? TRUE : FALSE;

	assert( isWritePtr( sessionInfoPtr, sizeof( SESSION_INFO ) ) );

	REQUIRES( isEnumRange( providedAuthType, SSH_AUTHTYPE ) );
	REQUIRES( isEnumRange( requiredAuthType, SSH_AUTHTYPE ) );
	...
	if( requiredAuthType == SSH_AUTHTYPE_AMBIGUOUS )
		{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int processAuthFailure( INOUT_PTR SESSION_INFO *sessionInfoPtr,
							   IN_LENGTH_SHORT const int length,
							   IN_ENUM( SSH_AUTHTYPE ) \
									const SSH_AUTHTYPE_TYPE providedAuthType,
							   IN_BOOL const BOOLEAN partialAuthOK )
	...
	return( reportAuthFailure( sessionInfoPtr, providedAuthType, 
							   requiredAuthType, partialAuthOK ) );
	}
...
static int readAuthResponse( INOUT_PTR SESSION_INFO *sessionInfoPtr,
	...
	REQUIRES( isBooleanValue( isPAMAuth ) );
...
static int sendDummyAuth( INOUT_PTR SESSION_INFO *sessionInfoPtr,
...
static int pamAuthenticate( INOUT_PTR SESSION_INFO *sessionInfoPtr,
	...
	LOOP_MED( i = 0, i < noPrompts && cryptStatusOK( status ), i++ )
		{
		ENSURES( LOOP_INVARIANT_MED( i, 0, noPrompts - 1 ) );
...
static int processPamAuthentication( INOUT_PTR SESSION_INFO *sessionInfoPtr )
	...
	LOOP_SMALL( pamIteration = 0, pamIteration < 5, pamIteration++ )
		...
		ENSURES( LOOP_INVARIANT_SMALL( pamIteration, 0, 4 ) );
...
int processClientAuth( INOUT_PTR SESSION_INFO *sessionInfoPtr,
	...
	/* The way in which we handle authentication here isn't totally
	   ...
	   password and a private key and the server only accepts pubkey-based
	...
	/* If we tried straight password authentication and the server requested 
	   ...
	   PAM authentication */
	if( hasPassword && requiredAuthType == SSH_AUTHTYPE_PAM )
	...
	if( !needFurtherAuth )
		...
		return( reportAuthFailure( sessionInfoPtr, 
								   hasPassword ? SSH_AUTHTYPE_PASSWORD : \
												 SSH_AUTHTYPE_PUBKEY, 
								   requiredAuthType, FALSE ) );
		}
	...
	if( sessionInfoPtr->privateKey == CRYPT_ERROR || \
		!( requiredAuthType == SSH_AUTHTYPE_PUBKEY || \
		   requiredAuthType == SSH_AUTHTYPE_AMBIGUOUS ) )
		{
	...
	/* The additional authentication failed, provide more specific details 
	   for the caller */
	return( processAuthFailure( sessionInfoPtr, length, SSH_AUTHTYPE_PUBKEY, 
								TRUE ) );
	}
#endif /* USE_SSH */
