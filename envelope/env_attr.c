...
static int exitError( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	setObjectErrorInfo( envelopeInfoPtr, errorLocus, errorType );
	return( status );
	}

CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int exitErrorInited( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
							IN_ATTRIBUTE const CRYPT_ATTRIBUTE_TYPE errorLocus )
	{
	assert( isWritePtr( envelopeInfoPtr, sizeof( ENVELOPE_INFO ) ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int exitErrorNotInited( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
							   IN_ATTRIBUTE const CRYPT_ATTRIBUTE_TYPE errorLocus )
	{
	assert( isWritePtr( envelopeInfoPtr, sizeof( ENVELOPE_INFO ) ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int exitErrorNotFound( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
							  IN_ATTRIBUTE const CRYPT_ATTRIBUTE_TYPE errorLocus )
	{
	assert( isWritePtr( envelopeInfoPtr, sizeof( ENVELOPE_INFO ) ) );
...
static BOOLEAN moveVirtualCursor( INOUT_PTR CONTENT_LIST *contentListPtr,
	...
	LOOP_SMALL_INITCHECK( doContinue = TRUE, doContinue == TRUE )
		...
		LOOP_SMALL_ALT( i = 0, 
						...
						i++ )
			{
			ENSURES_B( LOOP_INVARIANT_SMALL_ALT( i, 0, 
...
CHECK_RETVAL_PTR \
static const void *getAttrFunction( IN_PTR_OPT TYPECAST( CONTENT_LIST * ) \
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int setCursorSelection( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
							   IN_ATTRIBUTE \
	...
	assert( isWritePtr( envelopeInfoPtr, sizeof( ENVELOPE_INFO ) ) );
	assert( contentListCurrent == NULL || \
			isReadPtr( contentListCurrent, sizeof( CONTENT_LIST ) ) );
...
static int instantiateCertChain( INOUT_PTR CONTENT_LIST *contentListItem, 
	...
	REQUIRES( isShortIntegerRangeMin( certChainDataLength, 
	...
	if( cryptStatusError( status ) )
		...
		retExtErr( status,
				   ( status, errorInfo, &localErrorInfo,
					 "Couldn't import certificate chain from signed data" ) );
 		}
...
static int getCurrentAttributeInfo( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	assert( isReadPtr( contentListItem, sizeof( CONTENT_LIST ) ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int getSignatureResult( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	assert( contentListItem == NULL || \
			isReadPtr( contentListItem, sizeof( CONTENT_LIST ) ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int getSignatureKey( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
...
static int checkOtherAttribute( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	*checkType = MESSAGE_CHECK_NONE;
	...
	switch( attribute )
		...
		case CRYPT_OPTION_ENCR_HASHPARAM:
			/* This has already been checked by the kernel */
			ENSURES( value == bitsToBytes( 256 ) || \
					 value == bitsToBytes( 384 ) || \
					 value == bitsToBytes( 512 ) );
			return( CRYPT_OK );
...
int getEnvelopeAttribute( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	*valuePtr = 0;
	...
	switch( attribute )
		...
		default:
			REQUIRES( attribute == CRYPT_ENVINFO_COMPRESSION || \
					  ...
					  attribute == CRYPT_IATTRIBUTE_ENVFORMAT || \
					  attribute == CRYPT_IATTRIBUTE_ENVUSAGE );
		}
	...
	switch( attribute )
		...
		case CRYPT_OPTION_ENCR_HASHPARAM:
			if( envelopeInfoPtr->defaultHashParam == 0 )
				{
				return( exitErrorNotInited( envelopeInfoPtr, 
											CRYPT_OPTION_ENCR_HASHPARAM ) );
				}
			*valuePtr = envelopeInfoPtr->defaultHashParam;
			return( CRYPT_OK );
		...
		case CRYPT_IATTRIBUTE_ENVFORMAT:
			*valuePtr = envelopeInfoPtr->type;
			return( CRYPT_OK );

		case CRYPT_IATTRIBUTE_ENVUSAGE:
			/* Return an indication of the envelope's usage, signing, 
			   encryption, or plain data (other usage types aren't needed 
			   externally) */
			*valuePtr = ( envelopeInfoPtr->usage == ACTION_SIGN ) ? \
						  CRYPT_CONTENT_SIGNEDDATA : \
						( envelopeInfoPtr->usage == ACTION_CRYPT ) ? \
						  CRYPT_CONTENT_ENVELOPEDDATA : \
						( envelopeInfoPtr->usage == ACTION_NONE ) ? \
						  CRYPT_CONTENT_DATA : CRYPT_CONTENT_NONE;
			return( CRYPT_OK );
		}
