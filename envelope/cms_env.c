...
/* Set the algorithm ID parameters from a hash/MAC context */

CHECK_RETVAL STDC_NONNULL_ARG( ( 2 ) ) \
static int setAlgoIDparams( const CRYPT_CONTEXT iCryptContext,
							OUT_PTR ALGOID_PARAMS *algoIDparams )
	{
	int hashAlgorithm, hashSize DUMMY_INIT, status;

	assert( isWritePtr( algoIDparams, sizeof( ALGOID_PARAMS ) ) );

	REQUIRES( isHandleRangeValid( iCryptContext ) );

	/* The extended hash algorithms can have various different hash sizes, 
	   to get the exact variant that's being used we have to query the block 
	   size */
	...
	if( cryptStatusError( status ) )
		return( status );
	ENSURES( isHashAlgo( hashAlgorithm ) || isMacAlgo( hashAlgorithm ) );
...
static const BYTE *getContentOID( IN_ENUM( CRYPT_CONTENT ) \
	...
	LOOP_MED( i = 0, 
		...
		ENSURES_N( LOOP_INVARIANT_MED( i, 0, 
									   FAILSAFE_ARRAYSIZE( contentOIDs, \
														   OID_INFO ) - 1 ) );
...
static int copyFromAuxBuffer( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int writeEOCs( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeSignedDataHeader( INOUT_PTR STREAM *stream,
	...
	LOOP_MED( actionListPtr = DATAPTR_GET( envelopeInfoPtr->actionList ), 
		...
		{
		int actionSize DUMMY_INIT;

		REQUIRES( sanityCheckActionList( actionListPtr ) );

		ENSURES( LOOP_INVARIANT_MED_GENERIC() );

		status = setAlgoIDparams( actionListPtr->iCryptHandle, 
								  &algoIDparams );
		if( cryptStatusOK( status ) )
			{
			status = actionSize = \
	...
	/* Write the SignedData header, version number, and SET OF DigestInfo */
	...
	LOOP_MED( actionListPtr = DATAPTR_GET( envelopeInfoPtr->actionList ), 
		...
		if( cryptStatusOK( status ) )
			{
			status = writeContextAlgoIDex( stream, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeEncryptedContentHeader( INOUT_PTR STREAM *stream,
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeEncryptionHeader( INOUT_PTR STREAM *stream, 
	...
	REQUIRES( matchOID( oid, oidLength, OID_CMS_AUTHDATA ) || \
			  blockedPayloadSize == CRYPT_UNUSED || \
			  isIntegerRangeMin( blockedPayloadSize, MIN_IVSIZE ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeEncryptedDataHeader( INOUT_PTR STREAM *stream,
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeEnvelopedDataHeader( INOUT_PTR STREAM *stream,
									 const ENVELOPE_INFO *envelopeInfoPtr )
	{
	const BYTE *contentOID = getContentOID( envelopeInfoPtr->contentType );
	int blockedPayloadSize, encrContentInfoSize, status;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeAuthenticatedDataHeader( INOUT_PTR STREAM *stream,
							const ENVELOPE_INFO *envelopeInfoPtr )
	{
	const ACTION_LIST *actionListPtr = \
	...
	const BYTE *contentOID = getContentOID( envelopeInfoPtr->contentType );
	ALGOID_PARAMS algoIDparams;
	int macActionSize DUMMY_INIT, status;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeAuthEncDataHeader( INOUT_PTR STREAM *stream,
								   const ENVELOPE_INFO *envelopeInfoPtr )
	{
	...
	/* Authenticated encryption derives the encryption and MAC keys from the
	   generic-secret value, so we have to send the information needed to 
	   recreate them to the generic-secret context */
	actionListPtr = findAction( envelopeInfoPtr, ACTION_xxx ); 
	REQUIRES( actionListPtr != NULL && \
	...
	REQUIRES( actionListPtr != NULL && \
			  sanityCheckActionList( actionListPtr ) );
	iMacContext = actionListPtr->iCryptHandle;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeCompressedDataHeader( INOUT_PTR STREAM *stream,
...
static int writeEnvelopeHeader( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
	const ACTION_LIST *actionListPtr;
	STREAM stream;
	...
	switch( envelopeInfoPtr->usage )
		...
		case ACTION_HASH:
			/* We don't do DigestedData so should never get here */
			retIntError();
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeKeyex( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
...
/* Write the signing certificate chain.  This can grow arbitrarily large and 
   in particular can become larger than the main envelope buffer for the
   (admittedly highly unlikely) situation where multiple signatures with long 
   chains and a small envelope buffer are used, so we emit the certificate 
   chain into a dynamically-allocated auxiliary buffer if there isn't enough 
   room to emit it into the main buffer  */

CHECK_RETVAL_SPECIAL STDC_NONNULL_ARG( ( 1 ) ) \
static int writeCertchainTrailer( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
	STREAM stream;
	...
	ENSURES( DATAPTR_ISSET( envelopeInfoPtr->lastAction ) );
	...
	if( cryptStatusOK( status ) )
		status = certChainSize = stell( &stream );
	sMemDisconnect( &stream );
	if( cryptStatusError( status ) )
		return( status );
	ENSURES( isBufsizeRangeNZ( certChainSize ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int writeSignatures( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
	LOOP_INDEX_PTR ACTION_LIST *actionListPtr;
	ERROR_INFO localErrorInfo;
	int noSigs = 0, status = CRYPT_OK;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int writeMAC( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
	const ACTION_LIST *actionListPtr;
	STREAM stream;
	MESSAGE_DATA msgData;
	BYTE hash[ CRYPT_MAX_HASHSIZE + 8 ];
	const int noEOCs = ( envelopeInfoPtr->usage == ACTION_CRYPT ) ? 2 : 3;
	const int eocSize = ( envelopeInfoPtr->payloadSize == CRYPT_UNUSED ) ? \
						( noEOCs * sizeofEOC() ) : 0;
	...
	if( envelopeInfoPtr->payloadSize == CRYPT_UNUSED )
		{
		status = writeEOCs( envelopeInfoPtr, noEOCs );
		if( cryptStatusError( status ) )
			return( status );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int emitPreamble( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
	int status = CRYPT_OK;
	...
	if( envelopeInfoPtr->envState == ENVSTATE_KEYINFO )
		...
		clearErrorInfo( &localErrorInfo );
		status = writeKeyex( envelopeInfoPtr, &localErrorInfo );
		if( cryptStatusError( status ) )
			{
			retExtErr( status,
					   ( status, ENVELOPE_ERRINFO, &localErrorInfo,
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int emitPostamble( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr,
	...
	/* Write the end-of-contents octets for the OCTET STRING/SEQUENCE, [0],
	   and SEQUENCE if necessary.  Alongside the obvious indefinite-length 
	   payload this can also be required if the SET OF RecipientInfo was 
	   written with an indefinite length, which makes the enclosing header 
	   indefinite-length as well.
	   
	   If the trailer has an indefinite length then we need to add an EOC 
...
STDC_NONNULL_ARG( ( 1 ) ) \
void initCMSEnveloping( INOUT_PTR ENVELOPE_INFO *envelopeInfoPtr )
	{
	int value, status;
	...
	if( cryptStatusError( status ) || !checkAlgoID( value, 0 ) )
		{
		/* The overall algorithm isn't encodable, switch to SHA-2 */
		...
		if( cryptStatusError( status ) || \
			!checkAlgoID( envelopeInfoPtr->defaultHash, value ) )
			{
			/* The algorithm is encodable but the parameter isn't,
			   switch to (implied SHA-2)-256 */
			envelopeInfoPtr->defaultHashParam = bitsToBytes( 256 );
			}
		else
	...
	if( cryptStatusError( status ) || \
		!checkAlgoID( value, ( value == CRYPT_ALGO_RC4 ) ? \
	...
	if( cryptStatusError( status ) || \
		...
		envelopeInfoPtr->defaultMAC = value;	/* int vs.enum */
	}
#endif /* USE_CMS */
