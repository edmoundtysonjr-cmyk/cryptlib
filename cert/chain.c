...
/* Assemble a single certificate chain from a collection of certificates,
   consisting of the components in the chain.  This code implements the 
   principle of least surprise and explicitly constructs a single 
   unambiguous chain rather than trying to handle the spaghetti PKI mess,
   for the specific reason that non-web use can't tolerate surprises where 
   some other path through the spaghetti suddenly trusts a certificate that 
   no-one is expecting to be trusted.  In particular with the web PKI the
   endless churn of cross-certification and de-certification means that 
   things can pop in and out of trust more or less at random, while we want
   to build a chain that the caller is expecting to be trusted, not that's
   inadvertently trusted via some cross-certification that they've never 
   heard of before */
...
/* When matching by subjectKeyIdentifier we don't use values less than 40
   ...
   This is a rather weak identifier but not a security issue, it just means
   that we'll fail with a signature-verification error rather than a not-
   found error */
...
/* Match an identifier in a chaining-information structure */
...
static void getSubjectChainingInfo( OUT_PTR CHAINING_INFO *chainingInfo,
...
STDC_NONNULL_ARG( ( 1, 2 ) ) \
static void getIssuerChainingInfo( OUT_PTR CHAINING_INFO *chainingInfo,
								   const CHAIN_INFO *chainInfo )
	{
	assert( isWritePtr( chainingInfo, sizeof( CHAINING_INFO ) ) );
	assert( isReadPtr( chainInfo, sizeof( CHAIN_INFO ) ) );
...
/* Determine whether a given certificate is the subject or issuer for the 
   ...
   has this as a fallback (RFC 3280 section 4.2.1.1, "the keyIdentifier 
   ...
   path construction").  This is checked later in
   cert/chk_cert.c:checkNameChaining() which may enforce strict name 
   chaining depending on how the compliance level is set.
   ...
   Since the latter is rather unlikely to occur in a standard PKCS #7/TLS 
   ...
   but enable the latter if preferStrictChaining is set.
   
   If preferStrictChaining is enabled we require that the DN *and* the keyID 
...
static BOOLEAN isSubject( IN_PTR const CHAINING_INFO *chainingInfo,
						  IN_PTR const CHAIN_INFO *chainInfo,
						  IN_BOOL const BOOLEAN preferStrictChaining )
	{
	...
	REQUIRES_B( isBooleanValue( preferStrictChaining ) );
...
CHECK_RETVAL_BOOL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static BOOLEAN isIssuer( IN_PTR const CHAINING_INFO *chainingInfo,
						 IN_PTR const CHAIN_INFO *chainInfo )
	{
	assert( isReadPtr( chainingInfo, sizeof( CHAINING_INFO ) ) );
	assert( isReadPtr( chainInfo, sizeof( CHAIN_INFO ) ) );
...
#ifdef USE_CERTLEVEL_PKIX_PARTIAL
	/* If that fails then we chain by keyID */
...
static int getChainingAttribute( INOUT_PTR CERT_INFO *certInfoPtr,
								 IN_ATTRIBUTE \
	...
	REQUIRES( attributeType == CRYPT_CERTINFO_SUBJECTKEYIDENTIFIER || \
...
static void freeCertChain( IN_ARRAY( certChainSize ) \
	...
	LOOP_EXT( i = 0, i < certChainSize, i++, MAX_CHAINLENGTH )
		{
		ENSURES_V( LOOP_INVARIANT_EXT( i, 0, certChainSize - 1,
									   MAX_CHAINLENGTH ) );
		...
		status = krnlSendNotifier( iCertChain[ i ], IMESSAGE_DESTROY );
		assert( cryptStatusOK( status ) );	/* Warn in debug mode */
...
static int convertCertToChain( OUT_HANDLE_OPT CRYPT_CERTIFICATE *iCertChain,
	...
	*iCertChain = CRYPT_ERROR;
	...
	/* Export the leaf certificate and re-import it to create the required 
	   ...
	   certificate chain.  Since this is just a straight export-reimport
	   we don't do anything with the error information since it's both 
	   unlikely to occur and unlikely to prove useful */
...
static int buildChainInfo( OUT_ARRAY( certChainSize ) CHAIN_INFO *chainInfo,
	...
	LOOP_EXT( i = 0, i < certChainSize, i++, MAX_CHAINLENGTH )
		...
		ENSURES( LOOP_INVARIANT_EXT( i, 0, certChainSize - 1, 
									 MAX_CHAINLENGTH ) );
...
CHECK_RETVAL_RANGE( 0, MAX_CHAINLENGTH ) STDC_NONNULL_ARG( ( 1 ) ) \
static int findLeafNode( IN_ARRAY( certChainSize ) const CHAIN_INFO *chainInfo,
	...
	LOOP_INDEX currentCertPos;
	int bestMatch = CRYPT_ERROR, optionalMatch;
	...
	/* Walk down the chain from the currently selected certificate checking 
	   ...
	   optional filtering by certificate usage type.  
	   
	   This algorithm handles chains with PKIX path-kludge certificates as 
	   well as normal ones since isIssuer() matches a self-issued 
	   certificate against itself, which sets certIsIssuer and skips it, 
	   avoiding potential endless loops on subject == issuer path-kludge 
	   certificates */
	LOOP_EXT( currentCertPos = 0, currentCertPos < certChainSize, 
		...
		LOOP_INDEX_ALT childCertPos;
		int value, status;

		ENSURES( LOOP_INVARIANT_EXT( currentCertPos, 0, certChainSize - 1,
									 MAX_CHAINLENGTH ) );
		...
		LOOP_EXT_ALT( childCertPos = 0, childCertPos < certChainSize, 
			...
			{
			ENSURES( LOOP_INVARIANT_EXT_ALT( childCertPos, 0, certChainSize - 1,
											 MAX_CHAINLENGTH ) );
		...
		/* If we're not selecting certificates based on a specific usage 
		   type, we're done */
		if( requestedUsage == CRYPT_KEYUSAGE_NONE )
			{
...
static int sortCertChain( INOUT_ARRAY( certChainSize ) CRYPT_CERTIFICATE *iCertChain,
						  ...
						  IN_RANGE( 1, MAX_CHAINLENGTH ) const int certChainSize,
						  IN_PTR_OPT const CHAINING_INFO *chainingInfo,
	...
	BOOLEAN chainInfoValid[ MAX_CHAINLENGTH + 8 ], moreMatches = TRUE;
	const int maxMatchLevel = preferStrictChaining ? 1 : 0;
	LOOP_INDEX i;
	int orderedChainIndex, status;

	assert( isWritePtrDynamic( iCertChain, \
							   sizeof( CRYPT_CERTIFICATE ) * certChainSize ) );
	assert( isReadPtrDynamic( chainInfo, \
							  sizeof( CHAIN_INFO ) * certChainSize ) );
	assert( chainingInfo == NULL || \
	...
	REQUIRES( isBooleanValue( preferStrictChaining ) );
	...
	*orderedCertChainSize = 0;
	...
	if( leafCert != CRYPT_UNUSED )
		...
		{
		ENSURES( chainingInfo != NULL && leafCertEntry != CRYPT_UNUSED );
	...
	LOOP_MED_WHILE( moreMatches )
		...
		/* Find the certificate with the current issuer as its subject.  If 
		   ...
		   (matchLevel = 1), if that fails we fall back to a standard match 
		   (matchLevel = 0).  This is required to handle the significant 
		   number of CAs that don't get chaining by keyID right */
		REQUIRES( maxMatchLevel == 0 || maxMatchLevel == 1 );
		LOOP_EXT_REV_ALT( matchLevel = maxMatchLevel, \
						  !moreMatches && matchLevel >= 0, matchLevel--, 2 + 1 )
			{
			int LOOP_ITERATOR_ALT2;

			ENSURES( LOOP_INVARIANT_REV_ALT( matchLevel, 0, maxMatchLevel ) );

			LOOP_EXT_ALT2( i = 0, i < certChainSize, i++, MAX_CHAINLENGTH )
				{
				ENSURES( LOOP_INVARIANT_EXT_ALT2( i, 0, certChainSize - 1,
												  MAX_CHAINLENGTH ) );

				/* If this isn't the issuer, continue */
				if( !chainInfoValid[ i ] || \
					!isSubject( &localChainingInfo, &chainInfo[ i ], 
								matchLevel ? TRUE : FALSE ) )
					continue;

				/* We've found the issuer, move the certificate to the 
				   ordered chain and prepare to find its issuer */
				REQUIRES( rangeCheck( orderedChainIndex, 
									  0, MAX_CHAINLENGTH - 1 ) );
				orderedChain[ orderedChainIndex++ ] = iCertChain[ i ];
				getIssuerChainingInfo( &localChainingInfo, &chainInfo[ i ] );
				chainInfoValid[ i ] = FALSE;
				moreMatches = TRUE;	/* Exit outer matchLevel loop as well */
				ENSURES( orderedChainIndex > 0 && \
						 orderedChainIndex <= certChainSize && \
						 orderedChainIndex <= MAX_CHAINLENGTH );
						 /* Post-increment so 1..n, not 0..n-1 */
				break;
				}
			ENSURES( LOOP_BOUND_OK_ALT2 );
			}
		ENSURES( LOOP_BOUND_EXT_REV_OK_ALT( 2 + 1 ) );
		}
	ENSURES( LOOP_BOUND_OK );
	...
	LOOP_EXT( i = 0, i < certChainSize, i++, MAX_CHAINLENGTH )
		...
		if( chainInfoValid[ i ] )
			{
			status = krnlSendNotifier( iCertChain[ i ], 
	...
	LOOP_SMALL( i = 0, i < certChainSize, i++ )
		{
		ENSURES( LOOP_INVARIANT_SMALL( i, 0, certChainSize - 1 ) );
		iCertChain[ i ] = orderedChain[ i ];
		}
	ENSURES( LOOP_BOUND_OK );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int buildCertChain( OUT_HANDLE_OPT CRYPT_CERTIFICATE *iLeafCertPtr, 
						   INOUT_ARRAY( certChainEnd ) \
								CRYPT_CERTIFICATE *iCertChain, 
						   IN_RANGE( 1, MAX_CHAINLENGTH ) \
								const int certChainEnd,
						   OUT_RANGE( 0, MAX_CHAINLENGTH ) \
								int *updatedCertChainEnd,
	...
	{
	CRYPT_CERTIFICATE iLeafCert, savedLeafCert;
	...
	assert( isWritePtr( iLeafCertPtr, sizeof( CRYPT_CERTIFICATE ) ) );
	assert( isWritePtrDynamic( iCertChain, \
							   sizeof( CRYPT_CERTIFICATE ) * certChainEnd ) );
	assert( keyID == NULL || isReadPtrDynamic( keyID, keyIDlength ) );
	...
	/* Clear return value.  The setting for updatedCertChainEnd is a bit 
	   unusual, normally we'd set something like this to 0 or CRYPT_ERROR 
	   but in this case an error return means that we didn't change the
	   iCertChain values so it remains the same as when we were called */
	*iLeafCertPtr = CRYPT_ERROR;
	*updatedCertChainEnd = certChainEnd;

	/* Get the compliance level.  We use the first certificate in the chain 
	   as a proxy for the user object that created the certificate */
	...
	if( newCertChainEnd <= 0 )
		...
		*iLeafCertPtr = iLeafCert;
		*updatedCertChainEnd = 0;
	...
	/* Walk up the chain re-setting the pseudo-selfsigned flag on any
	   ...
	   Note that this has all sorts of other repercussions in checks 
	   elsewhere, for example cert/chk_chain.c:checkConstraints(), which is
	   why we never enable it except in PKIX full-on crazy mode */
	if( complianceLevel >= CRYPT_COMPLIANCELEVEL_PKIX_FULL )
		...
		LOOP_EXT( i = 0, i < newCertChainEnd - 1, i++, MAX_CHAINLENGTH )
			...
			ENSURES( LOOP_INVARIANT_EXT( i, 0, newCertChainEnd - 2,
										 MAX_CHAINLENGTH ) );
	...
	/* Since the objects that make up the chain were imported as individual 
	   ...
	   certificate to certificate chain by recreating it as a certificate-
	   chain object.  This replaces the original iLeafCert certificate object 
	   with a new certificate-chain object.
	   
	   sortCertChain() has converted { unordered-list } to { leafCert, 
	   remaining-ordered-list } so what we're doing here is converting the 
	   leafCert certificate object to a cert-chain object.
	   
	   It's also has detached the leaf certificate from iCertChain which 
	   means that the caller can no longer see it, so we save a copy here 
	   and replace it in the chain if the replacement operation fails so 
	   that the caller can deal with it */
	savedLeafCert = iLeafCert;
	status = convertCertToChain( &iLeafCert, iLeafCert,
								 ( options & KEYMGMT_FLAG_DATAONLY_CERT ) ? \
								   TRUE : FALSE );
	if( cryptStatusError( status ) )
		{
		/* Replace the removed leaf certificate in the chain for the caller 
		   to clean up */
		REQUIRES( rangeCheck( newCertChainEnd, 1, certChainEnd  - 1 ) );
		iCertChain[ newCertChainEnd ] = savedLeafCert;
		return( status );
		}
	...
	if( cryptStatusError( status ) )
		{
		/* Insert the replacement leaf certificate into the chain for the 
		   caller to clean up */
		REQUIRES( rangeCheck( newCertChainEnd, 0, certChainEnd - 1 ) );
		iCertChain[ newCertChainEnd ] = iLeafCert;
		return( status );
		}
	ENSURES_OBJECT( sanityCheckCert( certChainPtr ), 
					certChainPtr->objectHandle );
	REQUIRES_OBJECT( rangeCheck( newCertChainEnd, 1, MAX_CHAINLENGTH ),
					 certChainPtr->objectHandle );
	...
	krnlReleaseObject( certChainPtr->objectHandle );
	*iLeafCertPtr = iLeafCert;
	*updatedCertChainEnd = newCertChainEnd;
...
static BOOLEAN isCertPresent( INOUT_ARRAY( certChainLen ) \
	...
	LOOP_EXT( i = 0, i < certChainLen, i++, MAX_CHAINLENGTH )
		{
		ENSURES_B( LOOP_INVARIANT_EXT( i, 0, certChainLen - 1,
									   MAX_CHAINLENGTH ) );
...
/* Copy a certificate chain into a certificate object and canonicalise the 
   ...
   root.  This function is used when signing a certificate with a 
   certificate chain and takes as input 
   newCert + ( oldCert, oldCert.chain[ ... ] ) and produces as output 
   ( newCert, chain[ oldCert, oldCert.chain[ ... ] ], i.e. the chain for the 
   new certificate contains the old certificate and its attached certificate 
   chain */
...
int copyCertChain( INOUT_PTR CERT_INFO *certInfoPtr, 
	...
	LOOP_INDEX i;
	int srcChainEnd, newChainEnd DUMMY_INIT, status;

	assert( isWritePtr( certInfoPtr, sizeof( CERT_INFO ) ) );

	REQUIRES( sanityCheckCert( certInfoPtr ) );
	REQUIRES( isHandleRangeValid( iContextOrChain ) );
	REQUIRES( isBooleanValue( isCertCollection ) );

	/* Get the certificate chain associated with the object that we've 
	   been passed, which may be a signing context with attached certificate 
	   or a certificate depending on how we've been called */
	status = krnlSendMessage( iContextOrChain, IMESSAGE_GETDEPENDENT, 
							  &iChainCert, OBJECT_TYPE_CERTIFICATE );
	if( cryptStatusError( status ) )
		return( status );

	/* If we're building a certificate collection then all that we need to 
	   ensure is that there are no duplicate certificates rather than having 
	   a strict chain.  To handle duplicate checking we build a list of the 
	   fingerprints for each certificate in the chain */
	LOOP_EXT( i = 0, i < MAX_CHAINLENGTH + 1, i++,
			  MAX_CHAINLENGTH + 2 )
		{
		ENSURES( LOOP_INVARIANT_EXT( i, 0, MAX_CHAINLENGTH,
									 MAX_CHAINLENGTH + 2 ) );

		memset( certChainHashes[ i ], 0, CRYPT_MAX_HASHSIZE );
		}
	ENSURES( LOOP_BOUND_OK );
	if( isCertCollection )
		{
	...
	srcChainEnd = srcCertChainInfo->chainEnd;
	if( !isCertCollection || \
		...
		REQUIRES( rangeCheck( destCertChainInfo->chainEnd, 
							  0, MAX_CHAINLENGTH - 1 ) );
		destCertChainInfo->chain[ destCertChainInfo->chainEnd++ ] = iChainCert;
		ENSURES( rangeCheck( destCertChainInfo->chainEnd, 
							 1, MAX_CHAINLENGTH ) );
		}

	/* Copy over the rest of the chain if present */
	LOOP_EXT( i = 0, i < srcCertChainInfo->chainEnd, i++, MAX_CHAINLENGTH )
		{
		const CRYPT_CERTIFICATE iCopyCert = srcCertChainInfo->chain[ i ];

		ENSURES_OBJECT( LOOP_INVARIANT_EXT( i, 0, \
											srcCertChainInfo->chainEnd - 1,
											MAX_CHAINLENGTH ),
						chainCertInfoPtr->objectHandle );
	...
	/* If the chain being attached consists of a single certificate, which 
	   ...
	   a CA certificate, we're done */
	if( srcChainEnd <= 0 )
		return( CRYPT_OK );

	/* Extract the chaining information from each certificate and use it to 
	   sort the chain.  This is really a no-op because the chain being 
	   attached should already be sorted, but we perform the operation 
	   anyway as a hygiene measure.
	   
	   Since we know what the leaf certificate is and since chaining 
	   information such as the encoded DN data in the certInfo structure may 
	   not have been set up yet if it contains an unsigned certificate we 
	   feed in the leaf certificate and omit the chaining information */
	status = buildChainInfo( chainInfo, destCertChainInfo->chain,
							 destCertChainInfo->chainEnd );
	if( cryptStatusOK( status ) )
		{
		status = sortCertChain( destCertChainInfo->chain, &newChainEnd, 
	...
	destCertChainInfo->chainEnd = newChainEnd;
...
CHECK_RETVAL_SPECIAL STDC_NONNULL_ARG( ( 1, 2, 7 ) ) \
static int readSingleCert( INOUT_PTR STREAM *stream, 
						   OUT_HANDLE_OPT CRYPT_CERTIFICATE *iCryptCert,
						   IN_HANDLE const CRYPT_USER iCryptOwner,
						   IN_ENUM( CRYPT_CERTTYPE ) const CRYPT_CERTTYPE_TYPE type,
						   IN_BOOL const BOOLEAN dataOnlyCert,
						   IN_BOOL const BOOLEAN isIndefiniteLength,
	...
	REQUIRES( type == CRYPT_CERTTYPE_CERTCHAIN || \
			  ...
			  type == CRYPT_ICERTTYPE_CMP_CERTSEQUENCE || \
			  type == CRYPT_ICERTTYPE_TLS_CERTCHAIN || \
			  type == CRYPT_ICERTTYPE_TLS13_CERTCHAIN );
	REQUIRES( isBooleanValue( dataOnlyCert ) );
	REQUIRES( isBooleanValue( isIndefiniteLength ) );

	/* Clear return value */
	*iCryptCert = CRYPT_ERROR;

	/* If it's a TLS certificate chain then there's a 24-bit length field 
	   between certificates.  We can't really do much with this, including 
	   reading it because we don't have TLS code in the X.509 modules, and 
	   the certificate contains its own length data so we go with that */
	if( type == CRYPT_ICERTTYPE_TLS_CERTCHAIN || \
		type == CRYPT_ICERTTYPE_TLS13_CERTCHAIN )
		{
	...
	/* Find out how large the certificate data is and make sure that the 
	   ...
	   functions */
	status = getStreamObjectLength( stream, &length, MIN_CERTSIZE );
	if( cryptStatusError( status ) )
		return( status );
	if( !isShortIntegerRangeMin( length, MIN_CERTSIZE ) )
		return( CRYPT_ERROR_BADDATA );

	/* Since TLS certificate chains contain certificates interspersed with 
	   non-certificate TLS length data the higher-level code can't check the 
	...
	/* TLS 1.3 added TLS-style extensions to the certificate extensions 
	   because there wasn't enough extensibility already present, so we skip
	   any junk tacked onto the end of the certificate */
#ifdef USE_TLS13
	if( type == CRYPT_ICERTTYPE_TLS13_CERTCHAIN )
		{
...
/* Read certificate chain/sequence information.  Returning extended error 
   information from this function isn't possible because there's no 
   certificate object to associate it with.  That is, while assembling the 
   chain we don't know which one is the container for the chain, so we don't 
   know which one to set error information for.  Because of this the best 
   that we can do is DEBUG_DIAG() the error report */
	...
	*iCryptCert = CRYPT_ERROR;
	...
	switch( type )
		...
		case CRYPT_ICERTTYPE_TLS13_CERTCHAIN:
			/* For a standard certificate chain the outer wrapper has already been 
			   read by the caller so the certificate chain is everything that's left.
			   For an TLS certificate chain there's no outer wrapper to give us 
			   length information, however the length will be equal to the total 
		...
		case CRYPT_ICERTTYPE_CMP_CERTSEQUENCE:
			status = readSequence( stream, &certSequenceLength );
			break;
	...
	if( cryptStatusError( status ) )
		...
		retExt( status,
				( status, errorInfo,
				  "Invalid %s wrapper", getCertTypeNameLC( type ) ) );
		}
	if( certSequenceLength != CRYPT_UNUSED && \
		...
		retExt( CRYPT_ERROR_BADDATA,
				( CRYPT_ERROR_BADDATA, errorInfo,
				  "Invalid %s wrapper length %d, should be 1...%d", 
				  getCertTypeNameLC( type ), certSequenceLength,
	...
	if( certSequenceLength != CRYPT_UNUSED )
		{
		endPos = stell( stream );
		REQUIRES( isIntegerRange( endPos ) );
		REQUIRES( !checkOverflowAdd( endPos, certSequenceLength ) );
		endPos += certSequenceLength;
		ENSURES( isIntegerRangeMin( endPos, certSequenceLength ) );
		}

	/* Clear the certificate chain information as a general hygiene 
	   measure */
	LOOP_SMALL( i = 0, i < MAX_CHAINLENGTH, i++ )
		{
		ENSURES( LOOP_INVARIANT_SMALL( i, 0, MAX_CHAINLENGTH - 1 ) );
	...
	/* We've reached the certificate(s), read the collection of certificates 
	   ...
	   that gets the length encoding wrong by a few bytes.  We have to set 
	   the initial status to CRYPT_OK because certSequenceLength == 
	   CRYPT_UNUSED bypasses setting the status */
		...
		if( certChainEnd >= MAX_CHAINLENGTH )
			...
			retExt( CRYPT_ERROR_OVERFLOW,
					( CRYPT_ERROR_OVERFLOW, errorInfo,
					  "%s contains more than %d certificates", 
					  getCertTypeName( type ), MAX_CHAINLENGTH ) );
			}
		...
		if( cryptStatusError( status ) && status != OK_SPECIAL )
			...
			retExtErr( status,
					   ( status, errorInfo, &localErrorInfo,
					     "Invalid %s entry #%d", 
						 getCertTypeNameLC( type ), certChainEnd ) );
			}
		REQUIRES( rangeCheck( certChainEnd, 0, MAX_CHAINLENGTH - 1 ) );
		iCertChain[ certChainEnd++ ] = iNewCert;
		ENSURES( rangeCheck( certChainEnd, 1, MAX_CHAINLENGTH ) );
	...
	if( certChainEnd <= 0 )
		...
		retExt( CRYPT_ERROR_BADDATA,
				( CRYPT_ERROR_BADDATA, errorInfo,
				  "%s doesn't contain any certificates", 
	...
	status = buildCertChain( iCryptCert, iCertChain, certChainEnd, 
							 &certChainEnd, keyIDtype, keyID, keyIDlength, 
							 options );
	if( cryptStatusError( status ) )
		{
		freeCertChain( iCertChain, certChainEnd );
		retExt( status,
				( status, errorInfo,
				  "Couldn't build %s from individual certificates", 
				  getCertTypeName( type ) ) );
		}
...
int assembleCertChain( OUT_HANDLE_OPT CRYPT_CERTIFICATE *iCertificate,
	...
	int stateInfo = CRYPT_ERROR, certSetSize = 1, status;
	LOOP_INDEX i;
	...
	*iCertificate = CRYPT_ERROR;
	...
	/* Clear the certificate set information as a general hygiene 
	   measure */
	LOOP_SMALL( i = 0, i < MAX_CHAINLENGTH, i++ )
		{
		ENSURES( LOOP_INVARIANT_SMALL( i, 0, MAX_CHAINLENGTH - 1 ) );

		iCertSet[ i ] = CRYPT_ERROR;
		}
	ENSURES( LOOP_BOUND_OK );
	...
	if( cryptStatusError( status ) )
		{
		retExtObj( status,
				   ( status, errorInfo, iCertSource,
					 "Couldn't read initial certificate in chain from data "
					 "source" ) );
		}
	...
	if( !( ( keyIDtype == CRYPT_IKEYID_KEYID || \
		...
		if( cryptStatusError( status ) )
			...
			retExt( status,
					( status, errorInfo, 
					  "Certificate identity information doesn't match the "
					  "ID used to read it from a data source" ) );
			}
		}
	...
	LOOP_MED_WHILE( cryptStatusOK( status ) )
		{
		...
		if( cryptStatusError( status ) )
			break;		/* Error exit */
		if( selfSigned == TRUE )
			break;		/* Root certificate reached */
		...
		if( certSetSize >= MAX_CHAINLENGTH )
			...
			retExt( CRYPT_ERROR_OVERFLOW,
					( CRYPT_ERROR_OVERFLOW, errorInfo, 
					   "Certificate chain contains more than %d "
					   "certificates", MAX_CHAINLENGTH ));
			}
	...
	if( cryptStatusError( status ) )
		...
		retExtObj( status,
				   ( status, errorInfo, iCertSource,
					 "Couldn't read certificate #%d in chain from data "
					 "source", certSetSize ) );
		}

	/* Build the complete chain from the individual certificates */
	status = buildCertChain( &iCertChain, iCertSet, certSetSize,
							 &certSetSize, CRYPT_KEYID_NONE, NULL, 0, 
							 chainOptions );
	if( cryptStatusError( status ) )
		{
		freeCertChain( iCertSet, certSetSize );
...
/* Determine the size of and write a certificate path from a base 
   ...
   alongside the overall length, TLS requires that we return component-by-
   component lengths that need to be inserted as 24-bit (3 byte) values 
   between each certificate, and TLS 1.3 adds an additional 16-bit all-zero
   value after the certificate.  To handle this we take as an optional 
   parameter an array used to store the individual path component lengths
   and then insert the interstitial values when we write the certificates */
...
static int sizeofCertPath( const CERT_INFO *certInfoPtr,
	...
	const int extraLength = \
		( certFormatType == CRYPT_ICERTFORMAT_TLS_CERTCHAIN ) ? 3 : \
		( certFormatType == CRYPT_ICERTFORMAT_TLS13_CERTCHAIN ) ? 5 : 0;
	LOOP_INDEX i;
	int length = 0;
	...
	REQUIRES( ( ( certFormatType == CRYPT_CERTFORMAT_CERTCHAIN || \
				  certFormatType == CRYPT_ICERTFORMAT_CERTSET || \
				  certFormatType == CRYPT_ICERTFORMAT_CERTSEQUENCE ) && \
				  certSizeInfo == NULL ) || \
			  ( ( certFormatType == CRYPT_ICERTFORMAT_TLS_CERTCHAIN || \
				  certFormatType == CRYPT_ICERTFORMAT_TLS13_CERTCHAIN ) && \
				  certSizeInfo != NULL ) );
	...
	*certPathLength = 0;
	...
	if( !TEST_FLAG( certInfoPtr->flags, CERT_FLAG_CERTCOLLECTION ) )
		length = certInfoPtr->certificateSize + extraLength;
	LOOP_EXT( i = 0, i < certChainInfo->chainEnd, i++, MAX_CHAINLENGTH )
		{
		MESSAGE_DATA msgData;
		int status;

		ENSURES( LOOP_INVARIANT_EXT( i, 0, certChainInfo->chainEnd - 1,
									 MAX_CHAINLENGTH ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeCertPath( INOUT_PTR STREAM *stream, 
	...
	if( !TEST_FLAG( certInfoPtr->flags, CERT_FLAG_CERTCOLLECTION ) )
		{
		if( certFormatType == CRYPT_ICERTFORMAT_TLS_CERTCHAIN || \
			certFormatType == CRYPT_ICERTFORMAT_TLS13_CERTCHAIN ) 
			{
			sputc( stream, 0 );		/* 24-bit length */
	...
	LOOP_EXT( i = 0, 
			  i < certChainInfo->chainEnd && cryptStatusOK( status ),
			  i++, MAX_CHAINLENGTH )
		{
		ENSURES( LOOP_INVARIANT_EXT( i, 0, certChainInfo->chainEnd - 1,
									 MAX_CHAINLENGTH ) );
...
int sizeofCertCollection( IN_PTR const CERT_INFO *certInfoPtr,
	...
	/* If it's an TLS certificate chain then we have to pass in a dummy
	   component-length pointer to ensure that it's evaluated as an TLS 
...
int writeCertCollection( INOUT_PTR STREAM *stream, 
	...
	switch( certFormatType )
		...
		case CRYPT_ICERTFORMAT_TLS_CERTCHAIN:
		case CRYPT_ICERTFORMAT_TLS13_CERTCHAIN:
			status = CRYPT_OK;
			break;
