...
static const DN_COMPONENT_INFO certInfoOIDs[] = {
	...
	{ 40, MKDNOID( "\x55\x04\x43" ),	/* communicationsNetwork (2 5 4 67) */
	  ...
	  /* 0x44-0x4A are more PKI-related attributes */
	  /* 0x4B-0x60 are XML, URL, UUID, EPC, and more things starting with U */
	{ 41, MKDNOID( "\x55\x04\x61" ),	/* organizationIdentifier (2 5 4 97) */
	  "oid.2.5.4.97", 12, NULL, 0, CRYPT_MAX_TEXTSIZE, FALSE, FALSE },
	{ 42, MKDNOID( "\x55\x04\x62" ),	/* countryCode3c (2 5 4 98) */
	  "oid.2.5.4.98", 12, NULL, 0, 3, FALSE, FALSE },
	{ 43, MKDNOID( "\x55\x04\x63" ),	/* countryCode3n (2 5 4 99) */
	  "oid.2.5.4.99", 12, NULL, 0, 3, FALSE, FALSE },
	{ 44, MKDNOID( "\x55\x04\x64" ),	/* dnsName (2 5 4 100) */
	  "oid.2.5.4.100", 12, NULL, 0, CRYPT_MAX_TEXTSIZE, FALSE, FALSE },
	  /* 0x47-0x4C are more PKI-related attributes */
	{ 45, MKOID( "\x06\x0A\x09\x92\x26\x89\x93\xF2\x2C\x64\x01\x01" ),	/* userid (0 9 2342 19200300 100 1 1) */
	  "uid", 3, NULL, 0, CRYPT_MAX_TEXTSIZE, TRUE, FALSE },
	{ 46, MKOID( "\x06\x0A\x09\x92\x26\x89\x93\xF2\x2C\x64\x01\x03" ),	/* rfc822Mailbox (0 9 2342 19200300 100 1 3) */
	  "oid.0.9.2342.19200300.100.1.3", 29, NULL, 0, CRYPT_MAX_TEXTSIZE, TRUE, FALSE },
	{ 47, MKOID( "\x06\x0A\x09\x92\x26\x89\x93\xF2\x2C\x64\x01\x19" ),	/* domainComponent (0 9 2342 19200300 100 1 25) */
	  "dc", 2, "oid.0.9.2342.19200300.100.1.25", 30, CRYPT_MAX_TEXTSIZE, TRUE, FALSE },
	{ 48, MKOID( "\x06\x09\x2A\x86\x48\x86\xF7\x0D\x01\x09\x01" ),		/* emailAddress (1 2 840 113549 1 9 1) */
	  "email", 5, "oid.1.2.840.113549.1.9.1", 24, CRYPT_MAX_TEXTSIZE, TRUE, FALSE },
	{ 49, MKOID( "\x06\x07\x02\x82\x06\x01\x0A\x07\x14" ),				/* nameDistinguisher (0 2 262 1 10 7 20) */
	  "oid.0.2.262.1.10.7.20", 21, NULL, 0, CRYPT_MAX_TEXTSIZE, TRUE, FALSE },

	{ CRYPT_ATTRIBUTE_NONE, NULL }, { CRYPT_ATTRIBUTE_NONE, NULL }
	};

/* Check that a country code is valid as of ISO 3166 in 2021 */
...
static BOOLEAN checkCountryCode( IN_BUFFER_C( 2 ) const BYTE *countryCode )
  ...
  /*C*/	xA|	  xC|xD|   xF|xG|xH|xI|	  xK|xL|xM|xN|xO|	   xR|		xU|xV|xW|xX|xY|xZ, /*C*/
  /*D*/				xE|			   xJ|xK|	xM|	  xO|							   xZ, /*D*/
  /*E*/		  xC|	xE|	  xG|xH|						   xR|xS|xT,				   /*E*/
  /*F*/							xI|xJ|xK|	xM|	  xO|	   xR,						   /*F*/
  /*G*/	xA|xB|	 xD|xE|xF|xG|xH|xI|		 xL|xM|xN|	 xP|xQ|xR|xS|xT|xU|	  xW|	xY,	   /*G*/
  /*H*/								  xK|	xM|xN|		   xR|	 xT|xU,				   /*H*/
  /*I*/			 xD|xE|					 xL|xM|xN|xO|	xQ|xR|xS|xT,				   /*I*/
  /*J*/				xE|						xM|	  xO|xP,							   /*J*/
  /*K*/				xE|	  xG|xH|xI|			xM|xN|	 xP|   xR|			  xW|	xY|xZ, /*K*/
  /*L*/	xA|xB|xC|				xI|	  xK|				   xR|xS|xT|xU|xV|		xY,	   /*L*/
  /*M*/	xA|	  xC|xD|xE|xF|xG|xH|	  xK|xL|xM|xN|xO|xP|xQ|xR|xS|xT|xU|xV|xW|xX|xY|xZ, /*M*/
  /*N*/	xA|	  xC|	xE|xF|xG|	xI|		 xL|	  xO|xP|   xR|		xU|			   xZ, /*N*/
  /*O*/										xM,										   /*O*/
  /*P*/	xA|			xE|xF|xG|xH|	  xK|xL|xM|xN|		   xR|xS|xT|	  xW|	xY,	   /*P*/
  /*Q*/	xA,																			   /*Q*/
  /*R*/				xE|							  xO|		  xS|xU|	  xW,		   /*R*/
...
CHECK_RETVAL_BOOL STDC_NONNULL_ARG( ( 1 ) ) \
BOOLEAN sanityCheckDNComponent( IN_PTR const DN_COMPONENT *dnComponent )
	{
	...
	if( !checkVarStruct( dnComponent ) )
		{
		DEBUG_PUTS(( "sanityCheckDNComponent: VarStruct" ));
		return( FALSE );
		}
	...
	if( dnComponent->value != dnComponent->storage || \
		...
		dnComponent->valueStringType < 0 || \
		dnComponent->valueStringType > 20 )
		{
...
static DN_COMPONENT *findDNComponent( const DATAPTR_DN dnComponentList,
	...
	{
	LOOP_INDEX_PTR DN_COMPONENT *listPtr;
	int matchCount = 0;
...
static DN_COMPONENT *findDNComponentByOID( const DATAPTR_DN dnComponentList,
	...
	LOOP_MED( listPtr = DATAPTR_GET( dnComponentList ), listPtr != NULL, 
		...
		if( matchOID( oid, oidLength, dnComponentInfo->oid ) )
			return( listPtr );
		}
	ENSURES_N( LOOP_BOUND_OK );
...
static const DN_COMPONENT_INFO *findDNInfo( IN_INT const int type )
	...
	LOOP_LARGE( i = 0, 
		...
		ENSURES_N( LOOP_INVARIANT_LARGE( i, 0, 
										 FAILSAFE_ARRAYSIZE( certInfoOIDs, \
															 DN_COMPONENT_INFO ) - 1 ) );
...
const DN_COMPONENT_INFO *findDNInfoByOID( IN_BUFFER( oidLength ) const BYTE *oid, 
	...
	LOOP_LARGE( i = 0,
				...
				i++ )
		{
		const DN_COMPONENT_INFO *certInfoOID;
		...
		/* Perform a quick check of the OID.  The majority of all DN OIDs
		   ...
		   before we go for the full OID match.  We know that there's valid
		   data at offset 4 because the OID has to be at least MIN_OID_SIZE 
		   bytes long */
		certInfoOID = &certInfoOIDs[ i ];
		if( certInfoOID->oid[ 4 ] == oid[ 4 ] && \
			matchOID( oid, oidLength, certInfoOID->oid ) )
			return( certInfoOID );
		}
	ENSURES_N( LOOP_BOUND_OK );
	ENSURES_N( i < FAILSAFE_ARRAYSIZE( certInfoOIDs, DN_COMPONENT_INFO ) );
...
static int findDNInsertPoint( IN_PTR const DN_COMPONENT *listHeadPtr,
							  IN_INT const int type,
							  IN_BOOL const BOOLEAN fromExternalSource,
							  OUT_PTR_PTR_COND DN_COMPONENT **insertPointPtrPtr,
							  OUT_ENUM_OPT( CRYPT_ERRTYPE ) \
									CRYPT_ERRTYPE_TYPE *errorType )	
	{
	const DN_COMPONENT *prevElement = NULL;
	LOOP_INDEX_PTR const DN_COMPONENT *listPtr;
	int newValueSortOrder, status;
	...
	REQUIRES( ( type >= CRYPT_CERTINFO_FIRST_DN && \
				type <= CRYPT_CERTINFO_LAST_DN ) || \
			  ( type > 0 && type < 50 ) );
	REQUIRES( isBooleanValue( fromExternalSource ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 3, 7, 8 ) ) \
int insertDNstring( INOUT_PTR DATAPTR_DN *dnListHeadPtr, 
					IN_INT const int type,
	...
	*errorType = CRYPT_ERRTYPE_NONE;
	...
	if( valueLength > ( ( flags & DN_FLAG_NOCHECK ) ? \
						...
						( dnComponentInfo->wcsOK ) ? \
							( WCHAR_SIZE * dnComponentInfo->maxLength ) : \
	...
	REQUIRES( isShortIntegerRangeNZ( sizeof( DN_COMPONENT ) + \
									 valueLength ) );
...
int insertDNComponent( INOUT_PTR DATAPTR_DN *dnListHeadPtr,
		...
		*errorType = CRYPT_ERRTYPE_ATTR_VALUE;
		retExt( CRYPT_ARGERROR_STR1,
				( CRYPT_ARGERROR_STR1, errorInfo,
				  "Value can't be represented as an ASN.1 string type" ) );
		}

	return( insertDNstring( dnListHeadPtr, componentType, value, 
							valueLength, valueStringType, 0, errorInfo,
							errorType ) );
	}
...
static int deleteComponent( INOUT_PTR DATAPTR_DN *dnPtr, 
							INOUT_PTR DN_COMPONENT *theElement )
	{
...
int deleteDNComponent( INOUT_PTR DATAPTR_DN *dnPtr, 
					   IN_ATTRIBUTE const CRYPT_ATTRIBUTE_TYPE type,
					   IN_BUFFER_OPT( valueLength ) const void *value, 
					   IN_LENGTH_SHORT_Z const int valueLength )
	{
...
void deleteDN( INOUT_PTR_DATAPTR DATAPTR_DN *dnPtr )
	{
	DATAPTR_DN dn = *dnPtr;
	LOOP_INDEX_PTR DN_COMPONENT *dnComponentListPtr;
	...
	LOOP_MED_WHILE( dnComponentListPtr != NULL )
		{
...
int getDNComponentValue( IN_DATAPTR_OPT const DATAPTR_DN dn,
	...
	*valueLength = 0;
	if( value != NULL )
		{
		REQUIRES( isShortIntegerRangeNZ( valueMaxLength ) ); 
...
BOOLEAN compareDN( IN_DATAPTR_OPT const DATAPTR_DN dn1,
	...
	{
	LOOP_INDEX_PTR DN_COMPONENT *dn1ptr, *dn2ptr;
	...
	REQUIRES_B( isBooleanValue( dn1substring ) );
...
int copyDN( OUT_DATAPTR DATAPTR_DN *dnDestPtr, 
	...
	{
	DN_COMPONENT *destCursor = NULL;
	LOOP_INDEX_PTR DN_COMPONENT *srcCursor;
	...
	LOOP_MED( srcCursor = DATAPTR_GET( dnSrc ), 
		...
		/* Allocate memory for the new element and copy over the 
		   ...
		   after the copy */
		REQUIRES( isShortIntegerRangeNZ( sizeofVarStruct( srcCursor, \
														  DN_COMPONENT ) ) );
		if( ( newElement = ( DN_COMPONENT * ) \
				clAlloc( "copyDN", \
		...
		/* Clear any encoding information that may be present.  This is 
		   normally calculated as part of the encoding process, but we want 
		   to make sure that it's re-evaluated if we encode the DN again 
		   since changes in AVA values will affect overall RDN values, for
		   example adding a new AVA will change the previously-calculated 
		   RDN value */
		newElement->valueStringType = newElement->asn1EncodedStringType = \
			newElement->encodedAVAdataSize = \
			newElement->encodedRDNdataSize = 0;
...
int convertEmail( INOUT_PTR CERT_INFO *certInfoPtr, 
	...
	int certDataSize, publicKeyDataSize, subjectDNsize, issuerDNsize;
	int status;
	...
	issuerDNsize = certInfoPtr->issuerDNsize;
	...
	certInfoPtr->subjectDNsize = certInfoPtr->issuerDNsize = 0;
	...
	certInfoPtr->subjectDNsize = subjectDNsize;
	certInfoPtr->issuerDNsize = issuerDNsize;
