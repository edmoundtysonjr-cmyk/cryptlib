...
/* The following function (provided by Colin Plumb) is used to calculate the
   ...
   provide equivalent security from small-exponent attacks.  It's not clear 
   which paper Colin is referring to here, the obvious candidate would be 
   "On Diffie-Hellman Key Agreement with Short Exponents" from EuroCrypt'96 
   but it's not present in there and in any case the paper is by van 
   Oorschot and Wiener.  This leaves a tech report, possibly from Bell-
   Northern Research.  A more recent reference is the ENISA "Algorithms, key 
   size and parameters report", which (Table 3.1) draws on a number of 
   different sources to produce more or less the same figures.  "On the 
   (in)security of ElGamal in OpenPGP" references (Table 3) multiple 
   sources, specifically NIST SP 800-57, RFC 3766, and BSI-TR-02102-1 which
   all provide similar but nevertheless slightly different values.  Of all
   the ones in the mix, the alleged-Wiener ones are the most conservative.
...
static int generateDLPPublicValues( INOUT_PTR PKC_INFO *pkcInfo, 
	...
	int nPrimes, nFactors, factorBits;
	LOOP_INDEX i;
	...
	LOOP_LARGE( i = 0, i < MAX_NO_FACTORS, i++ )
		{
		ENSURES( LOOP_INVARIANT_LARGE( i, 0, MAX_NO_FACTORS - 1 ) );
	...
	LOOP_LARGE( i = 0, i < MAX_NO_PRIMES, i++ )
		{
		ENSURES( LOOP_INVARIANT_LARGE( i, 0, MAX_NO_PRIMES - 1 ) );
	...
	LOOP_LARGE( i = 0, i < nFactors, i++ )
		{
		ENSURES( LOOP_INVARIANT_LARGE( i, 0, nFactors - 1 ) );
	...
	LOOP_LARGE_INITCHECK( primeFound = FALSE, !primeFound )
		...
		LOOP_LARGE_REV_ALT( i = nFactors - 2, i >= 0, i-- )
			{
			ENSURES( LOOP_INVARIANT_REV_ALT( i, 0, nFactors - 2 ) );
		...
		LOOP_MAX_WHILE_ALT( indices[ nFactors - 1 ] > 0 )
			{
			...
			LOOP_LARGE_REV_ALT2( i = indexMoved, 
								 bnStatusOK( bnStatus ) && i >= 0, i-- )
				{
				ENSURES( LOOP_INVARIANT_REV_ALT2( i, 0, indexMoved ) );

				CK( BN_mul( &llProducts[ i ], &llProducts[ i + 1 ],
							&llPrimes[ indices[ i ] ], &pkcInfo->bnCTX ) );
				}
			ENSURES( LOOP_BOUND_LARGE_REV_OK_ALT2 );
			...
			LOOP_LARGE_ALT2( i = 0, i < nFactors, i++ )
				{
				ENSURES( LOOP_INVARIANT_LARGE_ALT2( i, 0, nFactors - 1 ) );
			...
			LOOP_LARGE_REV_ALT2( i = indexMoved - 1, i >= 0, i-- )
				{
				ENSURES( LOOP_INVARIANT_REV_ALT2( i, 0, indexMoved - 1 ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int generateDLPPublicValue( INOUT_PTR PKC_INFO *pkcInfo )
	{
	const DH_DOMAINPARAMS *domainParams = pkcInfo->domainParams;
...
/* Perform validity checks on the public key.  This function is only called 
   ...
   built-in values.  The difference between traditional (PKCS #3) DH keys 
   and more general DLP keys is that PKCS #3 has { p, g } while more general
   keys have { p, q, g } which allows additional checking to be done.
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int checkDLPDomainParameters( INOUT_PTR PKC_INFO *pkcInfo, 
	...
	if( !primeCheckQuick( p ) )
		{
		DEBUG_DIAG(( "DLP p value is non-prime" ));
		return( CRYPT_ARGERROR_STR1 );
		}
	...
	if( isPKCS3 && gBits > 8 )
		{
		DEBUG_DIAG(( "PKCS #3 DH g value size %d bits is too large, should "
					 "be 2...8 bits", gBits ));
	...
	/* Verify that q is not (obviously) composite.  Note that we can't 
	   ...
	   set up but not after the pkcInfo is fully initialised so we use
	   primeProbableFermat() instead */
	if( !primeCheckQuick( q ) )
		{
		DEBUG_DIAG(( "DLP q value is non-prime" ));
		return( CRYPT_ARGERROR_STR1 );
		}
	...
	CK( BN_div( NULL, tmp, tmp, q, &pkcInfo->bnCTX ) );
	if( bnStatusError( bnStatus ) )
		return( getBnStatus( bnStatus ) );
	if( !BN_is_zero( tmp ) )
		{
		DEBUG_DIAG(( "DLP q value doesn't divide (p - 1)" ));
		return( CRYPT_ARGERROR_STR1 );
		}
	...
	if( !BN_is_one( tmp ) )
		{
		DEBUG_DIAG(( "DLP g value isn't a generator of q" ));
		return( CRYPT_ARGERROR_STR1 );
		}
...
static int checkDLPPublicKey( INOUT_PTR PKC_INFO *pkcInfo )
	...
	if( !BN_is_zero( q ) && BN_num_bits( q ) < BN_num_bits( p ) - 1 )
		...
		if( !BN_is_one( tmp ) )
			{
			DEBUG_DIAG(( "DLP y value doesn't have the correct order "
						 "subgroup" ));
			return( CRYPT_ARGERROR_STR1 );
			}
		}

	/* Verify that y != g, since if y == g then the output of the DH 
	   computation is just y', the other side's y value.  y can never match 
	   g for PKCS #3 keys where g is constrained to be small, but can occur 
	   for FIPS 186/X9.42 keys */
	if( BN_cmp( y, g ) == 0 )
		{
		DEBUG_DIAG(( "DLP y value is the same as g" ));
		return( CRYPT_ARGERROR_STR1 );
		}
...
#if defined( USE_DH ) && ( defined( USE_SSH ) || defined( USE_TLS ) )
	if( domainParams == NULL )
		{
