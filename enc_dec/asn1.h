...
/* The highest encoded tag value and a check that a tag is valid, generally
   not used except to guide static analysers via ANALYSER_HINT() since both 
   peekTag() and readTag() are guaranteed to only return valid tags.  We 
   exclude 0, which is technically a tag for EOC, since it's not a tag value 
   that we'll see in front of an ASN.1 object */

#define MAX_TAG				( BER_CONTEXT_SPECIFIC | BER_CONSTRUCTED | \
							  MAX_SHORT_BER_ID )
#define isValidTag( tag )	( ( tag ) > 0 && ( tag ) < MAX_TAG )
...
/* Generalized ASN.1 type manipulation routines.  readRawObject() reads a
   complete object (including tag and length data), since this always 
   requires a tag we don't have the xxx/xxxData() variants that exist for 
   other functions.
   
   readUniversal() just skips the current object.  There are two variants
   of this, the standard one which checks that there's data present and an
   Opt variant that allows zero-length objects.  The latter is required for
   braindamaged PKI protocols that specify mandatory fields that are
   actually optional, for which the standard practice is to send a zero-
   length object */
...
#define readIntegerData( stream, integer, integerMaxLength, integerLength ) \
		readIntegerTag( stream, integer, integerMaxLength, integerLength, NO_TAG )
#define readInteger( stream, integer, integerMaxLength, integerLength ) \
		readIntegerTag( stream, integer, integerMaxLength, integerLength, DEFAULT_TAG )

/* Routines for handling bignums.  Since the BIGNUM struct isn't visible at 
...
typedef CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
		int ( *READ_BIGNUM_FUNCTION )( INOUT_PTR STREAM *stream, 
...
int writeBignumTag( INOUT_PTR STREAM *stream, 
					IN_PTR TYPECAST( const BIGNUM * ) \
						const struct BN *bignum, 
					IN_TAG const int tag );

#define readBignum( stream, bignum, minLen, maxLen, maxRange, checkType ) \
		readBignumTag( stream, bignum, minLen, maxLen, maxRange, checkType, DEFAULT_TAG )
...
/* Another special-case bignum routine that verifies that the bignum data to
   ...
   was transferred into the in-memory forum, and has the same signature as
   read Bignum() even though only the stream, bignum (as a const) and tag 
   are used */
...
/* Routines for handling UTC and Generalized time.  Alongside the time-write 
   functions with explicit types we also have a general readTime()writeTime() 
   that follows the RFC 3280 rules for dealing with the non-Y2K-safe times 
   used in ASN.1 */
...
int checkCertObjectEncodingLength( IN_BUFFER( objectLength ) \
								   ...
								   OUT_DATALENGTH_Z int *objectActualSize );
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int getStreamObjectLength( INOUT_PTR STREAM *stream, 
