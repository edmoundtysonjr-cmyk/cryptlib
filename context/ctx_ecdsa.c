...
/* Sign a single block of data.  There's a possibility of fault attacks 
   ...
   Since ECDSA is a DSA-style Schnorr signature scheme it comes with a near-
   infinite collection of side-channels which we try and mitigate but which
   can never really be patched because there are just too many of them.  In
   particular we're stuck on the ECC perpetual-vulnerability-machine 
   treadmill of having to continuously patch the code as new attacks come 
   up (as "A synthesis of side-channel attacks on elliptic curve 
   cryptography" puts it, the treadmill is "different countermeasures that 
   have been imagined and introduced year after year, attack after attack"), 
   as well as second-guessing what different compilers will do with the 
   attack workarounds.  To deal with this we push the mitigations to the 
   higher-level code in which the signing operation is exposed to attack, 
   including all of the future attacks yet to be discovered.
   
   Apart from the perpetual-vulnerability-machine issue, trying to counter
   problems at this level also runs into the problem of the bewildering 
   array of possible representations that the values are flipped in and
   out of during processing.  Typcally the affine coordinates (x, y) are
   replaced by projective coordinates (X, Y, Z) = (lambda*x, lambda*y,
   lambda), lambda != 0 and an element of the base field, but we can also
   use Jacobian projective coordinates = (lambda^2*x, lambda^3*y, lambda),
   lambda as before, which is apparently what's used here (see EC_POINT
   in ec_lcl.h), from which they can be recovered as x = X/Z^2, y = Y/Z^3.
   This means that we'd need to not only choose countermeasures that will
   hopefully withstand the next set of attacks and that won't be negated by
   what compilers will do, but that are specific to whatever representation
   is in effect at that point in the processing */
...
static int sign( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
	...
	/* Make k fixed-length to try and close one of Schnorr signature's infinite
	   ...
	   that the fixed-bit-length also addresses it */
	CK( BN_add( k, k, n ) );
	if( BN_num_bits( k ) <= BN_num_bits( n ) )
		{
		CK( BN_add( k, k, n ) );
		}
	else
		{
		/* Perform a dummy add of a value the same size as k to balance out 
		   the timing (the second add of n essentially never happens so it's 
		   probably unnecessary) */
		( void ) BN_add( hash, k, n );
		}
	if( bnStatusError( bnStatus ) )
		return( getBnStatus( bnStatus ) );
	...
	/* More generally, and usefully, check that the values aren't 
	   suspiciously small.  Annoyingly, we can't perform any more heavy-duty
	   ECC point verification because we only have the x-coordinate r of the
	   signing point kG, with s being merely a signature proof, however in
...
static int sigCheck( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
	...
	/* Decode the values from a DL data block and make sure that r and s are
	   valid, i.e. r, s = [1...n-1] */
	status = capabilityInfoPtr->decodeDLValuesFunction( eccParams->inParam2, 
...
static const CAPABILITY_INFO capabilityInfo = {
	...
	sign, sigCheck, readPublicKeyEccFunction, writePublicKeyEccFunction,
