...
static int getRandomData( INOUT_PTR_OPT void *dummy, 
...
static int primeProbableOld( INOUT_PTR PKC_INFO *pkcInfo, 
							 INOUT_PTR BIGNUM *candidate, 
							 IN_RANGE( 1, MAX_NO_PRIME_CHECKS ) \
								const int noChecks )
	{
...
int primeProbable( INOUT_PTR PKC_INFO *pkcInfo, 
	...
	static_assert( PRIME_TABLE_SIZE > MAX_NO_PRIME_CHECKS, 
				   "Prime table size" );
	...
	*isPrime = FALSE;
	...
	LOOP_LARGE( i = 0, i < noChecks, i++ )
		...
		/* Instead of using a bignum for the Miller-Rabin check we use a
		   ...
		   Obviously we're not going to do this when we generate our own 
		   keys so one base is as good as another, and small primes work 
		   well (even a single Fermat test has a failure probability of 
		   around 10e-44 for 512-bit primes if you're not trying to cook the 
		   primes, this is why Fermat works as a verification of the Miller-
		   Rabin test in generatePrime()).
		   
		   There are two situations in which we could run into problems with
		   maliciously-chosen candidates, an example of which is given in
		   checkCompositePrime() at the end of this module.  One is if the 
		   values are used in SRP, see Bleichenbacher's "Breaking a 
		   Cryptographic Protocol with Pseudoprimes", PKC'05, but we don't 
		   do SRP or similar PAKEs.  The other is if we're importing 
		   public/private key pairs where the public portions aren't 
		   integrity-protected or tied to the private key components, in 
		   which case it's possible for an attacker to replace the public-
		   key portions with maliciously-chosen ones.  For cryptlib's keys 
		   the private key is tied to a hash of the SPKI so any change to 
		   the public-key components will be detected, and for import-only 
		   formats like PGP or PKCS #12 which don't have this protection 
		   the data is purely a transport format for which it's assumed that 
		   the user has taken protection measures.
		   
		   If we ever need to defend against maliciously-chosen primes then 
		   the primality test will have to be strengthened, perhaps with the 
		   Baillie-PSW implementation in "A Performant, Misuse-Resistant API 
...
static int generatePrimeEx( INOUT_PTR PKC_INFO *pkcInfo, 
	...
	/* Allocate the array */
	REQUIRES( isIntegerRangeNZ( SIEVE_SIZE * sizeof( BOOLEAN ) ) );
...
#elif defined USE_BPSW
			/* See the comment in bn/bn_bpsw.c for why this isn't used */
			status = primeProbable( pkcInfo, candidate, 1, 
									&primeFound );
			if( cryptStatusOK( status ) )
				{
				int isPrime;
...
int generateBignumEx( OUT_PTR BIGNUM *bignum, 
					  IN_LENGTH_SHORT_MIN( 120 ) const int noBits, 
					  ...
					  IN_PTR_OPT const void *getRandomInfoPtr )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int generateBignum( OUT_PTR BIGNUM *bignum, 
					IN_LENGTH_SHORT_MIN( 120 ) const int noBits, 
					IN_BYTE const int high, IN_BYTE const int low )
	{
...
void checkCompositePrime( void )
	...
	status = importBignum( &pkcInfo->rsaParam_n, compositePrime, 
						   290, 289, 290, NULL, BIGNUM_CHECK_VALUE_PKC );
