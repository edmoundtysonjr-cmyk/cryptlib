...
static const KEYMGMT_ACL keyManagementACL[] = {
	...
	MK_KEYACL_EX( KEYMGMT_ITEM_PUBLICKEY,
		/* R */	ST_KEYSET_ANY | ST_DEV_P11 | ST_DEV_CAPI | ST_DEV_TPM | \
				ST_DEV_HW,
		/* W */	ST_KEYSET_FILE | ST_KEYSET_FILE_PARTIAL | ST_KEYSET_DBMS | \
				ST_KEYSET_LDAP | ST_DEV_P11 | ST_DEV_CAPI | ST_DEV_TPM | \
				ST_DEV_HW,
		...
		/*IDs*/	pubKeyIDs, FAILSAFE_ARRAYSIZE( pubKeyIDs, CRYPT_KEYID_TYPE ),
	...
	MK_KEYACL_RWD( KEYMGMT_ITEM_PRIVATEKEY,
		...
		/*FnQ*/	ST_NONE, ST_NONE,
		/*Obj*/	ST_CTX_PKC,
		/*IDs*/	privKeyIDs, FAILSAFE_ARRAYSIZE( privKeyIDs, CRYPT_KEYID_TYPE ),
		/*Flg*/	KEYMGMT_FLAG_CHECK_ONLY | KEYMGMT_FLAG_LABEL_ONLY | \
	...
	MK_KEYACL( KEYMGMT_ITEM_SECRETKEY,
		...
		/*FnQ*/	ST_NONE,
		/*Obj*/	ST_CTX_CONV,
		/*IDs*/	secKeyIDs, FAILSAFE_ARRAYSIZE( secKeyIDs, CRYPT_KEYID_TYPE ),
	...
	MK_KEYACL_RWD( KEYMGMT_ITEM_REQUEST,
		...
		/*IDs*/	certReqIDs, FAILSAFE_ARRAYSIZE( certReqIDs, CRYPT_KEYID_TYPE ),
	...
	MK_KEYACL_RWD( KEYMGMT_ITEM_REVREQUEST,
		...
		/*FnQ*/	ST_NONE, ST_KEYSET_DBMS_STORE,
		/*Obj*/	ST_CERT_REQ_REV,
		/*IDs*/	revReqIDs, FAILSAFE_ARRAYSIZE( revReqIDs, CRYPT_KEYID_TYPE ),
	...
	MK_KEYACL_RWD( KEYMGMT_ITEM_PKIUSER,
		...
		/*FnQ*/	ST_NONE, ST_NONE,
		/*Obj*/	ST_CERT_PKIUSER,
		/*IDs*/	pkiUserIDs, FAILSAFE_ARRAYSIZE( pkiUserIDs, CRYPT_KEYID_TYPE ),
	...
	MK_KEYACL_RWD( KEYMGMT_ITEM_REVOCATIONINFO,
		...
		/*FnQ*/	ST_NONE, ST_NONE,
		/*Obj*/	ST_CERT_CRL,
		/*IDs*/	revInfoIDs, FAILSAFE_ARRAYSIZE( revInfoIDs, CRYPT_KEYID_TYPE ),
		/*Flg*/	KEYMGMT_FLAG_CHECK_ONLY,
		ACCESS_KEYSET_FxRxD, ACCESS_KEYSET_FNxxx ),
...
static const IDTYPE_ACL idTypeACL[] = {
	{ CRYPT_KEYID_NAME, 
	  ST_KEYSET_ANY | ST_DEV_P11 | ST_DEV_CAPI | ST_DEV_TPM | ST_DEV_HW },
	{ CRYPT_KEYID_URI, 
	  ST_KEYSET_ANY | ST_DEV_P11 | ST_DEV_TPM | ST_DEV_HW },
	{ CRYPT_IKEYID_KEYID, 
	  ST_KEYSET_FILE | ST_KEYSET_FILE_PARTIAL | ST_KEYSET_FILE_RO | \
	  ST_KEYSET_DBMS | ST_KEYSET_DBMS_STORE | ST_DEV_P11 | ST_DEV_TPM | \
	  ST_DEV_HW },
	{ CRYPT_IKEYID_PGPKEYID, 
	  ST_KEYSET_FILE | ST_KEYSET_FILE_PARTIAL | ST_KEYSET_FILE_RO | \
	  ST_DEV_TPM | ST_DEV_HW },
	{ CRYPT_IKEYID_CERTID, 
	  ST_KEYSET_DBMS | ST_KEYSET_DBMS_STORE },
	{ CRYPT_IKEYID_ISSUERID, 
	  ST_KEYSET_FILE | ST_KEYSET_DBMS | ST_KEYSET_DBMS_STORE | ST_DEV_TPM | \
	  ST_DEV_HW },
	{ CRYPT_IKEYID_ISSUERANDSERIALNUMBER, 
...
int initKeymgmtACL( void )
	...
	LOOP_LARGE( i = 0, 
		...
		ENSURES( LOOP_INVARIANT_LARGE( i, 0, 
									   FAILSAFE_ARRAYSIZE( keyManagementACL, \
														   KEYMGMT_ACL ) - 1 ) );
		...
		if( keyMgmtACL->keysetR_subTypeA != ST_NONE || \
			...
			( keyMgmtACL->keysetR_subTypeB & \
				~( SUBTYPE_CLASS_B | ST_KEYSET_ANY | ST_DEV_P11 | \
									 ST_DEV_CAPI | ST_DEV_TPM | \
									 ST_DEV_HW ) ) != 0 || \
			keyMgmtACL->keysetR_subTypeC != ST_NONE )
			{
			DEBUG_DIAG(( "Key management/read ACLs inconsistent" ));
			retIntError();
			}
		...
		if( keyMgmtACL->keysetW_subTypeA != ST_NONE || \
			...
			{
			DEBUG_DIAG(( "Key management/write ACLs inconsistent" ));
			retIntError();
			}
		...
		if( keyMgmtACL->keysetFN_subTypeA != ST_NONE || \
			...
			{
			DEBUG_DIAG(( "Key management/findfirst ACLs inconsistent" ));
			retIntError();
			}
		...
		if( keyMgmtACL->keysetQ_subTypeA != ST_NONE || \
			...
			{
			DEBUG_DIAG(( "Key management/query ACLs inconsistent" ));
			retIntError();
			}
		...
		if( ( keyMgmtACL->objSubTypeA & ( SUBTYPE_CLASS_B | \
			...
			{
			DEBUG_DIAG(( "Key management object ACLs inconsistent" ));
			retIntError();
			}

		ENSURES( keyMgmtACL->allowedKeyIDs != NULL );
		LOOP_LARGE_ALT( j = 0, 
						j < FAILSAFE_ITERATIONS_SMALL && \
							keyMgmtACL->allowedKeyIDs[ j ] != CRYPT_KEYID_NONE, 
						j++ )
			{
			ENSURES( LOOP_INVARIANT_LARGE_ALT( j, 0, FAILSAFE_ITERATIONS_SMALL - 1 ) );

			ENSURES( isEnumRange( keyMgmtACL->allowedKeyIDs[ j ], 
								  CRYPT_KEYID ) );
			}
		...
		if( keyMgmtACL->specificKeysetSubTypeA != ST_NONE || \
			...
			{
			DEBUG_DIAG(( "Key management keyset ST ACLs inconsistent" ));
			retIntError();
			}
		...
		if( ( keyMgmtACL->specificObjSubTypeA & ( SUBTYPE_CLASS_B | \
			...
			{
			DEBUG_DIAG(( "Key management object ST ACLs inconsistent" ));
			retIntError();
			}
		}
	...
	LOOP_LARGE( i = 0, 
				i < FAILSAFE_ARRAYSIZE( idTypeACL, IDTYPE_ACL ) && \
					idTypeACL[ i ].idType != CRYPT_KEYID_NONE,
				i++ )
		{
		const IDTYPE_ACL *idACL;

		ENSURES( LOOP_INVARIANT_LARGE( i, 0, 
									   FAILSAFE_ARRAYSIZE( idTypeACL, \
														   IDTYPE_ACL ) - 1 ) );
		...
		if( ( idACL->keysetSubTypeB & \
				~( SUBTYPE_CLASS_B | ST_KEYSET_ANY | ST_DEV_P11 | \
									 ...
									 ST_DEV_HW ) ) != 0 )
			{
			DEBUG_DIAG(( "Key management ID ACLs inconsistent" ));
			retIntError();
			}
		}
	ENSURES( LOOP_BOUND_OK );
...
int preDispatchCheckKeysetAccess( IN_HANDLE const int objectHandle,
	...
	LOOP_MED( i = 0, 
			  i < FAILSAFE_ARRAYSIZE( keyManagementACL, KEYMGMT_ACL ) && \
					keyManagementACL[ i ].itemType != messageValue && \
					keyManagementACL[ i ].itemType != KEYMGMT_ITEM_NONE, 
			  i++ )
		{
	...
	if( keymgmtACL->idUseFlags & accessType )
		...
		LOOP_SMALL( index = 0,
					index < keymgmtACL->allowedKeyIDCount && \
						keymgmtACL->allowedKeyIDs[ index ] != CRYPT_KEYID_NONE,
					index++ )
			{
			ENSURES( LOOP_INVARIANT_SMALL( index, 0, 
										   keymgmtACL->allowedKeyIDCount - 1 ) );
		...
		LOOP_MED( index = 0, 
				  index < FAILSAFE_ARRAYSIZE( idTypeACL, IDTYPE_ACL ) && \
						idTypeACL[ index ].idType != CRYPT_KEYID_NONE, 
				  index++ )
			{
			ENSURES( LOOP_INVARIANT_MED( index, 0, 
										 FAILSAFE_ARRAYSIZE( idTypeACL, \
															 IDTYPE_ACL ) - 1 ) );
