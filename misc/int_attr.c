...
void *attributeFindStart( IN_PTR_OPT const void *attributePtr,
	...
	LOOP_MAX( iterationCount = 0, 
		...
		ENSURES_N( LOOP_INVARIANT_MAX( iterationCount, 0, 
									   FAILSAFE_ITERATIONS_MAX - 1 ) );
...
static const void *moveCursorByGroup( IN_PTR const void *currentCursor,
									  IN_PTR GETATTR_FUNCTION getAttrFunction,
									  IN_ENUM( CURSOR_MOVE ) \
										const CURSOR_MOVE_TYPE cursorMoveType, 
									  IN_INT int count, 
									  IN_BOOL const BOOLEAN absMove )
	{
	...
	REQUIRES_N( isBooleanValue( absMove ) );
...
CHECK_RETVAL_PTR STDC_NONNULL_ARG( ( 1, 2 ) ) \
static const void *moveCursorByAttribute( IN_PTR const void *currentCursor,
										  IN_PTR GETATTR_FUNCTION getAttrFunction,
										  IN_ENUM( CURSOR_MOVE ) \
											const CURSOR_MOVE_TYPE cursorMoveType, 
										  IN_INT int count, 
...
CHECK_RETVAL_PTR STDC_NONNULL_ARG( ( 1, 2 ) ) \
static const void *moveCursorByInstance( IN_PTR const void *currentCursor,
										 IN_PTR GETATTR_FUNCTION getAttrFunction,
										 IN_ENUM( CURSOR_MOVE ) \
											const CURSOR_MOVE_TYPE cursorMoveType, 
										 IN_INT int count, 
...
const void *attributeMoveCursor( IN_PTR_OPT const void *currentCursor,
	...
	LOOP_SMALL( i = 0, 
		...
		ENSURES_N( LOOP_INVARIANT_SMALL( i, 0, 
										 FAILSAFE_ARRAYSIZE( moveCodeMap, \
															 MOVECODE_MAP_INFO ) - 1 ) );
		}
	ENSURES_N( LOOP_BOUND_OK );
