...
/* Read a line of text from a stream.  The localError flag is set when the 
   ...
   a network I/O error.  The read options are:

	NONE: Default, read a single line of text.
	
	MULTILINE: Read a single line, continuing to the next one if a 
		continuation character is present at the end of the current one.
		
	RAW: Read a single line but don't perform any canonicalisation such as
		stripping leading/trailing whitespace.

	TRUNCATE: Read a single line of text, dropping any excess characters
		beyond the given length limit.
...
typedef enum {
	...
	READTEXT_TRUNCATE,		/* Truncate over-long lines */
	READTEXT_RAW,			/* Return non-canonicalised input */
	READTEXT_LAST			/* Last possible readtext option */
	} READTEXT_TYPE;
...
/* Check whether a value is a valid text character or not, used when text
   strings like HTTP headers are read.  This is currently mapped directly to 
   isPrint() but left with a distinct name to make it easier to find in the
   code */

#define isValidTextChar( value )	isPrint( value )
...
/* Prototypes for various extended error-handling functions.  retExt() 
   ...
   versions of the function that take additional error info parameters:
	...
	retExtErr() takes a pointer to existing error info, used when (for
		...
		can point to the same location if we're augmenting existing error
		information with additional text.  Called as:
...
#define checkVarStruct( structure ) \
		isShortIntegerRange( ( structure )->storageSize )
