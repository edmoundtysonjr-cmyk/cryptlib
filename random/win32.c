...
/* Try and connect to the system RNG if there's one present.  In theory we 
   ...
   being present, enabled, and accessible on anything but the latest systems
   are pretty slim, and since the TBS require you to to talk to the TPM at 
   the raw APDU level (not to mention all the horror stories in the MSDN 
   forums about getting any of this stuff to work) the amount of effort 
   required versus the potential gain really doesn't make it worthwhile (one 
   talk on TPM programming calls it "a horrifically bad programming 
   experience"):
   ...
   There is a higher-level API, the TSPI from the TCG, but under Windows 
   it's only available as a semi-experimental implementation provided as a 
   custom download, on the off chance that someone is using this we 
   optionally provide the TSPI version further down under the USE_TPM_RNG 
   define */
...
/* Try and get random data from the TPM.  Note that this uses the Unix-
   derived TSPI interface, not the unusable Windows TBS one, see the earlier 
   comment on this */
...
static void readPnPData( void )
	...
	LOOP_MAX( deviceCount = 0, 
			  ...
			  deviceCount++ )
		{
		ENSURES_V( LOOP_INVARIANT_MAX( deviceCount, 0, 
									   FAILSAFE_ITERATIONS_MAX ) );
...
/*	addRandomValue( randomState, GetQueueStatus( QS_ALLEVENTS ) ); */
	...
	if( getSysVar( SYSVAR_HWINTRINS ) & HWINTRINS_FLAG_RDTSC )
		...
		if( QueryPerformanceCounter( &performanceCount ) )
			...
			{
			/* Millisecond accuracy at best... there's also a 
			   GetTickCount64() which doesn't wrap every 49.7 days, but this
			   one is fine since we're only using it for entropy seeding */
			addRandomValue( randomState, GetTickCount() );
			}
		}
...
static void slowPollWindows( void )
	...
	LOOP_MED( nDrive = 0, nDrive < 20, nDrive++ )
		...
		/* Note: This only works if the user has turned on the disk
		   ...
		   traditionally disabled under Windows NT, under Win2K and newer 
		   the physical disk object is enabled by default while the logical 
		   disk object is disabled by default ('diskperf -yv' turns on the 
		   counters for logical drives in this case, since they're already 
		   on for physical drives), and under Windows 8 and newer they're
		   "automatically enabled on demand".
	...
	LOOP_LARGE( dwType = 0, dwType < 32, dwType++ )
		...
		ENSURES( LOOP_INVARIANT_LARGE( dwType, 0, 31 ) );

		LOOP_SMALL_ALT( i = 0, processInfo[ i ].type != CRYPT_ERROR, i++ )
			{
			ENSURES( LOOP_INVARIANT_SMALL_ALT_GENERIC() );
	...
	if( pNtPowerInformation != NULL )
		{
		typedef struct { int type; int size; } POWER_INFO;
		static const POWER_INFO powerInfo[] = {
		...
		LOOP_SMALL( i = 0, 
					i < FAILSAFE_ARRAYSIZE( powerInfo, POWER_INFO ) && \
						powerInfo[ i ].type != CRYPT_ERROR,
					i++ )
			{
			ENSURES_V( LOOP_INVARIANT_SMALL( i, 0, 
											 FAILSAFE_ARRAYSIZE( powerInfo, \
																 POWER_INFO ) - 1 ) );
...
static unsigned __stdcall threadSafeSlowPollWindows( void *dummy )
	...
	DEBUG_DIAG_ATOMIC( "slowPoll thread %lX running", GetCurrentThreadId() );
...
#endif /* 0 */
#if defined( _MSC_VER ) && VC_GE_2010( _MSC_VER )	
	DEBUG_DIAG_ATOMIC( "slowPoll thread %lX done", GetCurrentThreadId() );
#endif /* VC++ atomic diagnostic printing */
	_endthreadex( 0 );
	return( 0 );
	}
...
void slowPoll( void )
	...
	hThread = ( HANDLE ) _beginthreadex( NULL, 0,
										 ...
										 NULL, 0, &threadID );
#if defined( _MSC_VER ) && VC_GE_2010( _MSC_VER )	
	DEBUG_DIAG_ATOMIC( "%lX created slowPoll thread %lX", 
...
CHECK_RETVAL \
int waitforRandomCompletion( IN_BOOL const BOOLEAN force )
	{
	DWORD dwResult;
	const DWORD timeout = force ? 2000 : 300000L;
	int status;

	REQUIRES( isBooleanValue( force ) );
