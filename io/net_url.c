	...
	*urlType = URL_TYPE_NONE;
	...
	LOOP_MED( i = 0, 
		...
		ENSURES( LOOP_INVARIANT_MED( i, 0, 
									 FAILSAFE_ARRAYSIZE( urlSchemaInfo, \
														 URL_SCHEMA_INFO ) - 1 ) );
...
int parseURL( OUT_PTR URL_INFO *urlInfo, 
	...
	LOOP_MAX( urlIndex = 0, urlIndex < urlLen, urlIndex++ )
		...
		ENSURES( LOOP_INVARIANT_MAX( urlIndex, 0, urlLen - 1 ) );

		ch = byteToInt( url[ urlIndex ] );
	...
	if( ( offset = strFindCh( strPtr, strLen, '@' ) ) >= 0 )
		...
		/* Extract the user info, specifically as an RFC 3986 section 3.2.1 
