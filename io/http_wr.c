...
static int encodeRFC1866( INOUT_PTR STREAM *headerStream, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeReqMethod( INOUT_PTR STREAM *stream, 
						   IN_PTR const NET_STREAM_INFO *netStream,
						   IN_BOOL const BOOLEAN usePost )
	{
	int status;
	...
	REQUIRES( isBooleanValue( usePost ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeReqURI( INOUT_PTR STREAM *stream, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeReqHeaderRemainder( INOUT_PTR STREAM *stream, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeReqUpgradeHeaders( INOUT_PTR STREAM *stream, 
								   IN_PTR const HTTP_REQ_INFO *httpReqInfo )
	{
	assert( isWritePtr( stream, sizeof( STREAM ) ) );
	assert( isReadPtr( httpReqInfo, sizeof( HTTP_REQ_INFO ) ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeContentHeaders( INOUT_PTR STREAM *stream, 
...
int writeRequestHeader( INOUT_PTR STREAM *stream, 
						IN_PTR_OPT const HTTP_REQ_INFO *httpReqInfo,
	...
	REQUIRES( ( contentLength == 0 && contentType == NULL && \
				contentTypeLen == 0 ) || \
			  ( isBufsizeRangeNZ( contentLength ) && \
			    contentType != NULL && \
	...
	REQUIRES( ( httpReqInfo == NULL ) || \
			  ( httpReqInfo != NULL && \
				( ( httpReqInfo->attributeLen == 0 && \
					httpReqInfo->valueLen == 0 ) || \
				  ( httpReqInfo->valueLen > 0 ) ) ) );
	REQUIRES( isBooleanValue( forceGet ) );
	REQUIRES( netStream != NULL && sanityCheckNetStream( netStream ) );

	/* Write the method, "GET/POST/etc", and the URI */
	...
	/* If we're sending what should be a POST as a GET to deal with broken 
	   servers then we have to stop at this point because the payload data 
	   which is written by the caller is part of the request URI.  Otherwise
	   we continue with the rest of the request URI, "HTTP/1.1" and any
	   extra fields like Host: and Connection: */
	if( !forceGet )
		{
		status = writeReqHeaderRemainder( &headerStream, netStream );
		ENSURES( cryptStatusOK( status ) );
#ifdef USE_WEBSOCKETS
		if( TEST_FLAG( netStream->nhFlags, STREAM_NHFLAG_WS_UPGRADE ) )
			{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeRespMethod( INOUT_PTR STREAM *stream, 
							IN_PTR const NET_STREAM_INFO *netStream,
							IN_BOOL const BOOLEAN isHTTP10 )
	{
	assert( isWritePtr( stream, sizeof( STREAM ) ) );
	assert( isReadPtr( netStream, sizeof( NET_STREAM_INFO ) ) );

	REQUIRES( isBooleanValue( isHTTP10 ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static int writeRespUpgradeHeaders( INOUT_PTR STREAM *stream, 
									IN_PTR const HTTP_REQ_INFO *httpReqInfo )
	{
	assert( isWritePtr( stream, sizeof( STREAM ) ) );
	assert( isReadPtr( httpReqInfo, sizeof( HTTP_REQ_INFO ) ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int writeResponseHeader( INOUT_PTR STREAM *stream, 
								IN_PTR_OPT const HTTP_REQ_INFO *httpReqInfo,
								IN_BUFFER_OPT( contentTypeLen ) \
									const char *contentType, 
								IN_LENGTH_SHORT_Z const int contentTypeLen, 
...
static int writeFunction( INOUT_PTR STREAM *stream, 
	...
	REQUIRES( ( httpDataInfo->buffer == NULL && \
				httpDataInfo->bytesToWrite == 0 ) || \
			  ( httpDataInfo->buffer != NULL && \
				httpDataInfo->bytesToWrite > 0 ) );
	...
	*length = 0;
	...
	if( TEST_FLAG( netStream->nFlags, STREAM_NFLAG_ISSERVER ) )
		...
		/* If we have to override the use of the (correct) POST with the 
		   ...
		   URI to be written as the payload data, followed by the remainder
		   of the HTTP header after the payload write:
		   
			writeRequestHeader(): "GET /..../...?foo="
			bufferedTransportWrite(): "abcdefghijklmnop"
			writeReqHeaderRemainder(): 
							" HTTP/1.1\r\nHost: xxx\r\nConnection: xxx" */
...
STDC_NONNULL_ARG( ( 1 ) ) \
void setStreamLayerHTTPwrite( INOUT_PTR NET_STREAM_INFO *netStream )
	{
