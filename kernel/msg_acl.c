...
#define PUBKEY_KEYSET_OBJECT	( ST_KEYSET_FILE | ST_KEYSET_FILE_PARTIAL | \
								  ...
								  MKTYPE_CAPI( ST_DEV_CAPI ) | \
								  MKTYPE_TPM( ST_DEV_TPM ) | \
								  MKTYPE_HW( ST_DEV_HW ) )
...
static const CREATE_ACL createObjectIndirectACL[] = {
	...
	/* Keyset object instantiated from encoded data, used with crypto
	   devices where data is stored as PKCS #15 objects */
#if defined( USE_HARDWARE ) || defined( USE_TPM )
	{ OBJECT_TYPE_KEYSET,
	  { MKACP_N_FIXED( CRYPT_KEYSET_FILE ),	/* File keyset */
		MKACP_N( CRYPT_KEYOPT_NONE, 
				 CRYPT_KEYOPT_LAST - 1 ),	
		MKACP_N_FIXED( 0 ),	
		MKACP_S( 16, MAX_BUFFER_SIZE - 1 ),	/* Cert.object data */
...
static BOOLEAN checkAttributeRangeSpecial( const int rangeType,
										   const void *rangeInfo,
										   IN_INT_SHORT const int rangeInfoSize,
										   const int value )
	{
	/* Precondition: The range checking information is valid.  The apparent 
	   reversed check on the range type is because these are negative 
	   values */
	REQUIRES_B( rangeType <= RANGEVAL_ANY && \
				rangeType >= RANGEVAL_SUBTYPED );
	REQUIRES_B( rangeInfo != NULL );
	REQUIRES_B( isShortIntegerRangeNZ( rangeInfoSize ) );
	...
	if( rangeType == RANGEVAL_ALLOWEDVALUES )
		...
		LOOP_SMALL( i = 0, i < rangeInfoSize && \
						   allowedValuesInfo[ i ] != CRYPT_ERROR, i++ )
			{
			ENSURES_B( LOOP_INVARIANT_SMALL( i, 0, rangeInfoSize - 1 ) );
	...
	if( rangeType == RANGEVAL_SUBRANGES )
		...
		LOOP_SMALL( i = 0, i < rangeInfoSize && \
						   allowedValuesInfo[ i ].lowRange != CRYPT_ERROR, 
					i++ )
			{
			ENSURES_B( LOOP_INVARIANT_SMALL( i, 0, rangeInfoSize - 1 ) );
...
static BOOLEAN checkAttributeRangeWidechar( const void *value,
	...
	REQUIRES_B( isShortIntegerRangeNZ( maxLength ) );
				/* Also implies ( minLength/maxLength * WCHAR_SIZE ) < INT_MAX */
	...
	if( ( valueLength % WCHAR_SIZE ) || ( valueLength < WCHAR_SIZE ) )
		{
	...
	/* In theory we could also check whether we're being passed a wchar_t-
	   ...
	   with for us */
#if 0
	if( !IS_ALIGNED_OPT( value, WCHAR_SIZE ) )
		return( TRUE );
#endif /* 0 */
	...
	if( getWideChar( value ) < 0xFFFF )
		{
		return( ( valueLength < ( minLength * WCHAR_SIZE ) || \
				  valueLength > ( maxLength * WCHAR_SIZE ) ) ? \
				FALSE : TRUE );
		}
	...
	if( ( valueLength > WCHAR_SIZE * 2 ) && \
		getWideChar( value ) < 0xFF && \
		getWideChar( ( const BYTE * ) value + WCHAR_SIZE ) < 0xFF )
		{
		return( ( valueLength < ( minLength * WCHAR_SIZE ) || \
				  valueLength > ( maxLength * WCHAR_SIZE ) ) ? \
				FALSE : TRUE );
...
CHECK_RETVAL_BOOL STDC_NONNULL_ARG( ( 1 ) ) \
static BOOLEAN createAclConsistent( IN_PTR const CREATE_ACL *createACL,
									IN_BOOL const BOOLEAN doRecurse )
	{
	const PARAM_ACL *paramACL = getParamACL( createACL );
	const int paramACLSize = getParamACLSize( createACL );
	const int exceptionACLSize = getExceptionACLSize( createACL );
	...
	REQUIRES_B( isBooleanValue( doRecurse ) );

	/* Check the parameter ACLs within the create ACL */
	LOOP_SMALL( i = 0, i < paramACLSize && \
					   paramACL[ i ].valueType != PARAM_VALUE_NONE, i++ )
		{
		ENSURES_B( LOOP_INVARIANT_SMALL( i, 0, paramACLSize - 1 ) );

		if( !paramAclConsistent( &paramACL[ i ] ) )
			return( FALSE );
		}
	ENSURES_B( LOOP_BOUND_OK );
	...
	LOOP_MED( i = 0, i < exceptionACLSize && \
					 createACL->exceptionACL[ i ].type != OBJECT_TYPE_NONE, 
			  i++ )
		{
		const CREATE_ACL *exceptionACL;

		ENSURES_B( LOOP_INVARIANT_MED( i, 0, exceptionACLSize - 1 ) );

		exceptionACL = &createACL->exceptionACL[ i ];
...
int initMessageACL( void )
	...
	LOOP_MED( i = 0, i < FAILSAFE_ARRAYSIZE( compareACLTbl, COMPARE_ACL ) && \
					 compareACLTbl[ i ].compareType != MESSAGE_COMPARE_NONE, 
			  i++ )
		{
		...
		ENSURES( LOOP_INVARIANT_MED( i, 0, 
									 FAILSAFE_ARRAYSIZE( compareACLTbl, \
														 COMPARE_ACL ) - 1 ) );

		compareACL = &compareACLTbl[ i ];
		paramACL = getParamACL( compareACL );
		paramACLSize = getParamACLSize( compareACL );
	...
	LOOP_MED( i = 0, 
			  i < FAILSAFE_ARRAYSIZE( checkACLTbl, CHECK_ACL ) && \
					checkACLTbl[ i ].checkType != MESSAGE_CHECK_NONE, 
			  i++ )
		{
		const CHECK_ACL *checkACL;

		ENSURES( LOOP_INVARIANT_MED( i, 0, 
									 FAILSAFE_ARRAYSIZE( checkACLTbl, \
														 CHECK_ACL ) - 1 ) );

		checkACL = &checkACLTbl[ i ];
	...
	LOOP_MED( i = 0, 
			  ...
			  i++ )
		{
		const ATTRIBUTE_ACL_ALT *formatACL;

		ENSURES( LOOP_INVARIANT_MED( i, 0, 
									 FAILSAFE_ARRAYSIZE( formatPseudoACL, \
														 ATTRIBUTE_ACL_ALT ) - 1 ) );

		formatACL = &formatPseudoACL[ i ];
	...
	LOOP_MED( i = 0, 
			  i < FAILSAFE_ARRAYSIZE( createObjectACL, CREATE_ACL ) && \
					createObjectACL[ i ].type != OBJECT_TYPE_NONE,
			  i++ )
		{
		const CREATE_ACL *createACL;

		ENSURES( LOOP_INVARIANT_MED( i, 0, 
									 FAILSAFE_ARRAYSIZE( createObjectACL, \
														 CREATE_ACL ) - 1 ) );

		createACL = &createObjectACL[ i ];
	...
	LOOP_MED( i = 0, 
			  i < FAILSAFE_ARRAYSIZE( createObjectIndirectACL, CREATE_ACL ) && \
					createObjectIndirectACL[ i ].type != OBJECT_TYPE_NONE,
			  i++ )
		{
		const CREATE_ACL *createACL;

		ENSURES( LOOP_INVARIANT_MED( i, 0, 
									 FAILSAFE_ARRAYSIZE( createObjectIndirectACL, \
														 CREATE_ACL ) - 1 ) );

		createACL = &createObjectIndirectACL[ i ];
		ENSURES( isValidType( createACL->type ) );
			...
			switch( getSpecialRangeType( attributeACL ) )
				...
				case RANGEVAL_ALLOWEDVALUES:
					if( !checkAttributeRangeSpecial( RANGEVAL_ALLOWEDVALUES,
											...
											getSpecialRangeInfoSize( attributeACL ),
											*valuePtr ) )
						return( CRYPT_ARGERROR_NUM1 );
					break;
...
int preDispatchCheckExportAccess( IN_HANDLE const int objectHandle,
	...
	LOOP_MED( i = 0, 
			  i < FAILSAFE_ARRAYSIZE( formatPseudoACL, ATTRIBUTE_ACL_ALT ) && \
					formatPseudoACL[ i ].attribute != messageValue && 
					formatPseudoACL[ i ].attribute != CRYPT_CERTFORMAT_NONE,
			  i++ )
		{
		ENSURES( LOOP_INVARIANT_MED( i, 0, 
...
int preDispatchCheckCreate( IN_HANDLE const int objectHandle,
	...
	LOOP_MED( i = 0, i < createAclSize && \
		...
		{
		ENSURES( LOOP_INVARIANT_MED( i, 0, createAclSize - 1 ) );
		}
	ENSURES( LOOP_BOUND_OK );
	...
	if( createInfo->arg1 != 0 && createACL->exceptions[ 0 ] != 0 )
		...
		/* There are exception ACLs present, walk down the list of exception 
		   ACLs checking whether there's one for this object type */
		LOOP_SMALL( i = 0, i < exceptionACLSize && \
						   createACL->exceptions[ i ] != OBJECT_TYPE_NONE, 
					i++ )
			{
			ENSURES( LOOP_INVARIANT_SMALL( i, 0, exceptionACLSize - 1 ) );
	...
	/* If there's no object owner explicitly set, set the new object's owner 
	   ...
	   we set the owner to the default user object, and if it's being 
	   created through the (optional) crypto object handle we set the owner
