...
static int openDatabase( INOUT_PTR DBMS_INFO *dbmsInfo, 
...
STDC_NONNULL_ARG( ( 1 ) ) \
static void closeDatabase( INOUT_PTR DBMS_INFO *dbmsInfo )
	{
	DBMS_STATE_INFO *dbmsStateInfo = dbmsInfo->stateInfo;
...
static int performUpdate( INOUT_PTR DBMS_INFO *dbmsInfo, 
...
static int performStaticUpdate( INOUT_PTR DBMS_INFO *dbmsInfo, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int performQuery( INOUT_PTR DBMS_INFO *dbmsInfo, 
						 IN_STRING_OPT const char *command,
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int performStaticQuery( INOUT_PTR DBMS_INFO *dbmsInfo, 
							   IN_STRING_OPT const char *command,
							   IN_ENUM_OPT( DBMS_CACHEDQUERY ) \
								const DBMS_CACHEDQUERY_TYPE queryEntry,
							   IN_ENUM( DBMS_QUERY ) \
...
static int copyChar( OUT_BUFFER( bufMaxLen, *bufPos ) char *buffer, 
					 ...
					 IN_BOOL const BOOLEAN escapeQuotes )
	{
	int position = 0;
...
static int copyStringArg( OUT_BUFFER( bufMaxLen, *bufPos ) char *buffer, 
	...
	*bufPos = 0;
	...
	LOOP_MAX( index = 0, index < stringLen, index++ )
		...
		ENSURES( LOOP_INVARIANT_MAX( index, 0, stringLen - 1 ) );
...
int dbmsFormatQuery( OUT_BUFFER( outMaxLength, *outLength ) char *output, 
	...
	*outLength = 0;
	...
	LOOP_MAX_INITCHECK( inPos = 0, inPos < inputLength )
		...
		ENSURES( LOOP_INVARIANT_MAX_XXX( inPos, 0, inputLength - 1 ) );
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int initDbxSession( INOUT_PTR KEYSET_INFO *keysetInfoPtr, 
	...
	REQUIRES( isShortIntegerRangeNZ( sizeof( DBMS_STATE_INFO ) ) );
	...
	if( type == CRYPT_KEYSET_DATABASE_STORE )
		{
...
int endDbxSession( INOUT_PTR KEYSET_INFO *keysetInfoPtr )
	{
	assert( isWritePtr( keysetInfoPtr, sizeof( KEYSET_INFO ) ) );
	...
	if( DATAPTR_ISSET( keysetInfoPtr->keyData ) )
		...
		/* Since we may be ending the session in response to an error, we 
		   copy the low-level error information up to the keyset context in
		   case it's useful */
		copyErrorInfo( KEYSET_ERRINFO, &dbmsStateInfo->errorInfo ); 

		REQUIRES( isShortIntegerRangeNZ( keysetInfoPtr->keyDataSize ) ); 
		zeroise( dbmsStateInfo, keysetInfoPtr->keyDataSize );
