...
static int testLoop( const DES_TEST *testDES, int iterations )
	...
	ALIGN_DATA( keyData, DES_KEYSIZE, 16 );
	void *keyDataPtr = ALIGN_GET_PTR( keyData, 16 );
	LOOP_INDEX i;
	int status;

	memset( keyDataPtr, 0, DES_KEYSIZE );	/* Keep static analysers happy */
...
static int encryptCFB( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
	...
	while( noBytes > 0 )
		...
		LOOP_MED( i = 0, i < ivCount, i++ )
			{
			ENSURES( LOOP_INVARIANT_MED( i, 0, ivCount - 1 ) );
...
static int decryptCFB( INOUT_PTR CONTEXT_INFO *contextInfoPtr, 
	...
	if( ivCount > 0 )
		...
		LOOP_MED( i = 0, i < bytesToUse, i++ )
			{
			ENSURES( LOOP_INVARIANT_MED( i, 0, bytesToUse - 1 ) );
